      ...
      blocks:[
        ...
        {type:'commands',title:{th:'ฝึกวงจรเต็ม: แยก branch → แก้ → ดู diff → ย้อนกลับ',en:'Practise the full loop: branch → edit → diff → undo'},
          ...
          steps:[
            ...
            {title:{th:'สร้าง branch ใหม่และย้ายเข้าไปทันที',en:'Create a new branch and switch to it'},what:{th:'`git switch -c` ทำสองอย่างพร้อมกัน คือสร้าง branch ใหม่ (`-c` มาจาก create) แล้วย้ายคุณเข้าไปอยู่บนนั้น ชื่อ branch ควรบอกได้ว่าทำอะไร งานจริงของเราจะใช้ `feature/1-us001-patient-checkin` ในบทถัดไป แต่รอบนี้เป็นการซ้อม จึงใช้ชื่อ `practice/git-warmup` แล้วลบทิ้งตอนจบ',en:'`git switch -c` does two things at once: create a branch (`-c` for create) and move you onto it. The name should say what the work is. Our real branch will be `feature/1-us001-patient-checkin` in the next lesson; this round is a rehearsal, so we use `practice/git-warmup` and delete it at the end.'},cmd:`git switch -c practice/git-warmup`,expect:{th:'`Switched to a new branch ...` และถ้ารัน `git branch` จะเห็น `*` อยู่หน้า branch ใหม่',en:'`Switched to a new branch ...`, and `git branch` now shows `*` on the new branch.'}},
...
Refs #1`,
          note:{th:'`feat` คือชนิดของงาน, `us001` คือ scope ที่บอกว่าแตะส่วนไหนของระบบ, บรรทัดว่างเป็นตัวแบ่งที่ Git ใช้จริง (ห้ามลืม) และ `Refs #1` คือด้ายที่ร้อยกลับไปหา Issue',en:'`feat` is the kind of work, `us001` is the scope telling which part of the system it touches, the blank line is a real separator Git relies on, and `Refs #1` is the thread back to the issue.'}},
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'callout',title:{th:'ค่าเริ่มต้นของคอร์สนี้',en:'The default for this course'},text:{th:'งาน US-001 ใช้ worktree เดียว `wt-us001-patient-checkin` และ branch เดียว `feature/1-us001-patient-checkin` — repo ใหม่ที่ไฟล์ US-001 เป็น Issue แรกจะได้เลข **1** พอดี ถ้าเลขของคุณต่างออกไป ก่อนคัดลอกคำสั่ง Git ทุกบท ให้แทน `1` และ `#1` ด้วยเลข Issue จริงของคุณ ทั้ง branch, commit footer และ MR `Closes` ต้องตรงกัน รันคำสั่งงานจาก worktree นี้ ไม่ใช่ repo หลัก',en:'US-001 uses one worktree, `wt-us001-patient-checkin`, and one branch, `feature/1-us001-patient-checkin`. On a fresh repo where US-001 is your first issue the number is **1** — if yours differs, replace `1` and `#1` with your own issue number before copying Git commands in any lesson. The branch, commit footer, and MR `Closes` must match. Run task commands from this worktree, not the main folder.'}},
...
git status`,expect:{th:'อยู่บน main และ `working tree clean`',en:'You are on main with a clean tree and no leftover practice files.'}},
            ...
            {title:{th:'สร้าง worktree พร้อม branch ใหม่',en:'Create the worktree and its branch together'},what:{th:'คำสั่งนี้ทำ 3 อย่างในครั้งเดียว: สร้างโฟลเดอร์ `../wt-us001-patient-checkin`, สร้าง branch ชื่อ `feature/1-us001-patient-checkin` (จาก `-b`) และผูก branch นั้นเข้ากับโฟลเดอร์ใหม่ ส่วน `main` ท้ายคำสั่งคือจุดตั้งต้น เครื่องหมาย `..` แปลว่าวางโฟลเดอร์ไว้ข้างนอก repo หลัก ไม่ใช่ซ้อนอยู่ข้างใน',en:'This does three things at once: creates the folder `../wt-us001-patient-checkin`, creates the branch `feature/1-us001-patient-checkin` (that is what `-b` means), and binds that branch to the new folder. The trailing `main` is the starting point. The `..` places the folder beside the main repo rather than nested inside it.'},cmd:`git worktree add ../wt-us001-patient-checkin \\
  -b feature/1-us001-patient-checkin main`,expect:{th:'ข้อความ `Preparing worktree ...` และมีโฟลเดอร์ใหม่เกิดขึ้นข้าง ๆ repo เดิม',en:'A `Preparing worktree ...` message and a new folder beside the original repo.'}},
...
git status`,expect:{th:'`On branch feature/1-us001-patient-checkin` — ถ้ายังขึ้น main แปลว่า cd ผิดโฟลเดอร์',en:'`On branch feature/1-us001-patient-checkin`. If it still says main, you changed into the wrong folder.'}},
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'commands',title:{th:'ส่งงานทีละขั้น',en:'Deliver the work, step by step'},
          ...
          steps:[
            {title:{th:'ยืนยันว่าอยู่ถูก branch',en:'Confirm you are on the right branch'},what:{th:'ขั้นนี้ป้องกันอุบัติเหตุที่แก้ยากที่สุด คือ commit ลง main โดยไม่ตั้งใจ ถ้าบรรทัดแรกขึ้นว่า `On branch main` ให้หยุดทันทีและย้ายไป feature branch ก่อน',en:'This prevents the hardest accident to undo: committing to main. If the first line says `On branch main`, stop and move to your feature branch first.'},cmd:`git status`,expect:{th:'`On branch feature/1-us001-patient-checkin` พร้อมรายการไฟล์ที่จะ commit',en:'`On branch feature/1-us001-patient-checkin` with the list of files to commit.'}},
...
npm run build`,expect:{th:'ทั้งสามคำสั่งผ่าน และคุณเดิน happy path หลัง merge แล้ว',en:'All three pass and you have walked the happy path after the merge.'}},
            {title:{th:'ส่งขึ้น server',en:'Push to the server'},what:{th:'`push` ส่ง commit ในเครื่องขึ้นไปบน remote ส่วน `-u origin` เป็นการผูก branch ในเครื่องเข้ากับ branch บน server ใช้ครั้งแรกครั้งเดียว ครั้งต่อไปพิมพ์แค่ `git push` ก็พอ',en:'`push` sends your local commits to the remote. `-u origin` links your local branch to the server branch; you need it only the first time, after which plain `git push` is enough.'},cmd:`git push -u origin feature/1-us001-patient-checkin`,expect:{th:'ข้อความสำเร็จ พร้อมลิงก์สำหรับสร้าง Merge Request',en:'A success message with a link to create the Merge Request.'}}
          ],
...
H --> D[Developer continues]`,notes:{th:['ทุกจุดต้องเปิดซ้ำได้','ระบุ mock และช่องว่างตรง ๆ'],en:['Every item must be repeatable','Name mocks and gaps honestly']}},
        ...
        {type:'code',title:{th:'แบบฟอร์มส่งต่อแบบสั้น',en:'Short handoff template'},label:'handoff',code:{th:`US-001 / Issue #1 / MR <ลิงก์จริง>
...
คำถามค้าง: <คำถาม + เจ้าของคำตอบ>`,en:`US-001 / Issue #1 / MR <real link>
...
Open decisions: <question + owner>`},note:{th:'แทน #1 ด้วยเลข Issue จริง และเขียนเฉพาะสิ่งที่ตรวจแล้ว',en:'Replace #1 with your issue number and list only checks that ran.'}},
