...
Keep it short and never include anything that was not actually done.`},
        ...
        {type:'code',title:{th:'สร้าง template MR เก็บไว้ใช้งาน',en:'Create an MR template for reuse'},
...
Synthetic/mock data only. No production data.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: Draft MR',en:'Practice: draft an MR'},steps:{th:['ตรวจให้แน่ใจว่า quality gate ในบทที่ 14 ผ่านครบก่อน','เก็บงานค้างให้จบ แล้ว `git fetch` + `git merge origin/main`','push feature branch ด้วย `-u` เป็นครั้งแรก','ใช้ Prompt ร่าง MR แล้วแก้ให้ตรงกับสิ่งที่ทดสอบจริง','ใส่ `Closes #<เลข Issue ของคุณ>` ใน MR','สร้าง Draft MR แล้วให้เพื่อนอ่านและตอบว่าเข้าใจ scope หรือไม่','รับ comment อย่างน้อย 1 ข้อจากเพื่อน แล้วแก้ commit push ซ้ำให้ครบรอบ','สร้าง `docs/templates/mr-template.md` จาก template เปล่าด้านบน ในโฟลเดอร์ repo หลัก (ไม่ใช่ใน worktree) — ยังไม่ต้อง commit บทที่ 16 จะเก็บเข้า Git พร้อม issue template จากบทที่ 02'],en:['Make sure the lesson 14 quality gates pass first','Finish any leftover work, then `git fetch` + `git merge origin/main`','Push the feature branch with `-u` for the first time','Use the MR draft prompt and edit it to match what you actually tested','Add `Closes #<your issue number>` to the MR','Open a Draft MR and ask a peer whether the scope is clear','Take at least one comment from that peer and complete a fix → commit → push round','Create `docs/templates/mr-template.md` from the blank template above, in the main repo folder rather than the worktree — no commit yet; lesson 16 puts it into Git next to the lesson 02 issue template']},expected:{th:'มี Draft MR ที่ Developer สามารถเริ่ม review ได้โดยไม่ต้องถามว่า “ทำอะไรมา?” พร้อมไฟล์ template ที่ `docs/templates/mr-template.md` รออยู่ใน repo หลัก',en:'A Draft MR that a developer can start reviewing without first asking “What did you change?”, plus the template waiting at `docs/templates/mr-template.md` in the main repo.'}}
      ],
