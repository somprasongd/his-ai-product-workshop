window.COURSE = {
  ...
  lessons: [
    {
      id:'prerequisites', group:'start', no:'00', duration:'45 min', kind:'setup',
      ...
      outcomes:{th:['รู้ว่าเครื่องมือแต่ละตัวใช้ทำอะไร','ตรวจสอบว่า Git, Node.js, Editor และ AI Agent พร้อมใช้งาน','Clone Starter Repository และรัน Next.js/Storybook ได้','เข้าใจกฎข้อมูลสำหรับ training: ใช้ mock/synthetic data เท่านั้น'],en:['Know what each tool is for','Verify Git, Node.js, editor, and AI agent setup','Clone the starter repository and run Next.js/Storybook','Understand the training data rule: mock/synthetic data only']},
      blocks:[
        ...
        {type:'list',title:{th:'ติดตั้งก่อนเรียน',en:'Install before class'},items:{th:['[Git](https://git-scm.com/downloads) เวอร์ชันปัจจุบัน','[Node.js](https://nodejs.org/) 20.9+ (ข้อกำหนดขั้นต่ำของ Next.js 16)','[VS Code](https://code.visualstudio.com/) หรือ editor ที่ทีมใช้','[Chrome](https://www.google.com/chrome/) หรือ [Edge](https://www.microsoft.com/edge) สำหรับ DevTools','เลือก AI Agent อย่างน้อย 1 ตัว: [Codex CLI](https://developers.openai.com/codex/cli) หรือ [Claude Code](https://code.claude.com/docs/en/quickstart)','[GitHub account](https://github.com/signup) สำหรับ starter repo และสิทธิ์ GitLab self-hosted ของบริษัทสำหรับ workflow จริง'],en:['[Git](https://git-scm.com/downloads), current version','[Node.js](https://nodejs.org/) 20.9+ (minimum for Next.js 16)','[VS Code](https://code.visualstudio.com/) or your team editor','[Chrome](https://www.google.com/chrome/) or [Edge](https://www.microsoft.com/edge) for DevTools','At least one AI agent: [Codex CLI](https://developers.openai.com/codex/cli) or [Claude Code](https://code.claude.com/docs/en/quickstart)','A [GitHub account](https://github.com/signup) for the starter repo and access to the company self-hosted GitLab for real workflow practice']}},
        ...
        {type:'commands',title:{th:'ติดตั้งและเปิด AI Agent ครั้งแรก',en:'Install and open the AI agent for the first time'},
          lead:{th:'เลือก Agent ตัวใดตัวหนึ่งก็พอ ตัวอย่างนี้ใช้ Codex CLI บน macOS/Linux ถ้าใช้ Claude Code ให้ทำตามหน้า Quickstart ของ Claude Code แทน แต่ลำดับความคิดเหมือนกัน คือ ติดตั้ง → เปิดในโฟลเดอร์โปรเจกต์ → ทดสอบว่ามันอ่าน repo ได้',en:'One agent is enough. This example uses Codex CLI on macOS/Linux; if you use Claude Code, follow its Quickstart instead. The sequence is the same: install → open it inside the project folder → test that it can read the repo.'},
          steps:[
            {title:{th:'ติดตั้ง Codex CLI',en:'Install Codex CLI'},what:{th:'คำสั่งนี้ดาวน์โหลด install script แล้วรันทันที `-fsSL` คือการบอก curl ว่าให้เงียบ ๆ และตาม redirect ให้ด้วย ติดตั้งครั้งเดียวพอ',en:'This downloads the install script and runs it. `-fsSL` tells curl to stay quiet and follow redirects. You only need to do this once.'},cmd:`curl -fsSL https://chatgpt.com/codex/install.sh | sh`,expect:{th:'ข้อความติดตั้งสำเร็จ และอาจบอกให้เปิด Terminal ใหม่',en:'A success message, possibly asking you to open a new terminal.'}},
            {title:{th:'เปิด Agent ในโฟลเดอร์งาน',en:'Open the agent inside your working folder'},what:{th:'สิ่งสำคัญที่สุดของขั้นนี้คือ Agent จะเห็นเฉพาะโฟลเดอร์ที่คุณเปิดมันขึ้นมา ถ้าเปิดผิดโฟลเดอร์ มันจะไปแก้โปรเจกต์ผิดตัว',en:'The key idea here: the agent only sees the folder you launch it in. Launch it in the wrong folder and it will edit the wrong project.'},cmd:`codex`,expect:{th:'หน้าจอ chat ของ Agent พร้อมรับคำสั่ง และบอกว่ากำลังทำงานอยู่ใน directory ไหน',en:'The agent chat screen, ready for input and showing which directory it is working in.'}}
          ]},
        {type:'commands',title:{th:'เปิด Starter Repository ให้รันได้',en:'Get the Starter Repository running'},
          lead:{th:'ห้าคำสั่งนี้คือสิ่งที่ทำซ้ำทุกครั้งที่เริ่มโปรเจกต์ใหม่ ทำทีละบรรทัดและอ่านผลลัพธ์ทุกครั้ง',en:'These five commands are what you repeat at the start of any project. Run them one line at a time and read the output each time.'},
          steps:[
            {title:{th:'Clone โปรเจกต์ลงเครื่อง',en:'Clone the project to your machine'},what:{th:'`git clone` คือการคัดลอกทั้ง repository จาก server มาไว้ในเครื่อง พร้อมประวัติการเปลี่ยนแปลงทั้งหมด ไม่ใช่แค่ไฟล์ล่าสุด',en:'`git clone` copies the whole repository from the server to your machine, including its full change history — not just the latest files.'},cmd:`git clone https://github.com/somprasongd/his-ai-opd-checkin-starter.git`,expect:{th:'โฟลเดอร์ใหม่ชื่อ `his-ai-opd-checkin-starter` ปรากฏขึ้นในตำแหน่งปัจจุบัน',en:'A new folder named `his-ai-opd-checkin-starter` appears in the current location.'}},
            {title:{th:'เข้าไปในโฟลเดอร์โปรเจกต์',en:'Move into the project folder'},what:{th:'`cd` แปลว่า change directory คำสั่งถัดจากนี้ทั้งหมดต้องรันจากในโฟลเดอร์นี้ ไม่งั้น npm จะไม่เจอ `package.json`',en:'`cd` means change directory. Every command after this must run from inside this folder, otherwise npm cannot find `package.json`.'},cmd:`cd his-ai-opd-checkin-starter`,expect:{th:'ชื่อโฟลเดอร์ใน prompt ของ Terminal เปลี่ยนไปเป็นชื่อโปรเจกต์',en:'The folder name in your terminal prompt changes to the project name.'}},
          ...
          outro:{th:'ตอนนี้คุณควรมี Terminal 2 หน้าต่างรันค้างไว้ และ browser 2 tab (3000 กับ 6006) นี่คือ “ที่ทำงาน” มาตรฐานของเราตลอดคอร์ส',en:'You should now have two terminals running and two browser tabs (3000 and 6006). This is our standard workspace for the whole course.'}},
...
4. Where are the main folders for pages, components, and mock data?`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: ยืนยันว่าเครื่องพร้อมจริง',en:'Practice: confirm your machine is genuinely ready'},steps:{th:['รันคำสั่งตรวจเวอร์ชันทั้งสามให้ผ่าน','Clone starter repo แล้วรัน `npm install` จนจบ','เปิด `npm run dev` แล้วดูหน้าเว็บที่ localhost:3000','เปิด Terminal ที่สอง รัน `npm run storybook` แล้วเปิด localhost:6006','ส่ง Prompt แรกให้ Agent แล้วตรวจว่ามันอ้างถึงไฟล์จริงในโปรเจกต์'],en:['Pass all three version checks','Clone the starter repo and let `npm install` finish','Run `npm run dev` and view localhost:3000','In a second terminal run `npm run storybook` and open localhost:6006','Send the first prompt and check that the agent cites real project files']},expected:{th:'เห็นหน้า Training Starter และ Storybook เปิดได้โดยไม่มี error สำคัญ และ Agent อธิบายโปรเจกต์ได้จากไฟล์จริง',en:'The Training Starter page and Storybook both open without significant errors, and the agent describes the project from its real files.'}}
      ],
      ...
      wrap:{th:['Environment พร้อมก่อนเข้าเนื้อหา','รู้จุดแบ่งระหว่าง training mock กับ production system','รู้คำสั่งเปิด Next.js และ Storybook และรู้ว่าแต่ละคำสั่งทำอะไร'],en:['Environment ready before the course','Know the boundary between training mocks and production systems','Know how to start Next.js and Storybook — and what each command does']}
    },
    {
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'diagram',title:{th:'Git ในภาพเดียว',en:'Git in one picture'},
          lead:{th:'เส้นทางนี้วนเป็นวงกลม ทุกครั้งที่ทำงานหนึ่งชิ้นเสร็จ คุณจะกลับมาที่จุดเริ่มเสมอ',en:'This path is a loop: every finished piece of work brings you back to the start.'},
          diagram:`flowchart LR\nL[Local files] -->|git add + commit| B[Feature branch]\nB -->|git push| R[Remote GitHub/GitLab]\nR -->|Merge Request| M[main]\nM -->|git pull| L`,
          notes:{th:['`git add` + `git commit` = เลือกการเปลี่ยนแปลงแล้วบันทึกเป็นเวอร์ชันพร้อมเหตุผล','`git push` = ส่งเวอร์ชันในเครื่องขึ้นไปบน server ให้คนอื่นเห็น','Merge Request = ขออนุญาตรวมงานเข้า main โดยมีคน review','`git pull` = ดึงงานล่าสุดของทีมลงมา ทำก่อนเริ่มงานใหม่เสมอ'],en:['`git add` + `git commit` = select changes and save them as a version with a reason','`git push` = send local versions to the server so others can see them','Merge Request = ask to fold work into main, with review','`git pull` = fetch the team’s latest work; always do this before starting something new']}},
        ...
        {type:'commands',title:{th:'ฝึกวงจรเต็ม: แยก branch → แก้ → ดู diff → ย้อนกลับ',en:'Practise the full loop: branch → edit → diff → undo'},
          ...
          steps:[
            ...
            {title:{th:'สร้าง branch ใหม่และย้ายเข้าไปทันที',en:'Create a new branch and switch to it'},what:{th:'`git switch -c` ทำสองอย่างพร้อมกัน คือสร้าง branch ใหม่ (`-c` มาจาก create) แล้วย้ายคุณเข้าไปอยู่บนนั้น ชื่อ branch ควรบอกได้ว่ากำลังทำ Issue ไหน เช่น `feature/us001-patient-search`',en:'`git switch -c` does two things at once: create a branch (`-c` for create) and move you onto it. The name should say which issue it serves, e.g. `feature/us001-patient-search`.'},cmd:`git switch -c feature/us001-patient-search`,expect:{th:'`Switched to a new branch ...` และถ้ารัน `git branch` จะเห็น `*` อยู่หน้า branch ใหม่',en:'`Switched to a new branch ...`, and `git branch` now shows `*` on the new branch.'}},
        ...
        {type:'practice',title:{th:'ลงมือทำ: Branch แรกของคุณ',en:'Practice: your first branch'},steps:{th:['เช็กว่าอยู่ main และ working tree สะอาด','สร้าง branch `feature/us001-patient-search`','แก้ README เล็กน้อย','ใช้ `git diff` ดูการเปลี่ยนแปลง แล้ว restore กลับโดยไม่ commit','อธิบายให้เพื่อนฟังว่าแต่ละคำสั่งทำอะไร'],en:['Confirm you are on main with a clean working tree','Create branch `feature/us001-patient-search`','Make a small README edit','Use `git diff` to inspect it, then restore without committing','Explain to a peer what each command did']},expected:{th:'สามารถสร้าง branch, เห็น diff และย้อนการแก้ไฟล์ที่ยังไม่ commit ได้ โดยอธิบายเหตุผลของแต่ละคำสั่งได้',en:'You can create a branch, inspect a diff, and undo an uncommitted edit — and explain why each command was used.'}}
      ],
...
git status`,expect:{th:'`On branch feature/us001-patient-checkin` — ถ้ายังขึ้น main แปลว่า cd ผิดโฟลเดอร์',en:'`On branch feature/us001-patient-checkin`. If it still says main, you changed into the wrong folder.'}},
        ...
        {type:'commands',title:{th:'เมื่องานจบแล้ว: เก็บกวาด worktree',en:'When the work is done: clean up the worktree'},
          lead:{th:'อย่าปล่อยให้มี worktree ค้างเป็นสิบอัน เพราะจะสับสนว่าโฟลเดอร์ไหนคืองานไหน ทำขั้นตอนนี้หลังจาก push และเปิด MR แล้วเท่านั้น',en:'Do not let a dozen worktrees pile up — you will lose track of which folder is which. Only do this after you have pushed and opened the MR.'},
          steps:[
    ...
    },
    {
      ...
      blocks:[
        {type:'prose',title:{th:'Storybook คืออะไร และทำไม PM/BA/Designer ควรใช้',en:'What Storybook is, and why PM/BA/Design should use it'},body:{th:[
          ...
          'คำว่า story หนึ่งอัน = component หนึ่งตัว ในสถานะหนึ่งสถานะ เช่น `PatientSearch / Empty` คือ “ช่องค้นหาผู้ป่วย ในตอนที่ค้นแล้วไม่พบใครเลย” ทั้งชื่อและเนื้อหาอ่านได้โดยไม่ต้องรู้จัก React',
          'กติกาที่เราจะใช้คือ **component กับ story ต้องเกิดพร้อมกันเสมอ** ถ้า Agent ส่ง component มาโดยไม่มี story แปลว่ามันส่งของที่คุณตรวจไม่ได้ ให้ตีกลับ',
          'ข้อควรระวังคือ story ไม่ใช่ของฟรี ทุก story ที่เพิ่มคือของที่ต้องดูแลต่อ ดังนั้นสร้างเฉพาะ state ที่ **มีความหมายต่อผู้ใช้หรือ acceptance criteria** ไม่ใช่สร้างเพื่อให้ตัวเลขดูเยอะ'
        ],en:[
          ...
          'One story = one component in one state. `PatientSearch / Empty` means “the patient search field when the search found nobody.” Both the name and the content are readable without knowing React.',
          'Our rule: **a component and its story are always created together.** If the agent delivers a component with no story, it has delivered something you cannot review — send it back.',
          'One caution: stories are not free. Each one is something to maintain. Create only states that are **meaningful to users or to an acceptance criterion**, not stories that exist to inflate a count.'
        ]}},
        ...
        {type:'commands',title:{th:'เปิด Storybook และหาสิ่งที่ต้องตรวจ',en:'Open Storybook and find what to review'},
          ...
          steps:[
            ...
            {title:{th:'ไล่ดู story ทีละ state',en:'Walk through the states one at a time'},what:{th:'คลิกชื่อ component ทางซ้าย แล้วคลิก story ย่อยทีละอัน ดูทีละ state อย่ารีบ และจดสิ่งที่สะดุดตาเป็นภาษาของ product เช่น “ข้อความตอนไม่พบข้อมูลยังไม่บอกว่าผู้ใช้ควรทำอะไรต่อ”',en:'Click a component on the left, then each story beneath it. Take them one at a time and note anything that snags, in product language: “the empty message does not tell the user what to do next.”'},expect:{th:'มีรายการ feedback ที่เป็นพฤติกรรม/ข้อความ ไม่ใช่คำสั่งว่าต้องเขียนโค้ดอย่างไร',en:'A feedback list about behaviour and wording, not instructions about how to write code.'}}
          ]},
        {type:'prompt',title:{th:'Prompt: สร้าง PatientSearch พร้อม Story',en:'Prompt: build PatientSearch with its stories'},
...
When finished, list every file you created or changed.`},
          after:{th:['เปิด Storybook แล้วต้องเห็นครบทั้ง 5 story ถ้าขาดข้อไหนให้ตีกลับพร้อมระบุชื่อ story','ดูว่าข้อมูลเหมือนเดิมทุกครั้งที่ refresh ถ้าเปลี่ยนไปเรื่อย ๆ แปลว่าใช้ข้อมูลสุ่ม ซึ่งทำให้ review ยาก','ตรวจว่ายังไม่มีการแก้ไฟล์ page จริง ด้วย `git status`'],en:['Open Storybook and confirm all five stories exist; if one is missing, send it back naming the story','Refresh and confirm the data stays identical — random data makes review harder','Use `git status` to confirm no real page file was touched']}},
...
Summarize how you addressed each point.`},
          after:{th:['กลับไปเปิด story เดิมอีกครั้งเพื่อยืนยันด้วยตา ไม่ใช่เชื่อคำสรุป','ตรวจว่า story อื่นที่เคยผ่านแล้ว ยังไม่พังจากการแก้รอบนี้'],en:['Reopen the same stories and confirm with your eyes rather than trusting the summary','Check that previously approved stories did not break in this round']}},
        ...
        {type:'practice',title:{th:'ลงมือทำ: PatientSearch stories',en:'Practice: PatientSearch stories'},steps:{th:['ส่ง prompt สร้าง component ให้ Agent','รัน Storybook','เปิด story ทั้ง 5 state ทีละตัว','จด feedback ในภาษาของ Product ไม่ต้องอธิบายวิธีเขียน code','ส่ง Feedback Prompt แล้ว review ซ้ำจนผ่าน'],en:['Send the component prompt to the agent','Run Storybook','Open all five states one by one','Write product-language feedback without prescribing code','Send the feedback prompt and review again until it passes']},expected:{th:'มี PatientSearch component กับ stories 5 state ที่เปิดได้แยกกัน และยังไม่ผูกกับ page จริง',en:'A PatientSearch component with five independently viewable stories, not yet integrated into the real page.'}}
      ],
      quiz:{q:{th:'ใครควรเป็นเจ้าของการนิยาม “ต้องมี Story อะไรบ้าง”?',en:'Who should own the decision about which stories matter?'},options:{th:['AI อย่างเดียว','Human จาก requirement/state แล้วให้ AI implement','Storybook CLI'],en:['AI alone','Humans derive them from requirements/states, then AI implements','Storybook CLI']},answer:1,why:{th:'AI ช่วยเสนอได้ แต่ state ที่ต้องครอบคลุมเป็น product decision',en:'AI can suggest, but required state coverage is a product decision.'}},
    ...
    },
    {
      ...
      blocks:[
        {type:'prose',title:{th:'จาก “เอกสารที่ไม่มีใครอ่าน” สู่ “ตัวอย่างที่กดดูได้”',en:'From “documents nobody reads” to “examples you can click”'},body:{th:[
          ...
          'สิ่งที่บทนี้ทำคือเชื่อมสองอย่างเข้าด้วยกัน โดยให้ **AC แต่ละข้อที่เป็นสถานะ กลายเป็น story ที่เปิดดูได้จริง** เมื่อทำแบบนี้ การตรวจว่า requirement ครบหรือไม่ เปลี่ยนจากการอ่านเอกสารเทียบกับความจำ เป็นการเปิด Storybook ไล่ดูทีละรายการ',
          ...
          'This lesson connects them by turning **each state-shaped AC into a story you can actually open**. Checking requirement coverage then shifts from reading a document against memory to clicking through Storybook item by item.',
        ...
        {type:'two',title:{th:'Acceptance → Story',en:'Acceptance → Story'},left:{title:{th:'Requirement',en:'Requirement'},items:{th:['AC: ไม่พบผู้ป่วย','AC: ระบบค้นหาขัดข้อง','AC: พบผู้ป่วยหลายคน'],en:['AC: No patient found','AC: Search service fails','AC: Multiple patients found']}},right:{title:{th:'Story',en:'Story'},items:{th:['Empty','Error','WithResults'],en:['Empty','Error','WithResults']}}},
...
Also tell me how to view this test running inside Storybook.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: AC coverage matrix',en:'Practice: AC coverage matrix'},steps:{th:['เปิด requirement US-001','แยก AC ออกเป็นสองกอง: สถานะ กับ พฤติกรรม','จับคู่ AC ที่เป็นสถานะกับ story ที่มีอยู่ และหาว่าข้อไหนยังไม่มี story','เลือก 1 behavior flow แล้วเขียน Given/When/Then','ส่ง Prompt ให้ Agent implement แล้วเปิดดูใน Storybook ทีละขั้น'],en:['Open requirement US-001','Sort the ACs into two piles: states and behaviours','Match state ACs to existing stories and find which have none','Choose one behaviour flow and write Given/When/Then','Send the prompt, then watch it run step by step in Storybook']},expected:{th:'มีหลักฐานชัดว่า AC สำคัญถูก represent ด้วย story หรือ interaction และผู้เรียนอธิบาย coverage ได้',en:'There is clear evidence that key ACs are represented by stories or interactions, and you can explain the coverage.'}}
      ],
    ...
    },
    {
      ...
      blocks:[
        {type:'prose',title:{th:'ชิ้นส่วนดีทุกชิ้น ไม่ได้แปลว่าประกอบแล้วจะดี',en:'Good pieces do not guarantee a good assembly'},body:{th:[
          ...
          'ปัญหาเหล่านี้ไม่มีทางเห็นใน Storybook เพราะแต่ละ story เริ่มจากสถานะสะอาดเสมอ วิธีเดียวที่จะเจอคือเปิดหน้าจริงแล้วเดินทั้ง flow เหมือนเป็นผู้ใช้จริง',
          ...
          'None of these can appear in Storybook, because each story always starts from a clean state. The only way to find them is to open the real page and walk the whole flow like a user.',
...
When finished, describe step by step how the screen changes state from the first search to success.`},
          after:{th:['อย่าเชื่อคำอธิบาย ให้เปิด `/opd/check-in` แล้วเดินเองทุกเส้นทางในผังด้านบน','รัน `git diff --stat` ดูจำนวนไฟล์ ถ้าเยอะเกินคาดให้หยุดและถามก่อน','กลับไปเปิด Storybook อีกรอบ เพื่อยืนยันว่า story เดิมยังไม่พัง'],en:['Do not trust the description — open `/opd/check-in` and walk every path in the diagram yourself','Run `git diff --stat` and check the file count; if it is higher than expected, stop and ask','Reopen Storybook to confirm the existing stories did not break']}},
    ...
    },
    {
      ...
      blocks:[
        {type:'prose',title:{th:'ตรวจงาน AI โดยไม่ต้องอ่านโค้ดทุกบรรทัด',en:'Reviewing AI work without reading every line'},body:{th:[
          ...
          'เทคนิคที่ได้ผลที่สุดและใช้ได้ทันทีคือ **ทายก่อนดู** ก่อนเปิด diff ให้เดาในใจว่างานชิ้นนี้ควรแตะกี่ไฟล์ และควรเป็นไฟล์ประเภทไหน เช่น “เพิ่ม Empty state ให้ PatientSearch น่าจะแตะ 2 ไฟล์ คือ component กับ story ของมัน”',
          ...
          'The most effective technique, usable immediately, is **predict before you look**. Before opening the diff, guess how many files this work should touch and of what kind: “adding an Empty state to PatientSearch should touch two files — the component and its story.”',
...
If you cannot justify a file, say so plainly.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: Scope audit',en:'Practice: scope audit'},steps:{th:['ก่อนเปิด diff ให้เขียนลงกระดาษว่าคาดว่าจะเปลี่ยนกี่ไฟล์ และไฟล์อะไร','รัน `git status` และ `git diff --stat` แล้วเทียบกับที่เดาไว้','เปิด diff ของไฟล์ที่ไม่ได้คาดคิด','ส่ง Diff Review Prompt ให้ Agent','รัน lint → test → build ทีละตัว แล้วบันทึกผล'],en:['Before opening the diff, write down how many files and which files you expect','Run `git status` and `git diff --stat`, then compare with your prediction','Open the diff for any file you did not expect','Send the Diff Review Prompt','Run lint → test → build one at a time and record the results']},expected:{th:'สามารถอธิบาย changed-file set ได้ ไม่มีไฟล์นอก scope ที่อธิบายไม่ได้ และ quality checks ผ่าน',en:'You can explain the changed-file set, no unexplained out-of-scope files remain, and quality checks pass.'}}
      ],
      ...
      blocks:[
        ...
        {type:'commands',title:{th:'ส่งงานทีละขั้น',en:'Deliver the work, step by step'},
          ...
          steps:[
            ...
            {title:{th:'อ่าน diff เป็นรอบสุดท้าย',en:'Read the diff one last time'},what:{th:'นี่คือโอกาสสุดท้ายที่จะเห็นสิ่งที่กำลังจะถูกบันทึกถาวร ใช้เวลาสองนาทีก็พอ แต่อย่าข้าม เพราะหลัง commit แล้วการแก้จะยุ่งกว่าเดิม',en:'This is your last chance to see what is about to be recorded permanently. Two minutes is enough, but do not skip it — undoing after a commit is messier.'},cmd:`git diff`,expect:{th:'ไม่มีไฟล์ที่อธิบายความจำเป็นไม่ได้ กด `q` เพื่อออก',en:'No file you cannot justify. Press `q` to exit.'}},
            {title:{th:'เลือกไฟล์ที่จะบันทึก',en:'Stage the files to record'},what:{th:'`git add .` คือการเลือกทุกไฟล์ที่เปลี่ยนไปเข้าสู่ชุดที่จะ commit จุด `.` หมายถึง “ทุกอย่างในโฟลเดอร์นี้” — ปลอดภัยก็ต่อเมื่อคุณเพิ่งตรวจ diff มาแล้วจริง ๆ',en:'`git add .` selects every changed file into the commit set. The `.` means “everything in this folder” — safe only because you just reviewed the diff.'},cmd:`git add .`,expect:{th:'`git status` แสดงไฟล์เป็นสีเขียวใต้หัวข้อ `Changes to be committed`',en:'`git status` shows the files in green under `Changes to be committed`.'}},
            {title:{th:'บันทึกพร้อมเหตุผล',en:'Commit with a reason'},what:{th:'ข้อความ commit ควรบอกว่า “ทำอะไร” ไม่ใช่ “แก้ไฟล์ไหน” คำนำหน้า `feat:` เป็นธรรมเนียมที่บอกว่านี่คือฟีเจอร์ใหม่ (ถ้าเป็นการแก้บั๊กจะใช้ `fix:`) และควรอ้างรหัส Issue ไว้ด้วยเพื่อให้ตามย้อนกลับได้',en:'A commit message should say what was done, not which files changed. The `feat:` prefix conventionally marks a new feature (`fix:` marks a bug fix), and referencing the issue id makes it traceable.'},cmd:`git commit -m "feat(us001): add OPD patient check-in flow"`,expect:{th:'สรุปจำนวนไฟล์และบรรทัดที่ถูกบันทึก',en:'A summary of how many files and lines were recorded.'}},
...
Keep it short and never include anything that was not actually done.`},
        ...
        {type:'callout',title:{th:'ใช้กับ GitLab self-hosted ได้เหมือนกัน',en:'The same workflow works with self-hosted GitLab'},text:{th:'Starter อยู่ GitHub เพื่อให้ Fork/Download ง่าย แต่ผู้เรียนสามารถสร้าง project ใน GitLab บริษัท แล้วเปลี่ยน remote และ Push feature branch เข้า GitLab ได้',en:'The starter lives on GitHub for easy fork/download, but learners can create a project in the company GitLab, change the remote, and push the feature branch there.'}},
        ...
        {type:'practice',title:{th:'ลงมือทำ: Draft MR',en:'Practice: draft an MR'},steps:{th:['ตรวจให้แน่ใจว่า quality gate ผ่านครบก่อน','commit ด้วยข้อความที่บอกว่าทำอะไร','push feature branch','ใช้ Prompt ร่าง MR แล้วแก้ให้ตรงกับสิ่งที่ทดสอบจริง','สร้าง Draft MR แล้วให้เพื่อนอ่านและตอบว่าเข้าใจ scope หรือไม่'],en:['Confirm all quality gates passed first','Commit with a message that says what was done','Push the feature branch','Use the MR prompt, then edit it to match what you really tested','Open a Draft MR and ask a peer whether the scope is clear']},expected:{th:'มี Draft MR ที่ Developer สามารถเริ่ม review ได้โดยไม่ต้องถามว่า “ทำอะไรมา?”',en:'A Draft MR that a developer can start reviewing without first asking “What did you change?”'}}
      ],
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'capstone',steps:[
          {title:{th:'1 · ตั้ง Workspace',en:'1 · Set up the workspace'},hint:{th:'เริ่มจาก main ที่ clean แล้วใช้ worktree แยก task',en:'Start from clean main and isolate the task with a worktree'},guide:{th:'รัน `git status` → `git pull` → `git worktree add ../wt-us001-capstone -b feature/us001-capstone main` → `cd` เข้า worktree → `git status` เพื่อยืนยันว่าอยู่ branch ใหม่ → `npm install` (ดูรายละเอียดในบทที่ 03)',en:'Run `git status` → `git pull` → `git worktree add ../wt-us001-capstone -b feature/us001-capstone main` → `cd` into it → `git status` to confirm the new branch → `npm install` (details in lesson 03)'}},
          ...
          {title:{th:'3 · Component + State Map',en:'3 · Component + State Map'},hint:{th:'อย่าเริ่มจากหน้าใหญ่ก้อนเดียว และเขียนรายการของตัวเองก่อนถาม Agent',en:'Do not start with one giant page component, and write your own list before asking the agent'},guide:{th:'เริ่ม PatientSearch → PatientResults → SelectedPatient → CheckInForm → Confirmation → Success แล้วกำหนด meaningful states เฉพาะที่แต่ละ component ต้องรับผิดชอบ จากนั้นใช้ Prompt วิจารณ์จากบทที่ 06',en:'Start with PatientSearch → PatientResults → SelectedPatient → CheckInForm → Confirmation → Success, define only the meaningful states each owns, then use the critique prompt from lesson 06'}},
          {title:{th:'4 · Storybook First',en:'4 · Storybook First'},hint:{th:'สร้าง component + story คู่กัน และใช้ mock deterministic',en:'Create component + story pairs with deterministic mocks'},guide:{th:'ให้ Agent ทำ PatientSearch ก่อน พร้อม Default/Loading/Empty/WithResults/Error (ใช้ Prompt จากบทที่ 07) → เปิด Storybook review ทีละ story → ส่ง feedback เป็นภาษา product → ผ่านแล้วค่อยทำ component ถัดไป',en:'Have the agent build PatientSearch first with Default/Loading/Empty/WithResults/Error (prompt in lesson 07) → open Storybook and review story by story → send product-language feedback → only then move to the next component'}},
          {title:{th:'5 · Interaction',en:'5 · Interaction'},hint:{th:'เลือก behavior สำคัญ 1 flow เขียน Given/When/Then',en:'Choose one important behavior and write Given/When/Then'},guide:{th:'เช่น Given หน้าค้นหาแสดงอยู่ / When กรอก HN 65000123 แล้วกดค้นหา / Then แสดง Somchai Jaidee → ส่ง Prompt จากบทที่ 08 → เปิดดู interaction ทีละขั้นใน Storybook',en:'For example: Given the search screen is displayed / When HN 65000123 is entered and Search clicked / Then Somchai Jaidee appears → send the prompt from lesson 08 → watch the interaction step by step in Storybook'}},
          ...
          {title:{th:'8 · Diff & Quality Gates',en:'8 · Diff & Quality Gates'},hint:{th:'คาดการณ์ changed files ก่อนดูจริง',en:'Predict the changed files before reviewing them'},guide:{th:'เขียนคาดการณ์ก่อน → `git status` → `git diff --stat` → เปิด diff ของไฟล์ที่ไม่คาดคิด → Diff Review Prompt จากบทที่ 12 → `npm run lint` → `npm run test` → `npm run build` ทีละตัว',en:'Write your prediction first → `git status` → `git diff --stat` → open diffs of unexpected files → Diff Review Prompt from lesson 12 → `npm run lint` → `npm run test` → `npm run build`, one at a time'}},
          {title:{th:'9 · Draft MR',en:'9 · Draft MR'},hint:{th:'MR ต้องเล่า intent, scope, states และ evidence',en:'The MR should tell intent, scope, states, and evidence'},guide:{th:'`git status` ยืนยัน branch → `git add .` → `git commit -m "feat(us001): ..."` → `git push -u origin <branch>` → ใช้ Prompt ร่าง MR จากบทที่ 13 → ตัดสิ่งที่ยังไม่ได้ทดสอบจริงออก → เปิด Draft MR แล้วให้เพื่อน review ความชัดเจน',en:'`git status` to confirm the branch → `git add .` → `git commit -m "feat(us001): ..."` → `git push -u origin <branch>` → use the MR draft prompt from lesson 13 → remove anything you did not actually test → open a Draft MR and have a peer review its clarity'}}
        ]},
...
4. มี component ไหนที่ยังไม่มี story คู่กันหรือไม่
...
4. Are there components without a matching story?
...
Answer briefly as a numbered list, and do not act until I tell you to.`},
        ...
        {type:'practice',title:{th:'ผลลัพธ์ที่คาดหวังจาก Capstone',en:'Expected Capstone result'},steps:{th:['Storybook แสดง meaningful states ของ OPD Check-in','`/opd/check-in` เดิน flow ได้ด้วย mock data','ผู้เรียนอธิบาย error หนึ่งกรณีและวิธี verify fix ได้','Diff อยู่ใน scope และ quality checks ผ่าน','Draft MR พร้อม evidence สำหรับ Developer review'],en:['Storybook shows meaningful OPD Check-in states','`/opd/check-in` runs end-to-end on mock data','You can explain one error and how the fix was verified','Diff stays in scope and quality checks pass','Draft MR contains evidence for developer review']},expected:{th:'ไม่ต้องได้ “โค้ดที่สมบูรณ์แบบ” เป้าหมายคือ workflow ที่ควบคุมได้ ตรวจสอบได้ และ handoff ได้',en:'The goal is not “perfect code”; it is a controllable, verifiable workflow that can be handed off.'}}
      ],
