...
Synthetic/mock data only. No production data.`},
    ...
    },
    {
      ...
      intro:{th:'คุณเขียน Issue เองมาแล้วหนึ่งใบ และร่าง MR เองมาแล้วหนึ่งใบ พร้อมเก็บแบบฟอร์มเปล่าของทั้งคู่ไว้ที่ `docs/templates/` ตั้งแต่บทที่ 02 กับ 15 รอบนี้เราจะเอาสองไฟล์นั้นเข้า Git เป็น template ของทีม แล้วห่อเป็น skill ให้ Agent ทำซ้ำได้ในมาตรฐานเดียวกันทุกครั้ง',en:'You have written one issue by hand and drafted one MR by hand, and kept blank forms of both under `docs/templates/` since lessons 02 and 15. This lesson commits them into Git as team templates and wraps them in skills, so the agent repeats them to the same standard every time.'},
      outcomes:{th:['บอกได้ว่างานแบบไหนควรทำเป็น skill และแบบไหนไม่ควร','เอา template จากบทที่ 02 และ 15 เข้า Git พร้อม skill ใหม่ที่ `.agents/skills` ให้ทุกเครื่องมืออ่านได้','สร้าง skill สำหรับสร้าง Issue และร่าง MR ตาม template และลิงก์เข้ากับเครื่องมือที่ใช้','ตั้งเส้นความปลอดภัย: Agent ร่างและเสนอคำสั่ง คนเป็นผู้อนุมัติ'],en:['Tell which work belongs in a skill and which does not','Commit the lesson 02 and 15 templates into Git alongside new skills under `.agents/skills`, readable by every tool','Build skills for filing issues and drafting MRs, and link them into the tool you use','Set the safety line: the agent drafts and proposes, a human approves']},
      blocks:[
        {type:'prose',title:{th:'ทำเองก่อน แล้วค่อยทำให้เป็นระบบ',en:'By hand first, automated second'},body:{th:[
          'ลำดับของสองวันที่ผ่านมาไม่ใช่เรื่องบังเอิญ เราให้คุณ **เขียน Issue เองในบทที่ 02** และ **ตรวจกับแก้ร่าง MR เองในบทที่ 15** ก่อน เพราะถ้าคุณไม่เคยเขียนเอง คุณจะไม่มีเกณฑ์ในหัวไว้ตรวจของที่ Agent ร่างมา และจะลงเอยด้วยการกด approve ทุกอย่างที่มันเสนอ',
          ...
          'The order of the last two days was deliberate. You **wrote the issue by hand in lesson 02** and **reviewed and edited the MR draft yourself in lesson 15**, because without having written one you would have no standard in your head to judge the agent’s draft against — and would end up approving whatever it proposed.',
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'capstone',steps:[
          ...
          {title:{th:'10 · Diff & Quality Gates',en:'10 · Diff & Quality Gates'},hint:{th:'คาดการณ์ changed files ก่อนดูจริง',en:'Predict the changed files before reviewing them'},guide:{th:['เขียนคาดการณ์ changed files ก่อน','รัน `git status`','รัน `git log --oneline main..HEAD` อ่านว่า branch นี้เล่าเรื่องอะไร','รัน `git diff main...HEAD --stat`','เปิด diff ของไฟล์ที่ไม่คาดคิด','กวาดหาไฟล์ที่ห้าม commit เช่น `.env` หรือไฟล์ทดลองของ Agent','ใช้ Diff Review Prompt จากบทที่ 14','รัน `npm run lint`','รัน `npm run test`','รัน `npm run build`'],en:['Write your prediction first','Run `git status`','Run `git log --oneline main..HEAD` and read what this branch says it did','Run `git diff main...HEAD --stat`','Open diffs of unexpected files','Scan for files that must never be committed, such as `.env` or agent scratch files','Use the Diff Review Prompt from lesson 14','Run `npm run lint`','Run `npm run test`','Run `npm run build`']}},
