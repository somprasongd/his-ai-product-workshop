...
git remote -v`,expect:{th:'สองบรรทัดที่มีชื่อบัญชีของคุณ และลงท้ายด้วย `(fetch)` กับ `(push)`',en:'Two lines containing your username, ending in `(fetch)` and `(push)`.'}},
              {title:{th:'ติดตั้งและล็อกอิน GitHub CLI',en:'Install and sign in to the GitHub CLI'},what:{th:'`gh` คือเครื่องมือแยกจากตัว Agent ที่ให้คุณและ Agent ทำงานกับ Issue และ Pull Request จาก Terminal ได้ ตั้งแต่บทที่ 05 Explore Prompt จะให้ Agent รัน `gh issue view` เพื่ออ่าน Issue จริงของคุณ และบทที่ 16 จะใช้ต่อเป็น skill ติดตั้งด้วย `brew install gh` (macOS) หรือ `winget install GitHub.cli` (Windows) แล้วล็อกอิน',en:'`gh` is a tool separate from the agent that lets you — and the agent — work with issues and pull requests from the terminal. From lesson 05 the Explore Prompt has the agent run `gh issue view` to read your real issue, and lesson 16 builds on it with a skill. Install with `brew install gh` (macOS) or `winget install GitHub.cli` (Windows), then sign in.'},cmd:`gh auth login
...
Cite real file names for every point.`},
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'callout',title:{th:'Requirement จริงอยู่ที่ไหน',en:'Where the real requirement lives'},
          text:{th:'ไดอะแกรมกับตารางข้างบนเป็นภาพแนวคิด ไฟล์จริงคือ story ที่ตัดมาแล้ว 1 ชิ้น: `docs/requirements/US-001-opd-checkin.md` ใน starter repo ของคุณ (เปิดอ่านได้เลย) ส่วนโจทย์เต็ม “OPD Check-in Lite” ระดับ epic ไม่มีไฟล์ในคอร์สนี้ ไฟล์นั้นมี AC ดิบอยู่แล้ว — งานของคุณคือหั่นใหม่เป็น states กับ behaviour ตรึงค่าจาก mock data แล้วกลายเป็น Issue ในบล็อกถัดไป',en:'The diagram and the table above are the concept. The real artifact is one already-cut story: `docs/requirements/US-001-opd-checkin.md` in your starter repo (open it now). The full “OPD Check-in Lite” topic at epic level has no file in this course. That file already contains raw ACs — your job is to re-cut them into states and behaviours with fixed mock-data values, turning them into the issue in the next block.'}},
...
Synthetic/mock data only. No production data, no real patient records.`},
        ...
        {type:'code',title:{th:'เก็บแบบฟอร์มเปล่าไว้ในโปรเจกต์',en:'Keep the blank form in the project'},
          lead:{th:'สร้างไฟล์นี้ไว้ตั้งแต่ตอนนี้ จะได้เริ่ม Issue ใบถัดไปจากโครงเดียวกัน และบทที่ 16 จะให้ Agent อ่านไฟล์นี้ตอนร่าง Issue ให้ — สร้างด้วย editor ได้เลย ยังไม่ต้อง commit เพราะยังไม่มี branch ของใคร ไฟล์จะโผล่ใน `git status` เป็น untracked file ไปก่อน จนบทที่ 16 จะเก็บเข้า Git เป็นมาตรฐานของทีมพร้อม agent skill',en:'Create this file now, so the next issue starts from the same shape — lesson 16 has the agent read this exact file when drafting issues for you. Use your editor; no commit yet, because no branch exists at this point. The file sits in `git status` as an untracked file until lesson 16 commits it into Git as a team standard together with the agent skills.'},
          label:'docs/templates/issue-template.md',code:{
        ...
        {type:'agent-setup',store:'repoHost',title:{th:'เปิด Issue จริงบน GitHub หรือ GitLab',en:'File the real issue on GitHub or GitLab'},
          ...
          tools:[
            {id:'github',name:{th:'GitHub',en:'GitHub'},steps:[
              ...
              {title:{th:'เริ่ม Issue ใหม่',en:'Start a new issue'},what:{th:'กด **New issue** แล้ววาง Title จาก template ลงช่อง title',en:'Click **New issue** and paste the template title into the title field.'}},
              {title:{th:'วางเนื้อหา Issue',en:'Paste the issue body'},what:{th:'วางเนื้อ template ฉบับที่แก้แล้วลงช่อง description — Markdown ใช้ได้ทุกรูปแบบ',en:'Paste your revised template body into the description — full Markdown works.'}},
            ...
            {id:'gitlab',name:{th:'GitLab',en:'GitLab'},steps:[
              ...
              {title:{th:'เริ่ม Issue ใหม่',en:'Start a new issue'},what:{th:'กดปุ่ม **New item** ที่มุมขวาบน แล้วตั้ง **Type = Issue** (รุ่นเก่าปุ่มจะชื่อ **New issue** และไม่มีช่อง Type) จากนั้นวาง Title จาก template ลงช่อง Title',en:'Click **New item** in the upper-right corner and set **Type = Issue** (older versions name this button **New issue** and have no Type field), then paste the template title into the Title field.'}},
              {title:{th:'วางเนื้อหา Issue',en:'Paste the issue body'},what:{th:'วางเนื้อ template ฉบับที่แก้แล้วลงช่อง Description',en:'Paste your revised template body into the Description field.'}},
...
MR       Closes #12                           <- lesson 15`,
          note:{th:'ประโยชน์จริงคือ หกเดือนต่อมามีคนถามว่า “ทำไมหน้านี้ต้องบังคับเลือกคลินิก” คุณเดินจากโค้ดย้อนกลับไปหา commit → MR → Issue → requirement ได้ใน 30 วินาที บทที่ 16 จะทำให้ขั้นตอนนี้เป็น skill ที่ Agent ทำซ้ำให้ได้',en:'The real payoff: six months later someone asks “why is clinic required here?” and you can walk from the code back to the commit, the MR, the issue, and the requirement in thirty seconds. Lesson 16 turns this into a skill the agent repeats for you.'}},
...
Answer as questions back to me. Do not answer them on my behalf.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: เขียน Issue ของ US-001 ด้วยตัวเอง',en:'Practice: write the US-001 issue yourself'},steps:{th:['เปิด `docs/requirements/US-001-opd-checkin.md` ใน starter repo แล้วอ่านให้จบก่อน','ร่าง Issue ตาม template ด้านบน โดยยังไม่เปิดกลับมาดู template ระหว่างเขียน','แยก AC ออกเป็นสองกอง คือสถานะ และพฤติกรรม แล้วนับว่าแต่ละกองมีกี่ข้อ','ส่ง Prompt ด้านบนให้ Agent แล้วจดรายการ “สิ่งที่มันจะเดา” ไว้ทุกข้อ','แก้ Issue จนรายการนั้นเหลือเฉพาะเรื่องที่คุณตั้งใจให้เป็นอิสระของ Agent เช่น ชื่อไฟล์หรือโครงสร้างโฟลเดอร์','เปิด Issue จริงตามบล็อกด้านบน แล้วจดเลข Issue ไว้','สร้าง `docs/templates/issue-template.md` จากแบบฟอร์มเปล่าด้านบน — ยังไม่ต้อง commit บทที่ 16 จะเก็บเข้า Git พร้อม skill ของทีม','ทดลองพิมพ์ gh issue view <เลข> หรือ glab issue view <เลข> ใน terminal จนอ่าน Issue นี้ออก เพราะบทที่ 05 จะให้ Agent รันคำสั่งนี้เอง','ยังไม่ต้องเปิด branch และยังไม่ต้องเรียก Agent ให้ลงมือ — บทที่ 03 และ 04 จะพาไปเปิดพื้นที่ทำงานเอง'],en:['Open `docs/requirements/US-001-opd-checkin.md` in the starter repo and read it through first','Draft the issue using the template above, without looking back at it while you write','Sort your ACs into two piles — states and behaviours — and count each pile','Send the prompt above and write down every item the agent says it would guess','Revise the issue until that list contains only what you deliberately leave to the agent, such as file names or folder structure','File the real issue using the block above and note its number','Create `docs/templates/issue-template.md` from the blank form above — no commit yet; lesson 16 puts it into Git alongside the team skills','Try gh issue view <number> or glab issue view <number> in a terminal until it prints the issue — lesson 05 has the agent run this exact command','Do not open a branch or call the agent yet — lessons 03 and 04 set up the workspace']},expected:{th:'ได้ Issue หนึ่งใบที่มี Context, In/Out of scope, AC แยกสถานะกับพฤติกรรม และ Definition of Done โดยไม่มีข้อใดที่ต้องตีความ และคุณอธิบายได้ว่าทำไมแต่ละข้อใน Out of scope ถึงถูกตัดออก รวมถึงมีเลข Issue จริงที่อ่านกลับได้ด้วย gh issue view หรือ glab issue view พร้อมไฟล์แบบฟอร์มเปล่าที่ `docs/templates/issue-template.md` ที่ยังไม่ถูก commit',en:'One issue with context, in/out of scope, ACs split into states and behaviours, and a definition of done — with nothing left to interpretation, and you can explain why each out-of-scope item was cut, plus a real issue number you can read back with gh issue view or glab issue view, plus the uncommitted blank form at `docs/templates/issue-template.md`.'}}
      ],
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'commands',title:{th:'5 คำสั่งที่ใช้ทุกวัน',en:'The five commands you use every day'},
          ...
          steps:[
            {title:{th:'ฉันอยู่ตรงไหน และมีอะไรค้างอยู่',en:'Where am I, and what is pending?'},what:{th:'`git status` คือคำสั่งที่ควรพิมพ์บ่อยที่สุด มันบอก 3 อย่าง คือ อยู่ branch ไหน, ไฟล์ไหนถูกแก้แต่ยังไม่ commit และมีไฟล์ใหม่ที่ Git ยังไม่รู้จักหรือไม่ ให้รันทั้งก่อนและหลังให้ Agent ทำงาน',en:'`git status` is the command to type most often. It tells you three things: which branch you are on, which files are modified but not committed, and whether there are new files Git does not track yet. Run it before and after the agent works.'},cmd:`git status`,expect:{th:'ถ้ายังไม่ได้แก้อะไร จะเห็นข้อความทำนอง `nothing to commit, working tree clean` — ยกเว้น `docs/templates/` จากบทที่ 02 ซึ่งจะขึ้นเป็น untracked file ไปก่อน จนบทที่ 16 จะเก็บเข้า Git',en:'If nothing was edited you see something like `nothing to commit, working tree clean` — except `docs/templates/` from lesson 02, which stays an untracked file until lesson 16 commits it.'}},
        ...
        {type:'commands',title:{th:'ฝึกวงจรเต็ม: แยก branch → แก้ → ดู diff → ย้อนกลับ',en:'Practise the full loop: branch → edit → diff → undo'},
          ...
          steps:[
            {title:{th:'เช็กจุดตั้งต้นว่าสะอาด',en:'Confirm a clean starting point'},what:{th:'ก่อนแยก branch ต้องแน่ใจว่าไม่มีงานค้างอยู่ ถ้ามีไฟล์ค้างแล้วแยก branch มันจะติดตามไปด้วย และทำให้สับสนว่าอะไรเป็นงานของ task ไหน',en:'Before branching, make sure nothing is pending. Uncommitted files follow you onto the new branch and blur which task they belong to.'},cmd:`git status`,expect:{th:'`working tree clean` — ยกเว้น untracked `docs/templates/` จากบทที่ 02 ที่ตั้งใจเก็บไว้ ให้ผ่านไปได้ ส่วนไฟล์อื่นที่ถูกแก้ค้างอยู่ต้องจัดการก่อน',en:'`working tree clean` — the intentionally kept untracked `docs/templates/` from lesson 02 may stay, but any other modified file must be dealt with first.'}},
...
git status`,expect:{th:'อยู่บน main และ `working tree clean` (ยกเว้น `docs/templates/` จากบทที่ 02 ที่ยังไม่ถูก track)',en:'You are on main and the tree is clean, apart from the still-untracked `docs/templates/` from lesson 02.'}},
...
Answer briefly as a numbered list with file paths.`},
    ...
    },
    {
      ...
      blocks:[
        {type:'prose',title:{th:'ทำไม AI ออกแบบอะไรก็ได้ให้ “สวย” แต่ไม่ “ใช่”',en:'Why an AI can make anything “nice” but not “yours”'},body:{th:[
          'ลองสั่ง Agent ว่า “ทำหน้า landing page ให้หน่อย” แล้วดูผลลัพธ์ คุณมักได้การ์ดมุมโค้ง ไล่สีม่วง-น้ำเงิน hero อยู่กลางจอ และปุ่ม “Get Started” — ใช้งานได้ แต่หน้าตาเหมือนของทุกที่ เพราะ “ดีไซน์ที่ดี” ในหัว Agent คือค่าเฉลี่ยของทุกเว็บที่มันเคยเห็น ไม่ใช่ภาษาการออกแบบของแบรนด์คุณ',
          'ทางแก้ที่คนมักใช้มีสองแบบ และทั้งคู่ไม่ scale แบบแรกคือพิมพ์ prompt ยาว ๆ อธิบายสี ระยะห่าง ขนาดตัวอักษรใหม่ทุกครั้งที่สั่งงาน ได้ผลไม่ครบครึ่งอยู่ดี แบบที่สองคือแคปหน้าจอแล้วบอกว่า “ทำให้เหมือนภาพนี้” ซึ่ง Agent จะเลียนแบบ pixel ได้ แต่จับ “ระบบ” เบื้องหลังไม่ได้ เช่น ทำไมปุ่มใช้ border ไม่ใช่ shadow',
          'บทนี้แนะนำไฟล์ที่แก้ปัญหานี้ตรง ๆ ชื่อ **DESIGN.md** เป็น pattern ที่มาจากแนวคิดของ Google Stitch และถูกรวบรวมเป็นมาตรฐานกลางที่ [getdesign.md](https://getdesign.md/what-is-design-md) หลักการคือเขียนระบบดีไซน์แบบ **semantic** ไม่ใช่แค่ list ของ token แต่ผูก **token + กติกา + เหตุผล** ไว้ในไฟล์เดียว เหมือนมี Designer อาวุโสมานั่งอธิบายภาษาแบรนด์ให้ Agent ฟังก่อนเริ่มงาน',
          'ก่อนหน้านี้เราข้ามจาก “UI Design” ไปที่ “Component” ตรง ๆ บทนี้เติมขั้นที่ขาดคือ **Design System** — ชั้นที่แปล design ให้เป็นกติกาที่ใช้ซ้ำได้ ก่อนที่บทถัดไปจะเริ่มแตกเป็น component'
        ],en:[
          'Ask an agent to “build me a landing page” and look at the result. You usually get rounded cards, a purple-to-blue gradient, a centered hero, and a “Get Started” button — functional, but it looks like everywhere else, because the agent’s idea of “good design” is an average of every site it has seen, not your brand’s design language.',
          'The two fixes people reach for do not scale. The first is typing forty lines of prompt describing colors, spacing, and type sizes every single time, and still getting half of it wrong. The second is pasting a screenshot and saying “make it look like this” — the agent copies pixels but misses the system behind them, such as why a button uses a border instead of a shadow.',
          'This lesson introduces the file that solves this directly: **DESIGN.md**. It is a pattern that started with Google Stitch and was collected into a shared standard at [getdesign.md](https://getdesign.md/what-is-design-md). The idea is a **semantic** design spec — not just a token list, but **token + rule + reason** kept in one file, as if a senior designer sat down and explained the brand’s language to the agent before it started.',
          'Up to now we jumped straight from “UI Design” to “Component”. This lesson fills the missing step, **Design System** — the layer that turns a design into reusable rules before the next lesson starts breaking it into components.'
        ]}},
        ...
        {type:'code',title:{th:'โครงของ DESIGN.md ที่เราจะใช้',en:'The DESIGN.md shape we will use'},
          lead:{th:'ครึ่งบนเป็น YAML token (สี, ตัวอักษร, ระยะห่าง, component) ครึ่งล่างเป็นร้อยแก้วที่อธิบายเหตุผล ทั้งสองฝั่งต้องตรงกัน 1:1 — token ทุกตัวต้องมีคำอธิบาย และคำอธิบายทุกข้อต้องอ้าง token ที่มีจริง',en:'The top half is YAML tokens (colors, type, spacing, components); the bottom half is prose carrying the reasoning. The two sides must match 1:1 — every token needs an explanation, and every explanation must cite a token that actually exists.'},
...
If you cannot justify a file, say so plainly.`},
          after:{th:['เทียบคำตอบข้อ 2 กับผลของ `git diff --stat` ว่าจำนวนไฟล์ตรงกันหรือไม่','ถ้ามันตอบข้อ 3 ว่า “ไม่มี” ทั้งที่คุณเห็นไฟล์แปลก ๆ ให้ถามเจาะจงไฟล์นั้นโดยตรง','เก็บคำตอบข้อ 1 ไว้ใช้เขียน Merge Request ในบทถัดไป','บทที่ 16 จะห่อ prompt นี้เป็น skill ชื่อ `review-diff` ให้เรียกใช้ซ้ำได้ทุก branch'],en:['Compare answer 2 against `git diff --stat` and check the file counts agree','If it answers “none” to point 3 while you can see an odd file, ask about that file by name','Keep answer 1 — you will reuse it when writing the Merge Request','Lesson 16 wraps this prompt into a reusable skill named `review-diff`']}},
...
Keep it short and never include anything that was not actually done.`},
        ...
        {type:'code',title:{th:'เก็บแบบฟอร์ม MR เปล่าไว้ในโปรเจกต์',en:'Keep the blank MR form in the project'},
          lead:{th:'เหมือนกับ issue template ในบทที่ 02 — เก็บแบบฟอร์ม MR เปล่าไว้ที่ `docs/templates/mr-template.md` สร้างในโฟลเดอร์ repo หลัก ไม่ใช่ใน worktree ของ US-001 เพื่อให้ branch ฟีเจอร์มีแต่งานของมันเท่านั้น ยังไม่ต้อง commit บทที่ 16 จะเก็บเข้า Git พร้อม issue template และ agent skill',en:'Like the issue template in lesson 02 — keep a blank MR form at `docs/templates/mr-template.md`. Create it in the main repo folder, not inside the US-001 worktree, so the feature branch carries only its own work. No commit yet; lesson 16 puts it into Git next to the issue template and the agent skills.'},
          label:'docs/templates/mr-template.md',code:{
th:`## What
...
Synthetic/mock data only. No production data.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: Draft MR',en:'Practice: draft an MR'},steps:{th:['ตรวจให้แน่ใจว่า quality gate ในบทที่ 14 ผ่านครบก่อน','เก็บงานค้างให้จบ แล้ว `git fetch` + `git merge origin/main`','push feature branch ด้วย `-u` เป็นครั้งแรก','ใช้ Prompt ร่าง MR แล้วแก้ให้ตรงกับสิ่งที่ทดสอบจริง','ใส่ `Closes #<เลข Issue ของคุณ>` ใน MR','สร้าง Draft MR แล้วให้เพื่อนอ่านและตอบว่าเข้าใจ scope หรือไม่','รับ comment อย่างน้อย 1 ข้อจากเพื่อน แล้วแก้ commit push ซ้ำให้ครบรอบ','เก็บแบบฟอร์ม MR เปล่าไว้ที่ `docs/templates/mr-template.md` ในโฟลเดอร์ repo หลัก (ไม่ใช่ใน worktree) — ยังไม่ commit บทที่ 16 จะเก็บเข้า Git พร้อม issue template จากบทที่ 02'],en:['Make sure the lesson 14 quality gates pass first','Finish any leftover work, then `git fetch` + `git merge origin/main`','Push the feature branch with `-u` for the first time','Use the MR draft prompt and edit it to match what you actually tested','Add `Closes #<your issue number>` to the MR','Open a Draft MR and ask a peer whether the scope is clear','Take at least one comment from that peer and complete a fix → commit → push round','Keep the blank MR form at `docs/templates/mr-template.md`, in the main repo folder rather than the worktree — not committed yet; lesson 16 puts it into Git next to the lesson 02 issue template']},expected:{th:'มี Draft MR ที่ Developer สามารถเริ่ม review ได้โดยไม่ต้องถามว่า “ทำอะไรมา?” พร้อมแบบฟอร์ม MR เปล่าที่ `docs/templates/mr-template.md` รออยู่ใน repo หลัก',en:'A Draft MR that a developer can start reviewing without first asking “What did you change?”, plus the blank MR form waiting at `docs/templates/mr-template.md` in the main repo.'}}
      ],
    ...
    },
    {
      ...
      intro:{th:'คุณเขียน Issue เองมาแล้วหนึ่งใบ และร่าง MR เองมาแล้วหนึ่งใบ พร้อมเก็บแบบฟอร์มเปล่าของทั้งคู่ไว้ที่ `docs/templates/` ตั้งแต่บทที่ 02 กับ 15 รอบนี้เราจะเอาสองไฟล์นั้นเข้า Git เป็น template ของทีม แล้วห่อเป็น skill ให้ Agent ทำซ้ำได้ในมาตรฐานเดียวกันทุกครั้ง — พร้อมกับ Diff Review Prompt จากบทที่ 14 ที่จะกลายเป็น skill ตัวที่สามไว้ตรวจทุก branch',en:'You have written one issue by hand and drafted one MR by hand, and kept blank forms of both under `docs/templates/` since lessons 02 and 15. This lesson commits them into Git as team templates and wraps them in skills, so the agent repeats them to the same standard every time — joined by the Diff Review Prompt from lesson 14, which becomes a third skill for reviewing any branch.'},
      outcomes:{th:['บอกได้ว่างานแบบไหนควรทำเป็น skill และแบบไหนไม่ควร','เอา template จากบทที่ 02 และ 15 เข้า Git พร้อม skill ใหม่ที่ `.agents/skills` ให้ทุกเครื่องมืออ่านได้','สร้าง skill สำหรับสร้าง Issue ร่าง MR และตรวจ diff และลิงก์เข้ากับเครื่องมือที่ใช้','ตั้งเส้นความปลอดภัย: Agent ร่างและเสนอคำสั่ง คนเป็นผู้อนุมัติ'],en:['Tell which work belongs in a skill and which does not','Commit the lesson 02 and 15 templates into Git alongside new skills under `.agents/skills`, readable by every tool','Build skills for filing issues, drafting MRs, and reviewing diffs, and link them into the tool you use','Set the safety line: the agent drafts and proposes, a human approves']},
      blocks:[
  ...
  templates/
    issue-template.md      <- สร้างไว้ตั้งแต่บทที่ 02 / created back in lesson 02
    mr-template.md         <- สร้างไว้ตั้งแต่บทที่ 15 / created back in lesson 15
.agents/
...
- Never commit, push, or open an MR out of this review.`,
        ...
        {type:'agent-setup',title:{th:'ติดตั้ง skill ในเครื่องมือที่คุณใช้',en:'Install the skills in the tool you use'},
          ...
          tools:[
            {id:'chatgpt',name:{th:'ChatGPT Desktop App',en:'ChatGPT Desktop App'},steps:[
              {title:{th:'ตรวจ template ที่เก็บไว้ใน docs/templates',en:'Check the templates kept in docs/templates'},what:{th:'ไฟล์ทั้งสองถูกสร้างไว้แล้วตั้งแต่บทที่ 02 และ 15 ขั้นนี้แค่ตรวจว่ามีจริงและยังเป็นแบบฟอร์มเปล่าที่มีแต่หัวข้อ ไม่มีเนื้อหาของ US-001 ติดอยู่ ถ้าหายไป (เช่น clone ใหม่หรือเปลี่ยนเครื่อง) ให้สร้างใหม่จากแบบฟอร์มเปล่าในบทที่ 02 และ 15',en:'Both files were created back in lessons 02 and 15 — this step only checks they are really there and still blank forms with headings only, with no US-001 content baked in. If they are gone (a fresh clone or a new machine), recreate them from the blank forms in lessons 02 and 15.'},expect:{th:'`docs/templates/issue-template.md` และ `docs/templates/mr-template.md` มีอยู่จริงในโปรเจกต์',en:'`docs/templates/issue-template.md` and `docs/templates/mr-template.md` exist in the project.'}},
...
Create only this one file, run no commands, and show me the content first.`},
    ...
    {
      id:'capstone', group:'capstone', no:'17', duration:'90 min', kind:'capstone',
      ...
      blocks:[
        {type:'prose',title:{th:'วิธีใช้บทนี้ให้ได้ผลจริง',en:'How to get real value from this lesson'},body:{th:[
          'บทนี้ไม่มีเนื้อหาใหม่ ทุกอย่างที่ต้องใช้ผ่านมาหมดแล้วในบทที่ 00–16 สิ่งที่เปลี่ยนไปคือ **ไม่มีใครบอกลำดับให้คุณอีกแล้ว** คุณต้องเป็นคนตัดสินใจเองว่าขั้นถัดไปคืออะไร',
          ...
          'There is no new material here. Everything you need appeared in lessons 00–16. What changes is that **nobody tells you the order any more** — you decide what comes next.',
        ...
        {type:'list',title:{th:'Definition of Done',en:'Definition of Done'},items:{th:['Issue ถูกเปิดจริง พร้อม AC แยกสถานะกับพฤติกรรม','DESIGN.md สรุปจากภาพ reference จริง และหน้า /design แสดง token ตรงกับไฟล์','Search ด้วย HN หรือชื่อ','Loading / Empty / Error / WithResults','เลือกผู้ป่วยและแสดงข้อมูล','Clinic required; Chief Complaint optional','Preview/Confirm ก่อน final action','Success พร้อม synthetic queue number','Meaningful Storybook stories','อย่างน้อย 1 interaction scenario','lint/test/build ผ่าน','commit เป็นระยะตาม checkpoint ไม่ใช่ก้อนเดียว และทุก commit อ้าง `Refs #<เลข Issue>`','git diff reviewed, MR ระบุ `Closes #<เลข Issue>` และถูก merge แล้ว','branch กับ worktree ถูกเก็บกวาดหลัง merge'],en:['The issue is actually filed, with ACs split into states and behaviours','DESIGN.md is summarized from a real reference image, and /design shows tokens matching the file','Search by HN or name','Loading / Empty / Error / WithResults','Select and display a patient','Clinic required; Chief Complaint optional','Preview/Confirm before final action','Success with a synthetic queue number','Meaningful Storybook stories','At least one interaction scenario','lint/test/build pass','Committed at checkpoints rather than in one lump, every commit carrying `Refs #<issue>`','git diff reviewed, the MR says `Closes #<issue>`, and it is merged','Branch and worktree cleaned up after the merge']}},
        {type:'capstone',steps:[
          {title:{th:'1 · เขียน Issue และ Acceptance Criteria',en:'1 · Write the issue and acceptance criteria'},hint:{th:'อย่าเพิ่งเปิด branch จนกว่า AC ทุกข้อจะตอบได้ว่า ผ่าน หรือ ไม่ผ่าน',en:'Do not open a branch until every AC answers pass or fail'},guide:{th:['อ่าน `docs/requirements/US-001-opd-checkin.md` ให้จบก่อน','ร่าง Issue ตาม `docs/templates/issue-template.md` จากบทที่ 02 ให้ครบทั้ง Context, In/Out of scope, AC และ Definition of Done','แยก AC เป็นสองกอง คือสถานะ (Loading/Empty/Error/Validation/Success) และพฤติกรรม (Given/When/Then)','ใช้ค่าตายตัวจาก mock data ใน AC ที่เป็นพฤติกรรม เช่น HN 65000123','ส่ง Prompt ตรวจช่องว่างจากบทที่ 02 แล้วแก้ Issue ตามรายการ “สิ่งที่ Agent จะเดา”','เปิด Issue จริงใน repository ของคุณเอง (จากบทที่ 00) แล้วจดเลข Issue ไว้ใช้ทั้งในชื่อ branch, ใน commit และใน MR'],en:['Read `docs/requirements/US-001-opd-checkin.md` all the way through','Draft the issue with `docs/templates/issue-template.md` from lesson 02: context, in/out of scope, ACs, definition of done','Split the ACs into states (Loading/Empty/Error/Validation/Success) and behaviours (Given/When/Then)','Use fixed mock-data values in behaviour ACs, e.g. HN 65000123','Send the gap-check prompt from lesson 02 and fix everything the agent said it would guess','File the real issue in your own repository (from lesson 00) and note its number — you will use it in the branch name, the commits, and the MR']}},
          ...
          {title:{th:'3 · Explore & Plan',en:'3 · Explore & Plan'},hint:{th:'ยังไม่ให้ Agent แก้ไฟล์ ให้มันอ่าน AGENTS.md + Issue จริงของคุณผ่าน gh หรือ glab แล้วรายงาน component/state/file scope',en:'Do not allow edits yet. Ask it to read AGENTS.md + your real issue via gh or glab, and report component/state/file scope'},guide:{th:['ใช้ Explore Prompt จากบทที่ 05','รัน `git status` ยืนยันว่ายังไม่มีไฟล์ถูกแก้','review ว่าครบ Loading/Empty/Error/Validation/Success และ file scope แคบพอ','ถ้าไม่ครบ ใช้ Refine Prompt วนอีกรอบ'],en:['Use the Explore Prompt from lesson 05','Run `git status` to confirm nothing changed','Review Loading/Empty/Error/Validation/Success coverage and that file scope is narrow','If gaps remain, loop with the Refine Prompt']}},
          {title:{th:'4 · Design System',en:'4 · Design System'},hint:{th:'แนบภาพ reference ก่อนสั่ง Agent สรุป DESIGN.md อย่าปล่อยให้มันเดาแบรนด์เอง',en:'Attach a reference image before asking the agent to summarize DESIGN.md — do not let it guess the brand'},guide:{th:['เปิดภาพ reference ที่ `docs/design/opd-check-in-reference.png` ในโปรเจกต์ (หรือ mockup อื่นที่ได้รับมา)','ส่ง Prompt สรุป DESIGN.md จากบทที่ 07 พร้อมภาพแนบ','ตรวจ DESIGN.md ด้วย checklist ในบทที่ 07 ก่อนไปต่อ','ส่ง Prompt สร้างหน้า /design แล้วเทียบกับ DESIGN.md','ยังไม่ commit — จะรวมกับ Commit checkpoint 1 ในขั้นถัดไป'],en:['Open the reference image at `docs/design/opd-check-in-reference.png` in the project (or another mockup you received)','Send the DESIGN.md summarization prompt from lesson 07 with the image attached','Check DESIGN.md against the lesson 07 checklist before moving on','Send the /design page prompt and compare it against DESIGN.md','Do not commit yet — it bundles into Commit checkpoint 1 in the next step']}},
          {title:{th:'5 · Component + State Map',en:'5 · Component + State Map'},hint:{th:'อย่าเริ่มจากหน้าใหญ่ก้อนเดียว และเขียนรายการของตัวเองก่อนถาม Agent',en:'Do not start with one giant page component, and write your own list before asking the agent'},guide:{th:['เขียน state map ตามลำดับ PatientSearch → PatientResults → SelectedPatient → CheckInForm → Confirmation → Success','กำหนด meaningful states เฉพาะที่แต่ละ component ต้องรับผิดชอบ','ใช้ Prompt วิจารณ์จากบทที่ 08'],en:['Draft the state map in order: PatientSearch → PatientResults → SelectedPatient → CheckInForm → Confirmation → Success','Define only the meaningful states each component owns','Use the critique prompt from lesson 08']}},
          {title:{th:'6 · Storybook First',en:'6 · Storybook First'},hint:{th:'สร้าง component + Storybook story คู่กัน และใช้ mock deterministic',en:'Create component + Storybook story pairs with deterministic mocks'},guide:{th:['ให้ Agent ทำ PatientSearch ก่อน พร้อม Default/Loading/Empty/WithResults/Error (ใช้ Prompt จากบทที่ 09)','เปิด Storybook แล้ว review ทีละ Storybook story','ส่ง feedback เป็นภาษา product','ผ่านแล้วค่อยทำ component ถัดไป','**Commit checkpoint 1** — เมื่อ component แรกผ่าน review: `git add <path ของ component> DESIGN.md src/app/design` (รวม DESIGN.md กับหน้า /design จากขั้นก่อนหน้าถ้ายังไม่เคย commit) แล้ว `git commit -m "feat(us001): ..." -m "Refs #<เลข Issue>"`'],en:['Have the agent build PatientSearch first with Default/Loading/Empty/WithResults/Error (prompt in lesson 09)','Open Storybook and review one Storybook story at a time','Send product-language feedback','Only then move to the next component','**Commit checkpoint 1** — once the first component passes review: `git add <component path> DESIGN.md src/app/design` (bundle in DESIGN.md and the /design page from the earlier step if not committed yet) then `git commit -m "feat(us001): ..." -m "Refs #<issue>"`']}},
          {title:{th:'7 · Interaction',en:'7 · Interaction'},hint:{th:'เลือก behavior สำคัญ 1 flow เขียน Given/When/Then',en:'Choose one important behavior and write Given/When/Then'},guide:{th:['เขียน Given/When/Then เช่น Given หน้าค้นหาแสดงอยู่ / When กรอก HN 65000123 แล้วกดค้นหา / Then แสดง Somchai Jaidee','ส่ง Prompt จากบทที่ 10','เปิดดู interaction ทีละขั้นใน Storybook','**Commit checkpoint 2** — เมื่อ test ผ่าน: `git commit -m "test(us001): ..." -m "Refs #<เลข Issue>"`'],en:['Write Given/When/Then, e.g. Given the search screen is displayed / When HN 65000123 is entered and Search clicked / Then Somchai Jaidee appears','Send the prompt from lesson 10','Watch the interaction step by step in Storybook','**Commit checkpoint 2** — once the test passes: `git commit -m "test(us001): ..." -m "Refs #<issue>"`']}},
          {title:{th:'8 · Integrate Page',en:'8 · Integrate the page'},hint:{th:'ประกอบเฉพาะ component ที่ review แล้ว และ reuse mock service',en:'Assemble only reviewed components and reuse the mock service'},guide:{th:['ใช้ Integration Prompt จากบทที่ 12','เปิด `/opd/check-in`','เดินครบทั้ง 5 รอบ (happy path, ไม่เลือกคลินิก, empty, error, ย้อนกลับ/เริ่มใหม่)','จดปัญหาเป็นพฤติกรรม แล้วส่งแก้ทีละข้อ','**Commit checkpoint 3** — เมื่อเดินครบ 5 รอบแล้วผ่าน: commit พร้อมเขียนใน body ว่าเดินอะไรมาบ้าง'],en:['Use the integration prompt from lesson 12','Open `/opd/check-in`','Walk all five rounds (happy path, no clinic, empty, error, back/restart)','Note issues as behaviour and send them one at a time','**Commit checkpoint 3** — once all five rounds pass: commit, and write in the body which rounds you walked']}},
          {title:{th:'9 · Diagnose Intentional Error',en:'9 · Diagnose an intentional error'},hint:{th:'อ่าน error ก่อน fix และขอ root cause',en:'Read the error before fixing and ask for root cause'},guide:{th:['เลือก error จาก `docs/workshop-errors.md`','เก็บหลักฐานครบ 4 แหล่ง (Terminal, UI, Console, Network)','ใช้ Root-cause Prompt จากบทที่ 13','อธิบายด้วยคำตัวเองก่อนอนุญาตให้แก้','ใช้ Verify Prompt แล้ว reproduce เคสเดิม','**Commit checkpoint 4** — เมื่อยืนยันว่าหายจริง: `git commit -m "fix(us001): ..." -m "อาการที่หาย และยืนยันอย่างไร. Refs #<เลข Issue>"`'],en:['Pick an error from `docs/workshop-errors.md`','Collect evidence from all four sources (terminal, UI, console, network)','Use the Root-cause Prompt from lesson 13','Explain it in your own words before approving a fix','Use the Verify Prompt and reproduce the original case','**Commit checkpoint 4** — once you confirmed it is gone: `git commit -m "fix(us001): ..." -m "Symptom gone, and how it was verified. Refs #<issue>"`']}},
          {title:{th:'10 · Diff & Quality Gates',en:'10 · Diff & Quality Gates'},hint:{th:'คาดการณ์ changed files ก่อนดูจริง',en:'Predict the changed files before reviewing them'},guide:{th:['เขียนคาดการณ์ changed files ก่อน','รัน `git status`','รัน `git log --oneline main..HEAD` อ่านว่า branch นี้เล่าเรื่องอะไร','รัน `git diff main...HEAD --stat`','เปิด diff ของไฟล์ที่ไม่คาดคิด','กวาดหาไฟล์ที่ห้าม commit เช่น `.env` หรือไฟล์ทดลองของ Agent','ใช้ Diff Review Prompt จากบทที่ 14 หรือ skill `review-diff` จากบทที่ 16','รัน `npm run lint`','รัน `npm run test`','รัน `npm run build`'],en:['Write your prediction first','Run `git status`','Run `git log --oneline main..HEAD` and read what this branch says it did','Run `git diff main...HEAD --stat`','Open diffs of unexpected files','Scan for files that must never be committed, such as `.env` or agent scratch files','Use the Diff Review Prompt from lesson 14 or the `review-diff` skill from lesson 16','Run `npm run lint`','Run `npm run test`','Run `npm run build`']}},
          {title:{th:'11 · Draft MR',en:'11 · Draft MR'},hint:{th:'MR ต้องเล่า intent, scope, states และ evidence',en:'The MR should tell intent, scope, states, and evidence'},guide:{th:['รัน `git status` ยืนยัน branch และดูว่าเหลืออะไรค้าง','ถ้าเหลือ ให้ `git add <path>` เฉพาะที่เกี่ยว แล้ว commit ปิดท้าย (ไม่ใช้ `git add .`)','รัน `git fetch origin` แล้ว `git merge origin/main`','รัน `git push -u origin feature/us001-capstone`','ใช้ Prompt ร่าง MR จากบทที่ 15 หรือ skill `create-mr` จากบทที่ 16','ตัดสิ่งที่ยังไม่ได้ทดสอบจริงออก','ใส่ `Closes #<เลข Issue>` ลงใน MR','เปิด Draft MR แล้วให้เพื่อน review ความชัดเจน'],en:['Run `git status` to confirm the branch and see what is left','If anything is left, `git add <path>` only what belongs and commit it (never `git add .`)','Run `git fetch origin`, then `git merge origin/main`','Run `git push -u origin feature/us001-capstone`','Use the MR draft prompt from lesson 15 or the `create-mr` skill from lesson 16','Remove anything you did not actually test','Add `Closes #<issue number>` to the MR','Open a Draft MR and have a peer review its clarity']}},
          {title:{th:'12 · ปิดรอบให้จบ',en:'12 · Close the loop'},hint:{th:'งานยังไม่จบที่ Draft MR — จบเมื่อถูก merge และเก็บกวาดเรียบร้อย',en:'A draft MR is not the end — the end is merged and cleaned up'},guide:{th:['ขอให้เพื่อนหรือผู้สอน review แล้วให้ comment อย่างน้อย 1 ข้อ','แก้บน branch เดิม แล้ว commit ด้วยข้อความที่บอกว่าแก้ตาม review ข้อไหน','รัน `git push` เปล่า ๆ (ไม่ต้องใส่ `-u` อีก) แล้วดูว่า MR อัปเดตเอง','เปลี่ยน Draft เป็นพร้อม review แล้วให้ผู้ review เป็นคนกด merge','หลัง merge: `git switch main`, `git pull`, `git worktree remove ../wt-us001-capstone`, `git branch -d feature/us001-capstone`','ยืนยันว่า Issue ถูกปิดอัตโนมัติจาก `Closes #<เลข Issue>`'],en:['Ask a peer or the instructor to review and leave at least one comment','Fix on the same branch and commit with a message naming the review point','Run plain `git push` (no `-u` this time) and watch the MR update itself','Switch it from draft to ready and let the reviewer press merge','After the merge: `git switch main`, `git pull`, `git worktree remove ../wt-us001-capstone`, `git branch -d feature/us001-capstone`','Confirm the issue closed itself thanks to `Closes #<issue number>`']}}
        ]},
    ...
    categories: [
      ...
      },
      {
        id:'agent', name:{th:'AI Agent & Prompt',en:'AI Agent & Prompting'}, terms:[
          ...
          {term:'Agent Skill',th:'ไฟล์คำสั่งที่เก็บไว้ในโปรเจกต์เพื่อให้ Agent ทำงานที่ทำซ้ำบ่อยตามขั้นตอนเดียวกันทุกครั้ง เช่น การร่าง Issue หรือ MR ตาม template ของทีม หรือการรายงานผลตรวจ diff ตามหัวข้อเดิมทุกครั้ง เหมาะกับงานที่มีรูปแบบตายตัว ไม่เหมาะกับงานที่ต้องตัดสินใจ',en:'An instruction file kept in the project so the agent performs a repeated task the same way every time — drafting an issue or an MR from the team template, or reporting a diff review under the same headings every time, for instance. Good for fixed shapes, wrong for decisions.'},
      ...
      },
      {
        id:'ui', name:{th:'Next.js, Design System & Storybook',en:'Next.js, Design System & Storybook'}, terms:[
          ...
          {term:'Style Guide',alias:'/design',th:'หน้าเว็บที่แสดง design token ทั้งหมด (สี, ตัวอักษร, spacing, ตัวอย่าง component) ให้ human ตรวจได้ด้วยตา สร้างจาก DESIGN.md โดยตรง ไม่ใช่ของที่แยกดูแลเอง',en:'A page that displays every design token — colors, type, spacing, sample components — for a human to review by eye. It is generated directly from DESIGN.md, not maintained as a separate thing.'},
