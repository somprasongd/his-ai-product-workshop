window.COURSE = {
  meta: {
    ...
    subtitle: {
      th: 'PM, BA และ Product Design ใช้ AI สร้าง prototype ที่กดได้ ตรวจได้ และส่งต่อให้ Developer ทำต่อ โดยไม่ต้องเขียนโค้ดเอง',
      en: 'PM, BA, and Product Design use AI to build a working, verifiable prototype that developers can continue — without writing code themselves.'
    },
  ...
  groups: [
    ...
    { id:'extra', th:'บทเสริม', en:'Bonus' }
  ],
  // One visible output per lesson keeps the same US-001 work traceable across the course.
  journey: {
    prerequisites: { th:'Repo ของตนเอง + starter เปิดได้', en:'Own repository + running starter' },
    'mental-model': { th:'แผนที่ Requirement → งานที่ตรวจได้', en:'Requirement → reviewable work map' },
    'requirement-issue': { th:'Issue จริงพร้อม AC และเลข Issue', en:'Real issue with ACs and its number' },
    'git-basics': { th:'อ่าน status, diff และ commit ได้', en:'Read status, diff, and commits' },
    worktree: { th:'branch/worktree ของ Issue เดียว', en:'One issue branch and worktree' },
    'agent-plan': { th:'แผนที่คนตรวจและอนุมัติแล้ว', en:'Human-reviewed implementation plan' },
    ...
    'agent-skills': { th:'template ของทีม + skill สามตัวใน Git', en:'Team templates + three skills in Git' }
  },
  lessons: [
    {
      ...
      blocks:[
        ...
        {type:'diagram',share:false,title:{th:'เครื่องมือ 4 กลุ่มทำงานร่วมกันอย่างไร',en:'How the four tool groups work together'},
        ...
        {type:'list',title:{th:'ติดตั้งก่อนเรียน',en:'Install before class'},items:{th:['[Git](https://git-scm.com/downloads) เวอร์ชันปัจจุบัน','[Node.js](https://nodejs.org/) 20.9+ (ข้อกำหนดขั้นต่ำของ Next.js 16)','[VS Code](https://code.visualstudio.com/) หรือ editor ที่ทีมใช้','[Chrome](https://www.google.com/chrome/) หรือ [Edge](https://www.microsoft.com/edge) สำหรับ DevTools','เลือก AI Agent แบบแอป desktop อย่างน้อย 1 ตัว: [ChatGPT Desktop App](https://openai.com/chatgpt/download/) หรือ [Claude Desktop App](https://claude.com/download) — ไม่ต้องใช้ CLI agent','บัญชีที่เก็บโค้ดของคุณเอง: [GitHub](https://github.com/signup) หรือ GitLab (gitlab.com หรือ GitLab ของบริษัท) — ต้องมี เพราะคุณจะต้องเปิด Issue และ push branch ด้วยบัญชีตัวเอง','[GitHub CLI](https://cli.github.com/) หรือ [GitLab CLI](https://gitlab.com/gitlab-org/cli) ต้องติดตั้งและ login หนึ่งตัว — เป็นเครื่องมือแยกจากตัว Agent ใช้ตั้งแต่บทที่ 05 เพื่อให้ Agent อ่าน Issue จริงของคุณได้ และใช้ในบทที่ 15 เพื่อเปิด MR ของ Issue เดิม'],en:['[Git](https://git-scm.com/downloads), current version','[Node.js](https://nodejs.org/) 20.9+ (minimum for Next.js 16)','[VS Code](https://code.visualstudio.com/) or your team editor','[Chrome](https://www.google.com/chrome/) or [Edge](https://www.microsoft.com/edge) for DevTools','At least one AI agent desktop app: [ChatGPT Desktop App](https://openai.com/chatgpt/download/) or [Claude Desktop App](https://claude.com/download) — no agent CLI needed','An account that will host your own copy of the code: [GitHub](https://github.com/signup) or GitLab (gitlab.com or your company server) — required, because you will file issues and push branches under your own account','[GitHub CLI](https://cli.github.com/) or [GitLab CLI](https://gitlab.com/gitlab-org/cli) — install and sign in to one. A tool separate from the agent itself: used from lesson 05 so the agent can read your real issue, and in lesson 15 to open the MR for the same issue']}},
...
git remote -v`,expect:{th:'สองบรรทัดที่มีชื่อบัญชีของคุณ และลงท้ายด้วย `(fetch)` กับ `(push)`',en:'Two lines containing your username, ending in `(fetch)` and `(push)`.'}},
              {title:{th:'ติดตั้งและล็อกอิน GitHub CLI',en:'Install and sign in to the GitHub CLI'},what:{th:'`gh` คือเครื่องมือแยกจากตัว Agent ที่ให้คุณและ Agent ทำงานกับ Issue และ Pull Request จาก Terminal ได้ ตั้งแต่บทที่ 05 Explore Prompt จะให้ Agent รัน `gh issue view` เพื่ออ่าน Issue จริงของคุณ และบทที่ 15 ใช้ต่อกับ MR ติดตั้งด้วย `brew install gh` (macOS) หรือ `winget install GitHub.cli` (Windows) แล้วล็อกอิน',en:'`gh` is a tool separate from the agent that lets you — and the agent — work with issues and pull requests from the terminal. From lesson 05 the Explore Prompt has the agent run `gh issue view` to read your real issue, and lesson 15 uses it for the MR. Install with `brew install gh` (macOS) or `winget install GitHub.cli` (Windows), then sign in.'},cmd:`gh auth login
...
git push -u origin main`,expect:{th:'รีเฟรชหน้า GitLab แล้วเห็นไฟล์ของโปรเจกต์และ branch `main`',en:'Refresh the GitLab page and you see the project files and the `main` branch.'}},
              {title:{th:'ติดตั้งและล็อกอิน GitLab CLI',en:'Install and sign in to the GitLab CLI'},what:{th:'`glab` คือเครื่องมือแยกจากตัว Agent ที่ให้คุณและ Agent ทำงานกับ Issue และ Merge Request จาก Terminal ได้ ตั้งแต่บทที่ 05 Explore Prompt จะให้ Agent รัน `glab issue view` เพื่ออ่าน Issue จริงของคุณ และบทที่ 15 ใช้ต่อกับ MR ติดตั้งด้วย `brew install glab` (macOS) หรือ `winget install glab` (Windows) ถ้าใช้ GitLab ของบริษัท ต้องระบุ `--hostname` ตอนล็อกอิน',en:'`glab` is a tool separate from the agent that lets you — and the agent — work with issues and merge requests from the terminal. From lesson 05 the Explore Prompt has the agent run `glab issue view` to read your real issue, and lesson 15 uses it for the MR. Install with `brew install glab` (macOS) or `winget install glab` (Windows). On a company GitLab you must pass `--hostname` when signing in.'},cmd:`glab auth login --hostname gitlab.com
glab auth status`,expect:{th:'`glab auth status` บอกว่าล็อกอินกับ host ที่ระบุสำเร็จ',en:'`glab auth status` reports a successful login to that host.'}}
        ...
        {type:'commands',title:{th:'รันโปรเจกต์ครั้งแรก',en:'Run the project for the first time'},
          ...
          steps:[
            {title:{th:'ติดตั้ง dependency ของโปรเจกต์',en:'Install the project dependencies'},what:{th:'โปรเจกต์ Next.js ใช้ library ของคนอื่นจำนวนมาก `npm ci` อ่านรายการจาก `package.json` แล้วดาวน์โหลดลงโฟลเดอร์ `node_modules` ครั้งแรกจะช้าหน่อย ถือเป็นเรื่องปกติ',en:'A Next.js project depends on many third-party libraries. `npm ci` uses the pinned dependency versions in `package-lock.json` and downloads them into `node_modules`. The first run is slow — that is normal.'},cmd:`npm ci`,expect:{th:'จบด้วยสรุปจำนวน package ที่ติดตั้ง คำเตือน (warning) ไม่ใช่ปัญหา แต่คำว่า error คือปัญหา',en:'It ends with a summary of installed packages. Warnings are fine; the word “error” is not.'}},
...
4. Where are the main folders for pages, components, and mock data?`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: ยืนยันว่าเครื่องพร้อมจริง',en:'Practice: confirm your machine is genuinely ready'},steps:{th:['รันคำสั่งตรวจเวอร์ชันทั้งสามให้ผ่าน','ตั้ง `git config` ชื่อและอีเมล แล้วอ่านค่ากลับมาดู','ทำตามแท็บ GitHub หรือ GitLab จนได้ repository ของตัวเอง','รัน `git remote -v` แล้วยืนยันว่า origin เป็นบัญชีของคุณ ไม่ใช่ของผู้สอน','เปิดหน้าเว็บ repo ของคุณ แล้วยืนยันว่ากดสร้าง Issue ใหม่ได้','ติดตั้งและ login `gh` หรือ `glab` หนึ่งตัวให้สำเร็จ — บทที่ 05 จะให้ Agent ใช้คำสั่งนี้อ่าน Issue จริงของคุณ','รัน `npm ci`, `npm run dev` และ `npm run storybook` ให้ขึ้นครบ','ส่ง Prompt แรกให้ Agent แล้วตรวจว่ามันอ้างถึงไฟล์จริงในโปรเจกต์'],en:['Pass all three version checks','Set `git config` name and email, then read the values back','Follow either the GitHub or the GitLab tab until you own a repository','Run `git remote -v` and confirm origin is your account, not the instructor’s','Open your repo in the browser and confirm you can start a new issue','Install and sign in to `gh` or `glab` — lesson 05 has the agent use it to read your real issue','Run `npm ci`, `npm run dev`, and `npm run storybook` successfully','Send the first prompt and check that the agent cites real project files']},expected:{th:'คุณมี repository ของตัวเองที่เปิด Issue ได้ Git รู้จักชื่อคุณ หน้า Training Starter และ Storybook เปิดได้ และ Agent อธิบายโปรเจกต์ได้จากไฟล์จริง โดยมี gh หรือ glab login ไว้พร้อมใช้',en:'You own a repository where you can file issues, Git knows your name, the Training Starter page and Storybook both open, the agent describes the project from its real files, and gh or glab is signed in and ready.'}}
      ],
...
V --> M[MR + handoff]`,
        ...
        {type:'callout',title:{th:'เอกสารต้นทางที่ต้องเปิด',en:'Open the source document'},
          text:{th:'อ่าน `docs/requirements/US-001-opd-checkin-th.md` ใน starter ให้จบก่อนเขียน Issue อย่าตัด AC ข้อใดทิ้ง: จัดกลุ่มเป็น state/behavior และใช้ค่าจาก mock เพื่อทำให้ตรวจซ้ำได้',en:'Read `docs/requirements/US-001-opd-checkin.md` in the starter before filing the issue. Preserve all ACs, group them as states or behaviors, and use mock values for repeatable checks.'}},
...
Requirement: docs/requirements/US-001-opd-checkin-th.md

## In scope
- ค้นหาด้วย HN/ชื่อ เลือกผู้ป่วย เลือก clinic และ Preview ก่อนยืนยัน
- แสดง Loading, Empty, Error, Validation และ Success

## Out of scope
- HIS/backend, authentication, คิวจริง และ production deployment

## Acceptance Criteria - states
- AC2 ระหว่างค้นหาแสดง Loading
- AC3 ไม่พบข้อมูลแสดง Empty
- AC4 ค้นหาล้มเหลวแสดง Error พร้อม retry
- AC7 เมื่อเลือกผู้ป่วย แสดง HN ชื่อ วันเกิด และเพศจาก mock
...
## Definition of Done
- Storybook แสดง state, interaction test ครอบอย่างน้อยหนึ่ง behavior
- เดิน flow จริงและตรวจ keyboard/mobile ด้วยคน
- lint / test / build ผ่าน, diff อยู่ใน scope, เปิด Draft MR
...
## In scope
- Search by HN/name, select a patient and clinic, preview before confirming
- Show Loading, Empty, Error, Validation, and Success

## Out of scope
- Real HIS/backend, authentication, real queue, production deployment

## Acceptance Criteria - states
- AC2 Show Loading during search
- AC3 Show Empty for no matches
- AC4 Show Error with retry when search fails
- AC7 Show selected HN, name, date of birth, gender from mocks
...
## Definition of Done
- Storybook shows states; an interaction test covers one behavior or more
- A human walks the flow and checks keyboard/mobile
- lint / test / build pass, diff is scoped, Draft MR is open

## Data
Synthetic/mock data only. No real patient records.
The mock must reproduce special states on demand: normal / slow (~2s delay to make Loading visible) / empty / error — demonstrated through Storybook only, with no scenario switcher on the user-facing page.`},
          note:{th:'Issue อ้างไฟล์ requirement ต้นทาง แต่ไม่สั่งชื่อ component หรือไฟล์ implementation — Issue บอกว่า “อะไร” ส่วน “อย่างไร” คือแผนที่ Agent จะเสนอมาให้ review ในบทที่ 05',en:'The issue links its source requirement but does not prescribe component names or implementation files. It states what; the agent proposes how for review in lesson 05.'}},
        {type:'code',title:{th:'แบบฟอร์ม Issue สำหรับงานถัดไป',en:'Issue form for later work'},
          lead:{th:'ใช้แบบฟอร์มนี้เป็นตัวอย่างตอนเขียน Issue จริง หากทีมต้องการใช้ซ้ำ จะได้เก็บเป็นไฟล์ template ของทีมพร้อม Agent Skill ในบทเสริมท้ายคอร์ส',en:'Use this form as an example for the real issue. If your team wants to reuse it, the bonus lesson at the end of the course saves it as a team template with an agent skill.'},
          label:'Issue template · example',code:{
th:`Title: <รหัสงาน> <ผลลัพธ์ต่อผู้ใช้ในหนึ่งบรรทัด>
...
## Out of scope
- <สิ่งที่ตัดออก อย่างน้อย 2 ข้อ>

## Acceptance Criteria - states
- <แต่ละ state ที่หน้าจอต้องแสดง>
...
## Data
ใช้ synthetic/mock data เท่านั้น ห้ามใช้ข้อมูลผู้ป่วยจริง`,
en:`Title: <work-id> <user outcome in one line>

## Context
<The current problem in 2-3 lines, and what this issue covers>
Requirement: docs/requirements/<filename>

## In scope
- <What gets built this round>

## Out of scope
- <At least two deliberately cut items>

## Acceptance Criteria - states
- <Each state the screen must show>
        ...
        {type:'agent-setup',store:'repoHost',title:{th:'เปิด Issue จริงบน GitHub หรือ GitLab',en:'File the real issue on GitHub or GitLab'},
          ...
          tools:[
            {id:'github',name:{th:'GitHub',en:'GitHub'},steps:[
              ...
              {title:{th:'Submit แล้วจดเลข Issue',en:'Submit and note the issue number'},what:{th:'กด **Submit new issue** แล้วจดเลข Issue ที่ขึ้นมา เช่น `#12`',en:'Click **Submit new issue**, then note the issue number, e.g. `#12`.'},expect:{th:'ลิงก์แบบ `github.com/<ชื่อคุณ>/<repo>/issues/12` และเลข `12` ถูกจดไว้ — บทที่ 05 จะให้ Agent อ่าน Issue นี้ด้วย `gh issue view 12`',en:'A link like `github.com/<you>/<repo>/issues/12` with the number `12` written down — lesson 05 has the agent read this issue with `gh issue view 12`.'}}
            ]},
            {id:'gitlab',name:{th:'GitLab',en:'GitLab'},steps:[
              ...
              {title:{th:'Create แล้วจดเลข Issue',en:'Create and note the issue number'},what:{th:'กด **Create issue** แล้วจดเลข Issue ที่ขึ้นมา เช่น `#12`',en:'Click **Create issue**, then note the issue number, e.g. `#12`.'},expect:{th:'ลิงก์แบบ `gitlab.com/<ชื่อคุณ>/<repo>/-/issues/12` และเลข `12` ถูกจดไว้ — บทที่ 05 จะให้ Agent อ่าน Issue นี้ด้วย `glab issue view 12`',en:'A link like `gitlab.com/<you>/<repo>/-/issues/12` with the number `12` written down — lesson 05 has the agent read this issue with `glab issue view 12`.'}}
            ]}
          ]},
...
Branch   feature/12-us001-patient-checkin        <- lesson 04
Commit   feat(us001): add PatientSearch ...
         Refs #12                             <- lessons 09-13
MR       Closes #12                           <- lesson 15`,
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'diagram',title:{th:'Git ในภาพเดียว',en:'Git in one picture'},
          lead:{th:'วงจรของงานหนึ่งชิ้น: เริ่มจาก main ในเครื่อง แยกไปทำบน feature branch บันทึกงานเป็น commit สะสมบน branch นั้น ส่งขึ้น remote ด้วย `git push` ขอรวมเข้า main ผ่าน Merge Request แล้วดึงกลับลงเครื่องด้วย `git pull` เพื่อเริ่มรอบใหม่จาก main ล่าสุดเสมอ',en:'The loop of one piece of work: start from local main, split onto a feature branch, collect commits there, push the branch up with `git push`, merge it into main through a Merge Request, then pull it back down with `git pull` — so every new round starts from the latest main.'},
          diagram:`flowchart LR\nM["main\\nในเครื่อง (local)"] -->|"1. git switch -c"| B["feature branch"]\nB -->|"2. git add + commit"| B\nB -->|"3. git push"| R["Remote\\nGitHub / GitLab"]\nR -->|"4. Merge Request / PR"| RM["main\\nบน remote"]\nRM -->|"5. git pull"| M`,
          notes:{th:['`git switch -c` = แยก branch ใหม่ออกจาก main ในเครื่อง แล้วย้ายไปทำงานบนนั้นทันที จากจุดนี้งานทั้งหมดเกิดบน feature branch ไม่ใช่บน main','`git add` + `git commit` = บันทึกเวอร์ชันบน feature branch ลูกศรวนกลับเข้าตัวเองคือคุณ commit ซ้ำได้หลายรอบตามจุดตรวจ และงานทั้งหมดกองอยู่บน branch นี้','`git push` = ส่ง feature branch ขึ้นไปไว้บน GitHub/GitLab เป็นครั้งแรกที่งานออกจากเครื่อง ก่อนหน้านี้ยังไม่มีใครเห็นงานของคุณ','Merge Request / PR = ขอรวม feature branch เข้า main ฝั่ง remote โดยมี reviewer ตรวจก่อนกด merge นี่คือประตูเดียวที่งานเข้า main ได้','`git pull` = หลัง merge main บน remote จะล้ำหน้าสิ่งที่มีในเครื่อง ต้องสลับกลับไป main แล้ว pull ลงมาเครื่องจึงจะได้ของล่าสุด วงจรจึงปิดและพร้อมเริ่มรอบใหม่ นี่คือที่มาของกฎ “pull ก่อนแยก branch ใหม่เสมอ”'],en:['`git switch -c` = split a new branch off local main and move onto it at once. From this point all work happens on the feature branch, never on main','`git add` + `git commit` = save versions onto the feature branch. The arrow looping back to itself means you commit again at every checkpoint, and the work piles up on this branch','`git push` = send the feature branch up to GitHub/GitLab — the first moment your work leaves your machine. Before this, nobody has seen it','Merge Request / PR = ask to fold the feature branch into main on the remote, with a reviewer approving before merge. This is the only door into main','`git pull` = after the merge, main on the remote is ahead of your machine. Switch back to main and pull it down to get the latest — the loop closes and the next round starts fresh. That is where the rule “always pull before branching” comes from']}},
        {type:'commands',title:{th:'5 คำสั่งที่ใช้ทุกวัน',en:'The five commands you use every day'},
          ...
          steps:[
            {title:{th:'ฉันอยู่ตรงไหน และมีอะไรค้างอยู่',en:'Where am I, and what is pending?'},what:{th:'`git status` คือคำสั่งที่ควรพิมพ์บ่อยที่สุด มันบอก 3 อย่าง คือ อยู่ branch ไหน, ไฟล์ไหนถูกแก้แต่ยังไม่ commit และมีไฟล์ใหม่ที่ Git ยังไม่รู้จักหรือไม่ ให้รันทั้งก่อนและหลังให้ Agent ทำงาน',en:'`git status` is the command to type most often. It tells you three things: which branch you are on, which files are modified but not committed, and whether there are new files Git does not track yet. Run it before and after the agent works.'},cmd:`git status`,expect:{th:'ถ้ายังไม่ได้แก้อะไร จะเห็นข้อความทำนอง `nothing to commit, working tree clean` — หากมีไฟล์ใหม่ค้างบน main ให้ตรวจว่าเกิดจากอะไร ก่อนเริ่มงานต่อ',en:'If nothing was edited you see something like `nothing to commit, working tree clean` — If new files appear on main, identify them before proceeding.'}},
        ...
        {type:'prose',title:{th:'ทำไม node_modules ไม่เคยโผล่ใน git status',en:'Why node_modules never shows up in git status'},body:{th:[
          'ตอนรัน `npm ci` ในบทที่ 00 มีไฟล์ถูกสร้างขึ้นเป็นหมื่นไฟล์ในโฟลเดอร์ `node_modules` แต่ `git status` กลับไม่เคยพูดถึงมันเลย ไม่ใช่เพราะ Git มองไม่เห็น แต่เพราะโปรเจกต์มีไฟล์ชื่อ **`.gitignore`** ที่ระบุว่าอะไรบ้างที่ Git ควรทำเป็นมองไม่เห็น',
          ...
          'When you ran `npm ci` in lesson 00, tens of thousands of files appeared under `node_modules` — yet `git status` never mentions them. Not because Git cannot see them, but because the project has a **`.gitignore`** listing what Git should pretend not to see.',
          ...
          '**ของที่สร้างใหม่ได้เสมอ** — `node_modules/`, `.next/`, `storybook-static/` ไม่ต้องเก็บ เพราะรัน `npm ci` หรือ build ใหม่ก็ได้กลับมา',
          ...
          '**Anything regenerable** — `node_modules/`, `.next/`, `storybook-static/`: no need to store what `npm ci` or a build recreates',
        ...
        {type:'commands',title:{th:'ฝึกวงจรเต็ม: แยก branch → แก้ → ดู diff → ย้อนกลับ',en:'Practise the full loop: branch → edit → diff → undo'},
          ...
          steps:[
            {title:{th:'เช็กจุดตั้งต้นว่าสะอาด',en:'Confirm a clean starting point'},what:{th:'ก่อนแยก branch ต้องแน่ใจว่าไม่มีงานค้างอยู่ ถ้ามีไฟล์ค้างแล้วแยก branch มันจะติดตามไปด้วย และทำให้สับสนว่าอะไรเป็นงานของ task ไหน',en:'Before branching, make sure nothing is pending. Uncommitted files follow you onto the new branch and blur which task they belong to.'},cmd:`git status`,expect:{th:'`working tree clean` และไม่มีไฟล์ค้างจากการฝึกบทก่อน',en:'`working tree clean` with no leftover practice files.'}},
            {title:{th:'สร้าง branch ใหม่และย้ายเข้าไปทันที',en:'Create a new branch and switch to it'},what:{th:'`git switch -c` ทำสองอย่างพร้อมกัน คือสร้าง branch ใหม่ (`-c` มาจาก create) แล้วย้ายคุณเข้าไปอยู่บนนั้น ชื่อ branch ควรบอกได้ว่าทำอะไร งานจริงของเราจะใช้ `feature/12-us001-patient-checkin` ในบทถัดไป แต่รอบนี้เป็นการซ้อม จึงใช้ชื่อ `practice/git-warmup` แล้วลบทิ้งตอนจบ',en:'`git switch -c` does two things at once: create a branch (`-c` for create) and move you onto it. The name should say what the work is. Our real branch will be `feature/12-us001-patient-checkin` in the next lesson; this round is a rehearsal, so we use `practice/git-warmup` and delete it at the end.'},cmd:`git switch -c practice/git-warmup`,expect:{th:'`Switched to a new branch ...` และถ้ารัน `git branch` จะเห็น `*` อยู่หน้า branch ใหม่',en:'`Switched to a new branch ...`, and `git branch` now shows `*` on the new branch.'}},
        ...
        {type:'prose',title:{th:'commit เมื่อไร และข้อความควรเขียนอย่างไร',en:'When to commit, and what the message should say'},body:{th:[
          ...
          'ตลอดคอร์สนี้จุดตรวจมีรูปแบบซ้ำ ๆ อยู่สี่แบบ คือ component หนึ่งตัวกับ Storybook story ของมันผ่านสายตาคุณใน Storybook, interaction test ผ่าน, หน้าเว็บเดิน flow ได้ครบ และบั๊กหนึ่งตัวถูกแก้จบพร้อมหลักฐาน — สี่จังหวะนี้คือสี่ commit และคุณจะได้ทำจริงในบทที่ 09, 10, 12 และ 13',
          ...
          'This course repeats four such checkpoints: one component and its Storybook story pass your review in Storybook, an interaction test passes, the page walks the full flow, and one bug is fixed with evidence. Those four moments are four commits, and you will make them in lessons 09, 10, 12, and 13.',
...
Refs #12`,
    ...
    },
    {
      ...
      title:{th:'แยกพื้นที่ทำงานด้วย Git Worktree',en:'Isolate Work with Git Worktree'},
      intro:{th:'ใช้ branch และ worktree เดียวกับ US-001 ตลอดคอร์ส เพื่อเก็บงานแยกจาก main และกลับมาทำต่อได้',en:'Keep US-001 on one branch and worktree throughout the course, separate from main and ready to resume.'},
      outcomes:{th:['เข้าใจ 1 Issue = 1 Branch = 1 Worktree','สร้าง/ดู/ลบ worktree ได้','รู้เรื่อง port conflict และ dependency ต่อ worktree','รู้ว่า parallel เหมาะเมื่อ task แยกกันจริง'],en:['Understand 1 Issue = 1 Branch = 1 Worktree','Create/list/remove worktrees','Understand port conflicts and per-worktree dependencies','Use parallel work only when tasks are truly independent']},
      blocks:[
        {type:'prose',title:{th:'ปัญหาที่ Worktree เกิดมาเพื่อแก้',en:'The problem worktrees exist to solve'},body:{th:[
          ...
          'กฎที่เราจะใช้ตลอดคอร์สคือ **1 Issue = 1 Branch = 1 Worktree** จำง่ายและตัดปัญหาได้เกือบทั้งหมด'
        ],en:[
          ...
          'The rule we use all course: **1 Issue = 1 Branch = 1 Worktree.** Easy to remember, and it removes most of the confusion.'
        ]}},
        ...
        {type:'two',title:{th:'ตกลงต้องใช้ switch หรือ worktree',en:'So which one: switch or worktree?'},left:{title:{th:'`git switch -c` พอแล้ว',en:'`git switch -c` is enough'},items:{th:['ทำงานทีละเรื่อง ไม่มีอะไรค้าง','แก้เล็ก ๆ ที่จบได้ในไม่กี่นาที','ไม่มี Agent หรือ dev server รันค้างอยู่','พื้นที่ดิสก์จำกัด เพราะแต่ละ worktree ต้อง `npm ci` ของตัวเอง'],en:['One task at a time with nothing left open','A small change you finish in minutes','No agent session or dev server left running','Limited disk, since each worktree needs its own `npm ci`']}},right:{title:{th:'ใช้ worktree',en:'Use a worktree'},items:{th:['มี Agent session รันค้างอยู่กับไฟล์ชุดนั้น','มีงานแทรกด่วนแต่ยังไม่อยากทิ้งงานเดิมกลางคัน','อยากเปิด Storybook ของสองงานเทียบกัน','งานยาวข้ามวัน'],en:['An agent session is live on that set of files','Something urgent arrives and you will not abandon the current work','You want two Storybooks open side by side','Work that spans more than a day']}}},
        {type:'callout',title:{th:'ค่าเริ่มต้นของคอร์สนี้',en:'The default for this course'},text:{th:'งาน US-001 ใช้ worktree เดียว `wt-us001-patient-checkin` และ branch เดียว `feature/12-us001-patient-checkin` โดย **12 เป็นเพียงเลขตัวอย่าง** ก่อนคัดลอกคำสั่ง Git ทุกบท ให้แทน `12` และ `#12` ด้วยเลข Issue จริงของคุณ ทั้ง branch, commit footer และ MR `Closes` ต้องตรงกัน รันคำสั่งงานจาก worktree นี้ ไม่ใช่ repo หลัก',en:'US-001 uses one worktree, `wt-us001-patient-checkin`, and one branch, `feature/12-us001-patient-checkin`. **12 is only an example.** Before copying Git commands in any lesson, replace `12` and `#12` with your own issue number. The branch, commit footer, and MR `Closes` must match. Run task commands from this worktree, not the main folder.'}},
...
git status`,expect:{th:'อยู่บน main และ `working tree clean`',en:'You are on main with a clean tree and no leftover practice files.'}},
            ...
            {title:{th:'สร้าง worktree พร้อม branch ใหม่',en:'Create the worktree and its branch together'},what:{th:'คำสั่งนี้ทำ 3 อย่างในครั้งเดียว: สร้างโฟลเดอร์ `../wt-us001-patient-checkin`, สร้าง branch ชื่อ `feature/12-us001-patient-checkin` (จาก `-b`) และผูก branch นั้นเข้ากับโฟลเดอร์ใหม่ ส่วน `main` ท้ายคำสั่งคือจุดตั้งต้น เครื่องหมาย `..` แปลว่าวางโฟลเดอร์ไว้ข้างนอก repo หลัก ไม่ใช่ซ้อนอยู่ข้างใน',en:'This does three things at once: creates the folder `../wt-us001-patient-checkin`, creates the branch `feature/12-us001-patient-checkin` (that is what `-b` means), and binds that branch to the new folder. The trailing `main` is the starting point. The `..` places the folder beside the main repo rather than nested inside it.'},cmd:`git worktree add ../wt-us001-patient-checkin \\
  -b feature/12-us001-patient-checkin main`,expect:{th:'ข้อความ `Preparing worktree ...` และมีโฟลเดอร์ใหม่เกิดขึ้นข้าง ๆ repo เดิม',en:'A `Preparing worktree ...` message and a new folder beside the original repo.'}},
...
git status`,expect:{th:'`On branch feature/12-us001-patient-checkin` — ถ้ายังขึ้น main แปลว่า cd ผิดโฟลเดอร์',en:'`On branch feature/12-us001-patient-checkin`. If it still says main, you changed into the wrong folder.'}},
            {title:{th:'ติดตั้ง dependency ของ worktree นี้',en:'Install dependencies for this worktree'},what:{th:'`node_modules` ไม่ถูกแชร์ข้าม worktree เพราะ Git ไม่ได้ติดตามโฟลเดอร์นี้ จึงต้อง install ใหม่ในแต่ละ worktree ที่จะรันแอป',en:'`node_modules` is not shared between worktrees because Git does not track that folder, so each worktree that runs the app needs its own install.'},cmd:`npm ci`,expect:{th:'ติดตั้งเสร็จ และรัน `npm run dev` ใน worktree นี้ได้',en:'The install completes and `npm run dev` works inside this worktree.'}}
          ]},
        {type:'callout',tone:'warning',title:{th:'สิ่งที่มักเจอ',en:'Common friction'},text:{th:'แต่ละ worktree มีไฟล์ working copy ของตัวเอง จึงอาจต้อง `npm ci` และถ้ารันหลาย Next.js/Storybook พร้อมกันต้องใช้คนละ port เช่น 3001/6001 และ 3002/6002',en:'Each worktree has its own working files, so you may need `npm ci`. Multiple Next.js/Storybook processes also need different ports, e.g. 3001/6001 and 3002/6002.'}},
        {type:'commands',title:{th:'เมื่องานจบแล้ว: เก็บกวาด worktree',en:'When the work is done: clean up the worktree'},
          lead:{th:'อย่าปล่อยให้มี worktree ค้างเป็นสิบอัน เพราะจะสับสนว่าโฟลเดอร์ไหนคืองานไหน ทำขั้นตอนนี้ตอนจบจริง ๆ เท่านั้น คือหลังจาก MR ถูก merge แล้ว (ลำดับเต็มอยู่ในบทที่ 15) ถ้าทำก่อน คุณจะไม่มีที่ให้แก้งานตาม review',en:'Do not let a dozen worktrees pile up — you will lose track of which folder is which task. Do this only at the true end, after the MR is merged (the full order is in lesson 15). Clean up earlier and you have nowhere to make review fixes.'},
          steps:[
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'prompt',title:{th:'Explore Prompt — ใช้ก่อนเสมอ',en:'Explore Prompt — always use it first'},
          when:{th:'ใช้เป็น prompt แรกของทุก Issue หลังเปิด Agent ใน worktree ที่ถูกต้องแล้ว สังเกตว่าเราไม่ได้ชี้ให้มันอ่านไฟล์ requirement ในเครื่องเป็นหลัก แต่ให้อ่าน Issue จริงจาก GitHub/GitLab ผ่าน gh หรือ glab เพราะ Issue คือสัญญาฉบับสุดท้ายที่มี AC และเลขที่จะตามไปถึง commit และ MR ประโยค “ห้ามแก้ไขไฟล์” คือสิ่งที่ทำให้คุณยังมีอำนาจตัดสินใจอยู่',en:'Use it as the first prompt for every issue, after opening the agent in the correct worktree. Notice it is not pointed mainly at a local requirement file — it reads the real issue from GitHub/GitLab through gh or glab, because the issue is the final contract carrying the ACs and the number that will flow into commits and the MR. The “do not modify files” line is what keeps the decision in your hands.'},
          prompt:{th:`อ่านไฟล์ AGENTS.md
...
7. เลข Issue ที่งานนี้ต้องอ้างถึงคือเลขอะไร และมันจะไปโผล่ที่ไหนบ้างในขั้นถัดไป

สรุปเป็นแผนการทำงานสั้น ๆ เพื่อให้คนตรวจก่อนอนุมัติ`,en:`Read AGENTS.md.

Then run the command that reads this work's issue (read-only):
GitHub:  gh issue view <your issue number>
GitLab:  glab issue view <your issue number>
If the issue links to the full requirement doc, read that too for background.

The issue body is the contract for this work. Wherever the issue does not answer something, do not guess at this stage — note it and ask me.
...
7. Which issue number this work must reference, and where it will show up next

Summarize it as a short implementation plan for human review.`},
          after:{th:['รัน `git status` ทันที ต้องยังสะอาด ถ้าไม่สะอาดแปลว่า Agent ไม่ทำตามข้อห้าม','เทียบรายการ UI state กับ acceptance criteria ใน Issue ของคุณทีละข้อ','ถ้าคำสั่ง gh หรือ glab อ่าน Issue ไม่ได้ เช่น ยังไม่ login ให้แก้ตรงนั้นก่อน อย่าสั่งงานต่อโดยที่ Agent ยังไม่เห็น Issue จริง','ดูจำนวนไฟล์ที่มันจะแตะ ถ้าเยอะผิดปกติให้ถามว่า “ไฟล์นี้จำเป็นเพราะอะไร”'],en:['Run `git status` immediately — it must still be clean; if not, the agent ignored the constraint','Compare its UI state list against the acceptance criteria in your issue, one by one','If gh or glab cannot read the issue, e.g. you are not signed in, fix that first — never continue while the agent has not seen the real issue','Look at how many files it plans to touch; if the number looks high, ask “why is this file necessary?”']}},
...
Return the revised plan for another review. Do not implement yet.`},
        ...
        {type:'prompt',title:{th:'Implement Prompt — หลังอนุมัติแผนแล้วเท่านั้น (บทนี้เป็นตัวอย่าง ยังไม่ต้องรัน)',en:'Implement Prompt — only after the plan is approved (an example here — do not run it yet)'},
          when:{th:'แสดงไว้เพื่อให้เห็นภาพครบทั้ง 3 ขั้น Explore → Refine → Implement **แต่ในบทนี้ยังห้ามรัน** ให้อ่านเป็นตัวอย่างไว้ก่อน เพราะถ้ารันตอนนี้ Agent จะสร้างหน้า `/opd/check-in` ทั้งหน้าให้ทันที ทั้งที่เรายังไม่ได้วาง Design System (บทที่ 07) และยังไม่ได้แตกงานเป็น component ทีละชิ้นเพื่อ review (บทที่ 08–09) คุณจะได้ใช้รูปแบบนี้จริงตั้งแต่บทที่ 09 เป็นต้นไป สังเกตว่าเราไม่ได้อธิบายวิธีเขียนโค้ด แต่กำหนดขอบเขตและสิ่งที่ห้ามทำ รวมถึงห้าม commit/push ซึ่งเป็นการรักษาสิทธิ์ตรวจงานไว้ที่คน',en:'Shown so you can see all three stages — Explore → Refine → Implement — but **do not run it in this lesson yet**; read it as an example. If you ran it now, the agent would build the whole `/opd/check-in` page on the spot, before we have set up the design system (lesson 07) or broken the work into components for review (lessons 08–09). You will use this pattern for real from lesson 09 onward. Notice we never describe how to write code; we set boundaries and prohibitions — including no commit or push, which keeps the final check with a human.'},
...
When finished, run the relevant checks and summarize every changed file and why it changed.`},
        ...
        {type:'callout',tone:'warning',title:{th:'บทนี้หยุดไว้ที่ “แผนที่อนุมัติแล้ว”',en:'This lesson stops at the approved plan'},text:{th:'Practice ของบทนี้จบเมื่อแผนผ่านการ review เท่านั้น อย่าส่ง Implement Prompt ตอนนี้ ถ้าส่งไป Agent จะสร้างหน้า `/opd/check-in` ทั้งหน้าให้ทันที แล้วคุณจะไม่มีโอกาสได้ฝึกขั้นที่เหลือของคอร์ส คือการวาง Design System (บทที่ 07) และการแตกงานเป็น component ทีละชิ้น (บทที่ 08–09)',en:'This lesson’s practice ends once your plan passes review. Do not send the Implement Prompt now — the agent would build the entire `/opd/check-in` page immediately, and you would lose the chance to practise the remaining steps of the course: setting up the design system (lesson 07) and breaking the work into components one by one (lessons 08–09).'}},
        ...
        {type:'practice',title:{th:'ลงมือทำ: ให้ Agent เสนอแผน US-001',en:'Practice: ask the agent to plan US-001'},steps:{th:['เปิด Agent ใน worktree US-001','ส่ง Explore Prompt แล้วรอแผน','รัน `git status` เพื่อยืนยันว่ายังไม่มีไฟล์ถูกแก้','เปรียบเทียบ plan กับ acceptance criteria ทีละข้อ','ถ้ายังขาด ให้ใช้ Refine Prompt แล้ว review ซ้ำจนพอใจ','หยุดไว้ที่แผนที่อนุมัติแล้ว — ยังไม่ต้องส่ง Implement Prompt (จะได้ใช้จริงตั้งแต่บทที่ 09 เป็นต้นไป)'],en:['Open the agent in the US-001 worktree','Send the Explore Prompt and wait for the plan','Run `git status` to confirm no files changed','Compare the plan against the acceptance criteria one by one','If gaps remain, use the Refine Prompt and review again until satisfied','Stop at the approved plan — do not send the Implement Prompt yet (you will use it for real from lesson 09 onward)']},expected:{th:'ได้ plan ที่ระบุ component, state, file scope และ verification โดยยังไม่มีไฟล์ถูกแก้',en:'You get a plan listing components, states, file scope, and verification, with no files changed yet.'}}
      ],
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'prompt',title:{th:'Prompt: ให้ Agent พาทัวร์โปรเจกต์',en:'Prompt: ask the agent for a guided tour'},
          when:{th:'ใช้เมื่ออยากรู้ว่า component ใหม่ควรไปอยู่ที่ไหน ก่อนอนุญาตให้สร้างไฟล์ การถามก่อนช่วยจับกรณีที่ Agent กำลังจะวางไฟล์ผิดที่ ถ้ายังเก็บคำตอบจาก Prompt ของบทที่ 01 ไว้ ให้เปิดมาวางเทียบข้าง ๆ คำตอบรอบนี้ แล้วดูว่าข้อไหนที่ตอนนี้คุณตรวจความจริงได้เองแล้ว ไม่ต้องเชื่อ Agent แบบเดิม',en:'Use it when you want to know where a new component belongs, before allowing any file creation. Asking first catches cases where the agent is about to put files in the wrong place. If you kept your answer from the lesson 01 prompt, open it beside this round and see which points you can now verify yourself instead of taking the agent’s word.'},
...
Answer briefly as a numbered list with file paths.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: หา route และ script',en:'Practice: find the route and scripts'},steps:{th:['เปิด `src/app/opd/check-in/page.tsx`','เปิด `package.json` แล้วหา dev, storybook, lint, test และ build','เปิด URL จริงในเบราว์เซอร์แล้วเทียบกับไฟล์','ถาม Agent ด้วย Prompt ด้านบน แล้วตรวจว่า path ที่มันตอบมีอยู่จริง','เปิดคำตอบที่เก็บไว้จากบทที่ 01 มาเทียบกับคำตอบรอบนี้ ข้อไหนที่ตอนนี้คุณอ่านแล้วเข้าใจมากขึ้น และข้อไหนที่ยังต้องเชื่อ Agent — สังเกตด้วยว่าคำถามข้อ 2 (component ใหม่ควรวางที่ไหน) เป็นคำถามที่ตอนบทที่ 01 ยังถามไม่เป็น แต่ตอนนี้ต้องใช้จริง เพราะคุณกำลังจะให้ Agent สร้าง component'],en:['Open `src/app/opd/check-in/page.tsx`','Open `package.json` and find dev, storybook, lint, test, and build','Open the real URL in the browser and compare with the file','Send the prompt above and verify the paths it cites actually exist','Open the answer you kept from lesson 01 beside this round: which points do you now understand more deeply, and which still rest on trusting the agent? Notice that question 2 — where a new component belongs — is one you could not have framed back in lesson 01, yet now it matters, because you are about to have the agent build components']},expected:{th:'สามารถอธิบายว่า route, page และ feature folder เชื่อมกันอย่างไร และรู้คำสั่งที่ใช้รัน/ตรวจงาน',en:'You can explain how route, page, and feature folders connect and know the commands used to run/check work.'}}
      ],
    ...
    {
      id:'design-system', group:'day2', no:'07', duration:'60 min',
      title:{th:'จาก Design สู่ DESIGN.md: วาง Design System ก่อนสร้าง Component',en:'From Design to DESIGN.md: Set the Design System Before Components'},
      intro:{th:'ก่อนให้ Agent สร้าง component แม้แต่ตัวเดียว ให้มันอ่าน “ภาษาการออกแบบ” จากภาพหน้าจอหรือ Figma แล้วสรุปเป็นไฟล์เดียวที่อ้างอิงซ้ำได้ แทนที่จะอธิบายสี ฟอนต์ และระยะห่างใหม่ทุกครั้งที่สั่งงาน',en:'Before the agent builds a single component, have it read the brand’s design language from a screenshot or Figma file and summarize it into one file it can reference every time — instead of re-describing colors, fonts, and spacing in every prompt.'},
      outcomes:{th:['อธิบายได้ว่า DESIGN.md ต่างจาก Storybook อย่างไร และทำไมบทนี้ต้องมีทั้งคู่','สั่ง Agent สรุป design token จากภาพหน้าจอ/Figma เป็น DESIGN.md ตาม pattern มาตรฐาน','สั่ง Agent สร้างหน้า Style Guide ที่ /design โดยอ้างอิงจาก DESIGN.md เพียงไฟล์เดียว','รู้ว่าต้องตรวจอะไรก่อนอนุมัติ DESIGN.md และมันจะถูก commit ตรงไหนของ workflow'],en:['Explain how DESIGN.md differs from Storybook, and why this lesson needs both','Direct the agent to summarize design tokens from a screenshot or Figma file into DESIGN.md, following a standard pattern','Direct the agent to build a /design style guide page that reads from DESIGN.md alone','Know what to check before approving DESIGN.md, and where it fits in the commit workflow']},
      blocks:[
        {type:'prose',title:{th:'จากภาพอ้างอิงสู่กติกาที่ใช้ซ้ำ',en:'From visual reference to reusable rules'},body:{th:[
          'ภาพ `docs/design/opd-check-in-reference.png` ช่วยบอกลำดับสายตา สี และระยะห่าง แต่ไม่บอก Preview, Error หรือพฤติกรรมอื่นทั้งหมด จึงต้องอ่านคู่กับ requirement',
          'ให้ Agent ร่าง `DESIGN.md` เป็น token พร้อมหน้าที่และเหตุผล แล้วให้ Design ตรวจค่าจากภาพกับ Known Gaps ก่อนสร้าง component; หน้า `/design` ใช้แสดงค่าที่อนุมัติแล้ว'
        ],en:[
          'The `docs/design/opd-check-in-reference.png` image shows visual hierarchy, color, and spacing, but not Preview, Error, or the whole behavior. Read it alongside the requirement.',
          'Have the agent draft `DESIGN.md` with tokens, roles, and rationale. Design reviews image-backed values and Known Gaps before components are built; `/design` displays the approved values.'
        ]}},
        {type:'diagram',title:{th:'จากภาพ ไปเป็นกติกาที่ Agent อ้างอิงซ้ำได้',en:'From an image to rules the agent can reuse'},
          lead:{th:'ภาพนี้คือสิ่งที่บทนี้จะทำให้เกิดขึ้นจริง — DESIGN.md เป็นจุดเดียวที่ทั้งหน้า Style Guide และ Component ในบทถัดไปอ้างอิงกลับมา',en:'This is what this lesson produces — DESIGN.md becomes the single source both the style guide page and, later, every component refer back to.'},
          diagram:`flowchart LR\nD["Design reference\\nscreenshot or Figma"] --> AG[AI Agent summarizes]\nAG --> M["DESIGN.md\\ntoken + rule + rationale"]\nM --> S["/design\\nstyle guide page"]\nH{Human review} --> Commit[Commit both files]\nS --> H\nH -->|แก้ / revise| AG\nM -.ใช้ต่อใน / referenced later by.-> C[Component code]`,
          notes:{th:['Agent เห็นแค่ภาพที่คุณให้ ไม่ได้เดาแบรนด์เอาเอง','DESIGN.md ไม่ใช่ปลายทาง แต่เป็นจุดตั้งต้นของทั้ง Style Guide และ component ที่จะสร้างต่อ','ลูกศรที่วิ่งกลับไปหา Agent คือรอบแก้ไข ซึ่งเกิดได้หลายรอบก่อนอนุมัติ','component ในบทถัดไปจะอ้างอิง token จากไฟล์นี้ ไม่ใช่คิดค่าสีขึ้นใหม่เอง'],en:['The agent only sees the image you give it — it does not guess the brand on its own','DESIGN.md is not the destination; it is the starting point for both the style guide and every component built later','The arrow back to the agent is a revision round, and there can be several before approval','Components in the next lesson will reference tokens from this file rather than inventing new color values']}},
        {type:'two',title:{th:'DESIGN.md คืออะไร และไม่ใช่อะไร',en:'What DESIGN.md is, and is not'},left:{title:{th:'ตอบคำถามพวกนี้',en:'Answers these questions'},items:{th:['สีแต่ละตัวมีหน้าที่อะไร ไม่ใช่แค่ค่า hex','ทำไม heading ถึงหนัก/บางขนาดนี้','ระยะห่างและมุมโค้งมีสเกลอย่างไร','component หน้าตาประกอบจาก token ไหนบ้าง','กรณีไหนที่ยังไม่ถูกครอบคลุม (Known Gaps)'],en:['What job each color does, not just its hex value','Why a heading is this heavy or this light','What scale spacing and corner radii follow','Which tokens a component is built from','Which cases are not covered yet (Known Gaps)']}},right:{title:{th:'ไม่ใช่สิ่งนี้',en:'Is not this'},items:{th:['ไม่ใช่โค้ด component ที่พร้อมใช้ — ยังต้อง implement ต่อ','ไม่ใช่ Figma token export แบบ JSON ที่ไม่มีเหตุผลกำกับ','ไม่ใช่ brand guideline ที่เขียนคลุมเครือสำหรับคน เช่น “ดูพรีเมียมแต่เข้าถึงง่าย”','ไม่ใช่ของตายตัว — แก้ วนรีวิว และ commit ได้เหมือนโค้ด'],en:['Not ready-to-use component code — implementation still has to happen','Not a Figma JSON token export with no reasoning attached','Not a brand guideline written loosely for humans, like “feels premium yet approachable”','Not static — it gets revised, reviewed, and committed like code']}}},
        {type:'code',title:{th:'โครงของ DESIGN.md ที่เราจะใช้',en:'The DESIGN.md shape we will use'},
          lead:{th:'ส่วน YAML ระหว่าง `---` สองบรรทัดเก็บ token ส่วนข้อความข้างล่างอธิบายวิธีใช้ ตรวจว่าค่าที่อ้างมีจริง และแยกค่าที่อ่านจากภาพไม่ได้ไว้ใน Known Gaps',en:'YAML between the two `---` lines holds tokens; prose below explains their use. Check that referenced values exist and put values the image cannot establish under Known Gaps.'},
          label:'DESIGN.md (excerpt, US-001 OPD Check-in)',code:`---
version: alpha
name: OPD Check-in Lite
description: A calm clinical surface where a single teal accent marks every
  primary action. Generous spacing and hairline borders keep dense patient
  data easy to scan under time pressure.

colors:
  primary: "#0F766E"      # teal — every primary action (search, confirm)
  danger: "#B91C1C"       # errors only, never decorative
  ink: "#111827"          # body text
  canvas: "#F9FAFB"       # page background
  surface-card: "#FFFFFF" # patient card / form background
  hairline: "#E5E7EB"     # 1px separators, never a drop shadow

typography:
  heading-md: { fontSize: 20px, fontWeight: 600, lineHeight: 1.3 }
  body-md:    { fontSize: 15px, fontWeight: 400, lineHeight: 1.5 }
...
## Colors
**Teal** ({colors.primary}) carries every primary action — search, confirm,
save. Used scarcely, so it never competes with clinical data.

## Known Gaps
- Dark mode is not covered.
- Print layout for the visit slip is not covered.`,
          note:{th:'นี่เป็นตัวอย่างย่อ ไม่ใช่ค่าที่วัดได้แน่นอนจากภาพ ทีม Design ต้องยืนยัน token ที่สำคัญก่อนนำไปใช้จริง รูปแบบ DESIGN.md ยังเป็น alpha จึงให้ Agent ตรวจด้วย lint แล้วอ่าน warning ด้วยคน',en:'This is a short example, not a set of values measured exactly from the screenshot. Design must confirm important tokens before real use. The DESIGN.md format is still alpha, so have the agent lint it and review warnings with a human.'}},
        {type:'prompt',title:{th:'Prompt: ให้ Agent สรุป Design เป็น DESIGN.md',en:'Prompt: summarize the design in DESIGN.md'},
...
Tell me which URL to open to see the result.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: จากภาพสู่ Style Guide',en:'Practice: from image to style guide'},steps:{th:['เปิดภาพอ้างอิง `docs/design/opd-check-in-reference.png` ในโปรเจกต์ starter (หรือภาพ mockup อื่นที่ได้รับมา)','แนบภาพแล้วส่ง Prompt ให้ Agent สรุป DESIGN.md โดยแยกสิ่งที่เห็นจริงจากสิ่งที่ต้องถาม','ตรวจ DESIGN.md กับภาพ/requirement ด้วย checklist ด้านบน','ส่ง Prompt สร้างหน้า /design แล้วเปิดเทียบกับ DESIGN.md','ตรวจ token ตัวอย่างบนหน้า Style Guide ว่าตรงกับ DESIGN.md'],en:['Open the reference image at `docs/design/opd-check-in-reference.png` in the starter project (or another mockup you received)','Attach it and ask the agent to summarize DESIGN.md, separating visible facts from open decisions','Review DESIGN.md against the image, requirement, and checklist','Send the /design page prompt and compare the result against DESIGN.md','Verify a sample style-guide token against DESIGN.md']},expected:{th:'มี DESIGN.md ที่ไม่มี lint error พร้อมรายการ warning/คำถามที่ต้องยืนยัน และหน้า /design ที่แสดง token ตรงกับไฟล์ ยังไม่ commit',en:'A DESIGN.md with no lint errors and documented warnings/questions, plus a /design page matching its tokens. Not committed yet.'}}
      ],
    ...
    {
      id:'component-state', group:'day2', no:'08', duration:'75 min',
...
- WithResults: พบหลายรายการ เลือกได้รายการเดียว การ์ดแต่ละใบแสดง HN ชื่อ วันเกิด และเพศ
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'diagram',title:{th:'Component-driven loop',en:'Component-driven loop'},
          ...
          notes:{th:['ทุกอย่างเริ่มจาก Acceptance Criteria ไม่ใช่เริ่มจากไอเดียของ Agent','component ดึงหน้าตา (สี/font/spacing) จาก DESIGN.md ส่วน state ดึงจาก Acceptance Criteria — คนละแหล่งข้อมูล คนละคำถาม','วงลูป feedback คือส่วนที่คุณใช้เวลามากที่สุด และเป็นส่วนที่มีค่าที่สุด','ลูกศร approved จะเกิดขึ้นได้ต่อเมื่อคนเปิดดูจริงแล้วเท่านั้น'],en:['Everything starts from acceptance criteria, not from the agent’s idea','A component pulls its look (color/type/spacing) from DESIGN.md while its states come from acceptance criteria — different sources, different questions','The feedback loop is where you spend most of your time — and where most of the value is','The approved arrow only happens after a human actually looked']}},
...
Summarize how you addressed each point.`},
        ...
        {type:'list',title:{th:'Human review ใน Storybook ดูอะไร',en:'What humans review in Storybook'},items:{th:['คำและข้อความสอดคล้อง Requirement','Visual hierarchy อ่านง่าย','Loading/Empty/Error ไม่ทำให้ผู้ใช้สับสน','ข้อมูลยาว/optional field ไม่ทำ layout พัง','Interaction controls มีความหมาย','ใช้ Tab/Enter ได้และ focus มองเห็น','จอแคบยังอ่านและกดได้'],en:['Copy matches the requirement','Visual hierarchy is easy to scan','Loading/Empty/Error are understandable','Long/optional data does not break layout','Interaction controls are meaningful','Tab/Enter work and focus is visible','Narrow layout remains readable and actionable']}},
        {type:'commands',title:{th:'Commit checkpoint 1 · component แรกที่ผ่าน review',en:'Commit checkpoint 1 · the first reviewed component'},
          lead:{th:'สามคำสั่งนี้ **คุณเป็นคนรันเอง** ไม่ใช่ Agent (prompt ทุกอันในคอร์สห้าม Agent commit) และรันจากในโฟลเดอร์ worktree `wt-us001-patient-checkin` ทำก็ต่อเมื่อคุณเปิดดู Storybook story ครบทุก state แล้วพอใจ ถ้ายังมี state ที่ดูแล้วไม่ผ่าน อย่าเพิ่ง commit ถ้า `DESIGN.md` และหน้า `/design` จากบทที่ 07 ยังไม่เคย commit ให้รวมเข้ามาที่จุดนี้ด้วย — เป็นของที่ component แรกนี้พึ่งพาอยู่แล้ว',en:'You run these three yourself — not the agent (every prompt in this course forbids the agent from committing) — from inside the `wt-us001-patient-checkin` worktree. Only after you have opened every Storybook story state and are satisfied — if one state still looks wrong, do not commit yet. If `DESIGN.md` and the `/design` page from lesson 07 have not been committed yet, bundle them in here too — this first component already depends on them.'},
          steps:[
            {title:{th:'ดูว่ามีอะไรเปลี่ยนไปบ้าง และอยู่ branch ถูกหรือเปล่า',en:'See what changed, and confirm the branch'},what:{th:'บรรทัดแรกต้องเป็น `On branch feature/12-us001-patient-checkin` ถ้าไม่ใช่ ให้หยุดแล้วย้อนไปดูบทที่ 04 ก่อน จากนั้นอ่านรายชื่อไฟล์ว่าตรงกับงานที่เพิ่ง review หรือไม่',en:'The first line must read `On branch feature/12-us001-patient-checkin`. If not, stop and revisit lesson 04. Then read the file list and check it matches the work you just reviewed.'},cmd:`git status`,expect:{th:'ชื่อ branch ถูกต้อง และรายชื่อไฟล์ไม่มีอะไรที่คุณอธิบายไม่ได้',en:'The right branch name, and no file you cannot explain.'}},
            {title:{th:'เลือกเฉพาะไฟล์ของงานนี้',en:'Stage only this task’s files'},what:{th:'ระบุ path ของงานนี้ตรง ๆ แทนการใช้ `git add .` เพราะ `.` จะกวาดทุกไฟล์ที่เปลี่ยนในโฟลเดอร์เข้ามาด้วย รวมถึงไฟล์ที่ Agent เผลอสร้างทิ้งไว้ การระบุ path ทำให้ commit นี้มีแต่เรื่องเดียว ถ้ายังไม่เคย commit `DESIGN.md` กับหน้า `/design` ให้ใส่ path ของทั้งสองเข้ามาด้วย',en:'Name the path for this task instead of `git add .` — the dot sweeps in every changed file in the folder, including anything the agent left behind. Naming the path keeps this commit about one thing. If `DESIGN.md` and the `/design` page are not committed yet, add their paths too.'},cmd:`git add src/features/opd-checkin DESIGN.md src/app/design`,expect:{th:'`git status` แสดงไฟล์เป็นสีเขียวใต้หัวข้อ `Changes to be committed`',en:'`git status` shows those files in green under `Changes to be committed`.'}},
    ...
    {
      id:'acceptance-interaction', group:'day2', no:'10', duration:'60 min',
...
Also tell me how to view this test running inside Storybook.`},
    ...
    {
      id:'mock-data', group:'day3', no:'11', duration:'45 min',
...
Propose a plan first. Wait for my approval before implementing.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: บังคับทุก state ด้วย mock',en:'Practice: force every state with mocks'},steps:{th:['ส่ง Prompt ด้านบนก่อน แล้วอนุมัติแผนเมื่อมั่นใจว่าสลับได้ครบทั้ง 4 scenario โดยไม่ต้องแก้โค้ด component — ขั้นที่เหลือด้านล่างเริ่มได้หลังจากนี้เท่านั้น','ใช้ mock scenario `normal` แล้วค้นหา HN 65000123','สลับเป็น `empty` แล้วค้น 99999999 เพื่อตรวจ Empty state','สลับเป็น `slow` แล้วสังเกต Loading ว่าอยู่นานพอให้ผู้ใช้เข้าใจหรือไม่','สลับเป็น `error` แล้วตรวจว่า Error state บอกผู้ใช้ชัดเจนว่าควรทำอะไรต่อ','ทำซ้ำแต่ละ scenario สองครั้ง เพื่อยืนยันว่าได้ผลเหมือนเดิม'],en:['Send the prompt above first, approving the plan only once all four scenarios switch without editing component code — the steps below assume this already works','Use the `normal` scenario and search HN 65000123','Switch to `empty`, search 99999999, and inspect the Empty state','Switch to `slow` and judge whether Loading lasts long enough to be understood','Switch to `error` and check the Error state tells users what to do next','Repeat each scenario twice to confirm identical results']},expected:{th:'เปิด UI state ทั้ง 4 แบบได้ซ้ำ ๆ โดยไม่ต้องมี Backend จริงและไม่ใช้ข้อมูลผู้ป่วยจริง',en:'You can repeatedly reproduce all four UI states without a real backend or real patient data.'}}
      ],
    ...
    {
      id:'integration', group:'day3', no:'12', duration:'60 min',
      ...
      outcomes:{th:['เห็นความต่างระหว่าง isolated component กับ integrated flow','review state transition ทั้งหน้า','ตรวจ validation และ confirmation flow','ตรวจ keyboard และจอแคบ','ใช้ mock service เดิมโดยไม่สร้าง backend'],en:['See the difference between isolated components and an integrated flow','Review state transitions across the page','Check validation and confirmation flow','Check keyboard and narrow viewport','Reuse the mock service without adding a backend']},
      blocks:[
...
When finished, describe step by step how the screen changes state from the first search to success.`},
        ...
        {type:'commands',title:{th:'เดิน flow จริงอย่างเป็นระบบ',en:'Walk the real flow systematically'},
          ...
          steps:[
            ...
            {title:{th:'รอบที่ 6 — เริ่มรอบใหม่ด้วยผู้ป่วยคนเดิม',en:'Round 6 — start over with the same patient'},what:{th:'หลัง Success อย่ากดปุ่มเริ่มใหม่ ให้ค้นหาแล้วเลือกผู้ป่วยคนเดิมซ้ำ จุดที่ต้องดูคือคลินิกและ chief complaint ของรอบก่อนต้องไม่ติดค้างอยู่ในฟอร์ม — AC12 กำหนดว่าการเริ่ม check-in ใหม่ต้องเริ่มจากฟอร์มว่างเสมอ แม้จะเป็นผู้ป่วยคนเดียวกัน และนี่คืออีกจุดพังที่พบบ่อยรองจากรอบที่ 5',en:'After Success, do not press start-over — search and select the same patient again. The previous round’s clinic and chief complaint must not linger in the form: AC12 requires every new check-in to start from a clean form, even for the same patient. This is the next most common integration bug after round 5.'},expect:{th:'ช่องคลินิกกลับไปเป็นค่าว่าง “เลือกคลินิก” และช่องอาการเบื้องต้นว่างเปล่า',en:'The clinic select is back to its empty placeholder and the chief complaint field is blank.'}}
          ],
          outro:{th:'จดผลทั้ง 6 รอบไว้ ถ้ารอบไหนไม่ผ่าน ให้เขียนเป็นพฤติกรรมที่สังเกตได้ เช่น “กดย้อนกลับแล้วคลินิกที่เลือกไว้หาย” แล้วส่งให้ Agent แก้ทีละข้อ',en:'Record all six rounds. For any that fail, write the observed behaviour — “going back clears the selected clinic” — and send them to the agent one at a time.'}},
        ...
        {type:'practice',title:{th:'ลงมือทำ: Walk the flow',en:'Practice: walk the flow'},steps:{th:['ให้ Agent integrate ตาม prompt','เดินทั้ง 6 รอบตามตารางด้านบน','จดปัญหาเป็น behavior feedback ไม่ใช่ code instruction','ส่งให้ Agent แก้ทีละข้อ แล้วเดิน flow ซ้ำเพื่อยืนยัน','ยืนยันว่า Storybook เดิมยังไม่พัง','ลอง Tab/Enter และจอแคบ 375px ให้เห็น focus และ action หลัก'],en:['Ask the agent to integrate using the prompt','Walk all six rounds above','Write issues as behaviour feedback, not code instructions','Send them one at a time and re-walk the flow to confirm','Confirm the existing Storybook stories still work','Try Tab/Enter and a 375px viewport; focus and primary actions must remain visible']},expected:{th:'Flow หลักทำงานครบจาก Search → Select → Form → Preview → Back/Edit → Confirm → Success, state สำคัญทดสอบได้ และ keyboard/mobile ใช้งานได้',en:'The flow works from Search → Select → Form → Preview → Back/Edit → Confirm → Success, key states remain testable, and keyboard/mobile use works.'}}
      ],
    ...
    {
      id:'debugging', group:'day3', no:'13', duration:'75 min',
...
Do not modify any further files in this round.`},
    ...
    {
      id:'diff-quality', group:'day3', no:'14', duration:'60 min',
...
If you cannot justify a file, say so plainly.`},
          after:{th:['เทียบคำตอบข้อ 2 กับผลของ `git diff --stat` ว่าจำนวนไฟล์ตรงกันหรือไม่','ถ้ามันตอบข้อ 3 ว่า “ไม่มี” ทั้งที่คุณเห็นไฟล์แปลก ๆ ให้ถามเจาะจงไฟล์นั้นโดยตรง','เก็บคำตอบข้อ 1 ไว้ใช้เขียน Merge Request ในบทถัดไป','นำผล review นี้ไปอ้างใน MR และ handoff บทที่ 16'],en:['Compare answer 2 against `git diff --stat` and check the file counts agree','If it answers “none” to point 3 while you can see an odd file, ask about that file by name','Keep answer 1 — you will reuse it when writing the Merge Request','Use this review result in the MR and lesson 16 handoff']}},
        {type:'commands',title:{th:'Quality gate 3 ชั้น',en:'The three quality gates'},
          ...
          steps:[
            ...
            {title:{th:'รันชุดทดสอบอัตโนมัติ',en:'Run the automated tests'},what:{th:'`test` รัน test ทั้งหมดรวมถึง interaction test ที่เราให้ Agent สร้างในบทที่ 10 นี่คือจุดที่จะจับได้ว่าการแก้ครั้งล่าสุดไปทำให้ของเดิมพังหรือไม่',en:'`test` runs everything, including the interaction tests the agent wrote in lesson 10. This is where you catch the latest change breaking something that used to work.'},cmd:`npm run test`,expect:{th:'ทุก test ผ่าน ถ้ามี test ที่ fail ห้ามให้ Agent ลบ test ทิ้งเพื่อให้ผ่าน',en:'All tests pass. If one fails, never let the agent delete the test to make it pass.'}},
    ...
    {
      id:'delivery', group:'day3', no:'15', duration:'45 min',
      title:{th:'Commit, Push & Merge Request',en:'Commit, Push & Merge Request'},
      intro:{th:'ส่ง prototype ให้ Developer review ด้วย MR ที่บอก intent, scope, states, evidence, mock boundary และวิธีเปิดงานได้ชัด',en:'Deliver the prototype for developer review with an MR that explains intent, scope, states, evidence, mock boundaries, and how to run it.'},
      ...
      blocks:[
        ...
        {type:'commands',title:{th:'ส่งงานทีละขั้น',en:'Deliver the work, step by step'},
          ...
          steps:[
            ...
            {title:{th:'ตรวจซ้ำหลัง sync',en:'Recheck after syncing'},what:{th:'การ merge main อาจเปลี่ยนผลการทำงาน แม้ก่อนหน้านี้ผ่านแล้ว จึงรัน quality gate และเดิน happy path อีกครั้งบน revision ที่จะ push จริง',en:'Merging main can change behavior even if earlier checks passed. Re-run the gates and happy path on the revision you will actually push.'},cmd:`npm run lint
npm run test
npm run build`,expect:{th:'ทั้งสามคำสั่งผ่าน และคุณเดิน happy path หลัง merge แล้ว',en:'All three pass and you have walked the happy path after the merge.'}},
            {title:{th:'ส่งขึ้น server',en:'Push to the server'},what:{th:'`push` ส่ง commit ในเครื่องขึ้นไปบน remote ส่วน `-u origin` เป็นการผูก branch ในเครื่องเข้ากับ branch บน server ใช้ครั้งแรกครั้งเดียว ครั้งต่อไปพิมพ์แค่ `git push` ก็พอ',en:'`push` sends your local commits to the remote. `-u origin` links your local branch to the server branch; you need it only the first time, after which plain `git push` is enough.'},cmd:`git push -u origin feature/12-us001-patient-checkin`,expect:{th:'ข้อความสำเร็จ พร้อมลิงก์สำหรับสร้าง Merge Request',en:'A success message with a link to create the Merge Request.'}}
          ],
...
## Handoff
คำสั่งเปิด app/Storybook, AC → หลักฐาน, mock contract และคำถามที่ยังค้าง
...
## Handoff
How to run app/Storybook, AC → evidence, mock contract, and open decisions
...
Keep it short and never include anything that was not actually done.`},
        ...
        {type:'code',title:{th:'ตัวอย่าง MR พร้อมเปิดจริง',en:'A ready-to-open example MR'},
          lead:{th:'ถ้าไม่อยากใช้ Agent ร่างให้ เขียนเองตามตัวอย่างนี้ก็ได้ เติมเฉพาะสิ่งที่ทำจริง',en:'If you would rather not have the agent draft it, write it yourself following this example — with only what you actually did.'},
          label:'MR · US-001',code:`## What\nImplement OPD Patient Check-in (US-001).\n\nCloses #12\n\n## Implemented\n- Patient search and selection\n- Loading / Empty / Error states\n- Clinic validation\n- Preview / Back / Confirm / Success\n\n## Verification\n- Storybook reviewed (5 states)\n- Interaction test passes\n- Manual flow walkthrough: happy path, validation, empty, error\n- Keyboard and narrow viewport checked\n- npm run lint / npm run test / npm run build\n- git diff reviewed\n\n## Handoff\n- Run: npm ci, npm run dev, npm run storybook\n- AC → evidence: see linked story/test/manual step for each AC\n- Mock source: src/mocks/patients.ts, src/services/patient-service.ts\n- Open decisions: list owner and next issue, if any\n\n## Out of scope\n- Real backend integration, real queue, authentication\n\n## Data\nSynthetic/mock data only. No production data.`,
        ...
        {type:'code',title:{th:'แบบฟอร์ม MR สำหรับใช้ซ้ำ',en:'Reusable MR form'},
          lead:{th:'ใช้แบบฟอร์มเปล่านี้เป็นตัวอย่างร่าง MR รอบปัจจุบัน หากทีมต้องการใช้ซ้ำ จะได้เก็บเป็นไฟล์ template ของทีมพร้อม Agent Skill ในบทเสริมท้ายคอร์ส',en:'Use this blank form for the current MR. If your team wants to reuse it, the bonus lesson at the end of the course saves it as a team template with an agent skill.'},
          label:'MR template · example',code:{
th:`## What
<ผลลัพธ์ต่อผู้ใช้ในหนึ่งบรรทัด>
...
## Handoff
- <วิธีเปิด app/Storybook และ AC → หลักฐาน>
- <mock contract และคำถามค้างพร้อมเจ้าของ>
...
## Verification
- <Only checks you actually ran>

## Handoff
- <How to run app/Storybook and AC → evidence>
- <Mock contract and open decisions with owners>

## Out of scope
- <What you deliberately left out this round>

## Data
Synthetic/mock data only. No production data.`},
          note:{th:'ทุกหัวข้อคือคำสัญญากับ reviewer จึงเติมเฉพาะสิ่งที่ทำและตรวจจริง — ห้ามเติมเพื่อให้ดูครบ',en:'Every heading is a promise to the reviewer, so fill in only what was actually built and checked — never fill a heading just to look complete.'}},
        ...
        {type:'commands',title:{th:'ย้ายไปใช้ GitLab ของบริษัท',en:'Point the work at your company GitLab'},
          ...
          steps:[
            ...
            {title:{th:'ส่ง feature branch ขึ้น GitLab',en:'Push the feature branch to GitLab'},what:{th:'ส่งเฉพาะ feature branch ของคุณ ไม่ต้องส่ง main ขึ้นไปทับ หลัง push สำเร็จ GitLab จะให้ลิงก์สำหรับเปิด Merge Request มาในผลลัพธ์',en:'Push only your feature branch; do not push main over anything. After a successful push, GitLab returns a link for opening the Merge Request.'},cmd:`git push -u origin feature/12-us001-patient-checkin`,expect:{th:'สำเร็จ และได้ลิงก์สร้าง Merge Request ใน GitLab',en:'Success, plus a link to create the Merge Request in GitLab.'}}
          ]},
        {type:'practice',title:{th:'ลงมือทำ: Draft MR',en:'Practice: draft an MR'},steps:{th:['ตรวจให้แน่ใจว่า quality gate ในบทที่ 14 ผ่านครบก่อน','เก็บงานค้างให้จบ แล้ว `git fetch` + `git merge origin/main`','push feature branch ด้วย `-u` เป็นครั้งแรก','ใช้ Prompt ร่าง MR แล้วแก้ให้ตรงกับสิ่งที่ทดสอบจริง','ใส่ `Closes #<เลข Issue ของคุณ>` ใน MR','สร้าง Draft MR แล้วให้เพื่อนอ่านและตอบว่าเข้าใจ scope หรือไม่','รับ comment อย่างน้อย 1 ข้อจากเพื่อน แล้วแก้ commit push ซ้ำให้ครบรอบ — ขั้นนี้ต้องมีคนที่สอง นัดเพื่อนหรือ Developer เป็น reviewer ไว้ล่วงหน้า','ใช้แบบฟอร์มด้านบนเป็นตัวอย่าง โดยไม่สร้างไฟล์ใหม่บน main'],en:['Make sure the lesson 14 quality gates pass first','Finish any leftover work, then `git fetch` + `git merge origin/main`','Push the feature branch with `-u` for the first time','Use the MR draft prompt and edit it to match what you actually tested','Add `Closes #<your issue number>` to the MR','Open a Draft MR and ask a peer whether the scope is clear','Take at least one comment from that peer and complete a fix → commit → push round — this step needs a second person, so arrange a reviewer in advance','Use the blank form as a reference without creating a new file on main']},expected:{th:'มี Draft MR ที่ Developer สามารถเริ่ม review ได้โดยไม่ต้องถามว่า “ทำอะไรมา?” โดยไม่มีไฟล์ template ค้างบน main',en:'A Draft MR that a developer can start reviewing without first asking “What did you change?”, with no untracked template on main.'}}
      ],
    ...
    },
    {
      ...
      blocks:[
        {type:'prose',title:{th:'หน้าเว็บที่กดได้ยังไม่พอ',en:'A clickable page is not enough'},body:{th:['Developer ที่ไม่ได้เข้าห้องเรียนต้องรู้ว่า AC ไหนผ่าน, mock ไหนแทนระบบจริง, state ไหนตรวจแล้ว และคำถามใดต้องให้ PM/BA/Design ตอบ ใช้ Issue, Storybook, test และ MR ที่มีอยู่เป็นหลักฐาน ไม่สร้างเอกสารยาวใหม่','หากพฤติกรรมใหม่ยังไม่ตัดสินใจ ให้เปิด Issue เล็กถัดไปและรักษา prototype ที่ผ่านแล้วไว้เป็นฐาน'],en:['A developer outside the class needs to know which ACs passed, which mocks stand in for real systems, which states were checked, and which decisions PM/BA/Design still own. Point to the existing issue, Storybook, tests, and MR.','For undecided new behavior, file a small follow-up issue and keep the verified prototype as the base.']}},
        {type:'diagram',title:{th:'ชุดหลักฐานส่งต่อ',en:'The handoff evidence pack'},diagram:`flowchart LR
I[Issue + AC] --> H[Handoff]
S[Storybook] --> H
A[Running app] --> H
T[Checks + MR diff] --> H
H --> D[Developer continues]`,notes:{th:['ทุกจุดต้องเปิดซ้ำได้','ระบุ mock และช่องว่างตรง ๆ'],en:['Every item must be repeatable','Name mocks and gaps honestly']}},
        {type:'two',title:{th:'ทำแล้ว กับ ยังไม่ทำ',en:'Built versus open'},left:{title:{th:'ยืนยันด้วยหลักฐาน',en:'Evidence-backed'},items:{th:['AC → story/interaction/manual step','คำสั่งเปิด app และ Storybook','ผล lint/test/build บน revision ที่ส่ง'],en:['AC → story/interaction/manual step','App and Storybook run commands','Lint/test/build on the delivered revision']}},right:{title:{th:'ติดป้ายข้อจำกัด',en:'Label the limits'},items:{th:['ไม่มี backend, real queue หรือ auth','visual reference ไม่ใช่ behavior contract','local checks ไม่ใช่ production proof'],en:['No backend, real queue, or auth','Visual reference is not the behavior contract','Local checks are not production proof']}}},
        {type:'code',title:{th:'แบบฟอร์มส่งต่อแบบสั้น',en:'Short handoff template'},label:'handoff',code:{th:`US-001 / Issue #12 / MR <ลิงก์จริง>
เปิดงาน: npm ci → npm run dev → /opd/check-in
ตรวจ state: npm run storybook → <ชื่อ story>
AC → หลักฐาน: <ข้อ> → <story/test/ขั้นตอนจริง>
Mock: src/mocks/patients.ts, src/services/patient-service.ts
ตรวจแล้ว: <lint/test/build + revision>
ยังไม่ทำ: backend, real queue, auth
คำถามค้าง: <คำถาม + เจ้าของคำตอบ>`,en:`US-001 / Issue #12 / MR <real link>
Run: npm ci → npm run dev → /opd/check-in
Review states: npm run storybook → <story names>
AC → evidence: <item> → <story/test/manual step>
Mocks: src/mocks/patients.ts, src/services/patient-service.ts
Verified: <lint/test/build + revision>
Not built: backend, real queue, auth
Open decisions: <question + owner>`},note:{th:'แทน #12 ด้วยเลข Issue จริง และเขียนเฉพาะสิ่งที่ตรวจแล้ว',en:'Replace #12 with your issue number and list only checks that ran.'}},
        {type:'prompt',title:{th:'Prompt: ร่าง Handoff จากหลักฐาน',en:'Prompt: draft an evidence-backed handoff'},when:{th:'ใช้หลัง MR จากบทที่ 15 โดยคนตรวจทุกคำอ้างก่อนส่งต่อ',en:'Use after the lesson 15 MR; a human checks every claim before handoff.'},prompt:{th:`อ่าน AGENTS.md, US-001, Issue, MR/diff และ mock files ห้ามแก้ไฟล์หรือโพสต์ข้อความ
ร่าง handoff ตามแบบด้านบน: AC → หลักฐาน, คำสั่งรัน, mock contract, ผล check ที่มีจริง, ข้อจำกัดและคำถามค้าง
ถ้าหาหลักฐานไม่พบ เขียนว่า ยังไม่มีหลักฐาน ห้ามเดา`,en:`Read AGENTS.md, US-001, the issue, MR/diff, and mock files. Do not edit files or post anything.
Draft the handoff above: AC → evidence, run commands, mock contract, actual check results, limits, and open questions.
If evidence is missing, say no evidence yet. Never guess.`},after:{th:['เปิดหลักฐานทุกชิ้นตามที่ร่างไว้','ให้ Developer ลองเปิดงานโดยไม่อธิบายปากเปล่า'],en:['Open every cited piece of evidence','Let a developer run the work without verbal help']}},
        {type:'callout',title:{th:'ทำซ้ำด้วย Agent Skill ได้ภายหลัง',en:'An agent skill can repeat this later'},text:{th:'หลังทำ handoff ด้วยมือหนึ่งครั้ง จึงค่อยเก็บแบบฟอร์มเป็น template หรือ SKILL.md สำหรับรวบรวมหลักฐานซ้ำ คำอธิบาย skill ต้องสั้นและบอกว่าใช้เมื่อไร ตรวจว่า Agent ที่ใช้โหลด skill จาก path ใดจริง อย่าสมมติว่าทุกเครื่องมืออ่าน .agents/skills อัตโนมัติ Skill ช่วยร่าง; คนอนุมัติก่อนส่งออกนอกเครื่อง — บทเสริมท้ายคอร์สพาสร้าง skill สำหรับงานซ้ำแบบลงมือจริง',en:'After one handoff by hand, save the form as a template or SKILL.md for repeated evidence gathering. Give the skill a short description saying when it applies. Verify the skill path your agent actually supports; do not assume every tool auto-loads .agents/skills. A skill drafts; a human approves before sending anything off the machine — the bonus lesson at the end of the course walks through building one.'}},
        {type:'practice',title:{th:'ลงมือทำ: ให้คนอื่นเปิดงาน',en:'Practice: let someone else run it'},steps:{th:['ตรวจ Issue → branch → commit footer → MR Closes ให้เป็นเลขเดียวกัน','เติม AC → หลักฐานตามแบบฟอร์ม','ให้ Developer เปิด app และ Storybook จากคำสั่งที่ให้','จดทุกเรื่องที่เขายังต้องเดาและระบุเจ้าของคำตอบ'],en:['Match the issue number across branch, commit footers, and MR Closes','Fill AC → evidence in the template','Let a developer run app and Storybook from the written steps','Record every guess they would need and assign a decision owner']},expected:{th:'Developer เปิด prototype ได้เอง เห็นข้อจำกัด mock และรู้ว่าต้องถามใคร โดยไม่ต้องสร้าง US-001 ใหม่',en:'A developer can run the prototype, see mock boundaries, and find decision owners without rebuilding US-001.'}}
      ],
      quiz:{q:{th:'Handoff ใดช่วยลดงานรื้อทำใหม่?',en:'Which handoff reduces rework?'},options:{th:['ภาพหน้าจออย่างเดียว','Issue/AC ที่โยงกับ app, Storybook, checks และ MR','Agent บอกว่าเสร็จแล้ว'],en:['A screenshot alone','Issue/AC linked to app, Storybook, checks, and MR','The agent saying it is done']},answer:1,why:{th:'หลักฐานที่เปิดซ้ำได้บอกทั้ง intent, behavior และขอบเขต',en:'Repeatable evidence shows intent, behavior, and scope.'}},
      wrap:{th:['ส่งต่อหลักฐานที่เปิดซ้ำได้','แยก mock กับระบบจริง','คำถามที่ยังค้างต้องมีเจ้าของ'],en:['Hand off repeatable evidence','Separate mocks from real systems','Give open decisions an owner']}
    },
    {
      id:'capstone', group:'capstone', no:'17', duration:'180 min', kind:'capstone',
      title:{th:'Capstone · เพิ่มกรณีคลินิกไม่พร้อมรับ Check-in',en:'Capstone · Handle an Unavailable Clinic'},
      intro:{th:'รับโจทย์ใหม่ขนาดเล็กใน prototype US-001 ที่ merge แล้ว: คลินิกบางแห่งไม่พร้อมรับ check-in ผู้เรียนเขียน Issue ใหม่และกำกับ AI ทำงานตั้งแต่แผนถึง MR โดยรักษา flow เดิมให้ผ่าน',en:'Take a small follow-up on the merged US-001 prototype: a clinic may be unavailable. Write a new issue and guide AI from plan to MR while preserving the original flow.'},
      outcomes:{th:['แปลง brief ใหม่เป็น Issue และ AC ที่ตรวจได้','ใช้ branch/worktree ใหม่จาก main ที่มี US-001 แล้ว','เพิ่ม mock และ UI states โดยไม่รื้อ flow เดิม','พิสูจน์ behavior ใหม่และ regression ของ US-001','ส่ง MR และ handoff ที่ Developer ทำต่อได้'],en:['Turn a new brief into a checkable issue and ACs','Use a new branch/worktree from main containing US-001','Add mocks and UI states without rebuilding the flow','Prove the new behavior and US-001 regression','Deliver an MR and handoff a developer can continue']},
      blocks:[
        {type:'prose',title:{th:'โจทย์ใหม่บนฐานเดิม',en:'A new task on the existing base'},body:{th:[
          'บท 00–16 สร้างและส่ง US-001 แล้ว Capstone นี้เริ่มหลัง MR เดิมถูก reviewer merge เท่านั้น ใช้ repo เดิมและ flow เดิม แต่เปิด Issue **ใหม่** สำหรับปัญหาคลินิกไม่พร้อมรับ check-in',
          'สิ่งที่วัดคือการตัดสินใจของ PM/BA/Design: เขียน AC, ระบุ state และข้อความ, ตรวจแผนของ Agent, ดูงานจริงและผล regression แล้วส่งหลักฐานให้ Developer โดยไม่เขียนโค้ดเอง'
        ],en:[
          'Lessons 00–16 built and handed off US-001. Start this capstone only after the original MR has been merged by its reviewer. Use the same repository and flow, but open a **new issue** for clinic unavailability.',
          'The assessment is the PM/BA/Design decisions: write ACs, specify states and copy, review the agent plan, inspect the running change and regression evidence, and hand it to a developer without coding personally.'
        ]}},
        {type:'callout',tone:'warning',title:{th:'เงื่อนไขเริ่ม Capstone',en:'Capstone entry gate'},text:{th:'นัด review/merge งาน US-001 ให้เสร็จก่อนเริ่มช่วง Capstone แยกต่างหาก หาก MR เดิมยังไม่ผ่าน ให้แก้ตาม review ในงานเดิมก่อน แล้วจึงดึง main ล่าสุดมาสร้าง branch ใหม่',en:'Schedule the US-001 review and merge before a separate capstone session. If the original MR is still pending, address that review first; then pull the updated main and create the new branch.'}},
        {type:'code',title:{th:'Brief ที่ต้องแปลงเป็น Issue',en:'Brief to turn into an issue'},label:'Capstone brief · clinic availability',code:{
th:`ฐานงาน: US-001 ที่ merge แล้ว (Search → Select → Clinic → Preview → Confirm)
ปัญหา: บางคลินิกหยุดรับ check-in ชั่วคราว; เจ้าหน้าที่ต้องรู้ก่อน Preview
Mock ที่กำหนด:
- GEN / เวชกรรมทั่วไป = พร้อมรับ
- ENT / หู คอ จมูก = ไม่พร้อมรับ; อธิบายว่าไม่รับ check-in ขณะนี้
- Availability lookup แบบ Error = ตรวจสถานะไม่ได้; มี Retry
ผลที่ต้องได้:
- ENT หรือ Error ไป Preview/ออกเลขคิวไม่ได้
- จาก ENT เปลี่ยนเป็น GEN แล้วไปต่อได้; จาก Error ต้อง Retry สำเร็จก่อน
- ผู้ป่วยที่เลือกและ chief complaint ไม่หายเมื่อเปลี่ยนคลินิก
ขอบเขต: mock เท่านั้น; ไม่มี backend, คิวจริง, auth หรือปุ่มสลับ scenario บนหน้าจอผู้ใช้
หลักฐาน: Storybook states, interaction test, เดิน app, keyboard/mobile,
         lint/test/build และ regression ของ AC เดิม`,
en:`Base: merged US-001 (Search → Select → Clinic → Preview → Confirm)
Problem: a clinic may stop accepting check-ins; staff need to know before Preview
Fixed mocks:
- GEN / General Medicine = available
- ENT / Ear, Nose and Throat = unavailable; explain why
- Availability lookup Error = status unknown; offer Retry
Required result:
- ENT or Error cannot reach Preview or issue a queue number
- After ENT, switching to GEN continues; after Error, only a successful Retry unblocks the flow
- Selected patient and optional chief complaint are preserved
Boundary: mocks only; no backend, real queue, auth, or end-user scenario switcher
Evidence: Storybook states, interaction test, running app, keyboard/mobile,
...
P --> N["Clinic availability change"]
N --> R["New evidence + US-001 regression"]
R --> M["New MR + handoff"]`,notes:{th:['งานใหม่มี Issue และ MR ของตัวเอง แต่ใช้ prototype เดิมเป็นฐาน','ผู้เรียนออกแบบ state และเส้นทางแก้ไขจาก brief เอง','หลักฐาน regression ยืนยันว่าเพิ่มความสามารถโดยไม่ทำของเดิมเสีย'],en:['The follow-up has its own issue and MR on the existing prototype','Learners design states and recovery paths from the brief','Regression evidence shows the change preserves original behavior']}},
        {type:'two',title:{th:'งานใหม่ กับ regression',en:'New work and regression'},left:{title:{th:'สิ่งที่ต้องสร้าง',en:'Build this'},items:{th:['Issue ใหม่พร้อม AC และเลขของตัวเอง','mock GEN, ENT และ Error ที่ทำซ้ำได้','UI state, ข้อความและทางไปต่อที่ใช้ keyboard ได้','Storybook + interaction test อย่างน้อยหนึ่งเส้นทาง'],en:['A new issue with its own ACs and number','Repeatable GEN, ENT, and Error mocks','Keyboard-usable UI states, messages, and recovery','Storybook plus at least one interaction test']}},right:{title:{th:'สิ่งที่ต้องรักษา',en:'Keep working'},items:{th:['US-001 AC 1–14 โดยเฉพาะ Search/Select/Preview/Confirm','ข้อมูลผู้ป่วยและ complaint เมื่อเปลี่ยนคลินิก','mock/synthetic boundary และ diff ที่อยู่ใน scope','เลข Issue ใหม่โยง branch → commit Refs → MR Closes'],en:['US-001 ACs 1–14, especially Search/Select/Preview/Confirm','Selected patient and complaint when clinic changes','Mock/synthetic boundary and a scoped diff','New issue number across branch → commit Refs → MR Closes']}}},
        {type:'list',title:{th:'เกณฑ์ผ่าน Capstone',en:'Capstone assessment'},items:{th:['PM: Issue ใหม่กำหนดปัญหา scope และข้อจำกัด mock ชัด พร้อมลิงก์ US-001','BA: AC แยก state/behavior และตรวจ ENT, GEN, Error/Retry ได้จริง','Product Design: ข้อความ การแก้ทางตัน keyboard/focus และจอแคบผ่านการดูของจริง','Dev literacy: plan/diff อยู่ใน scope, check ผ่าน และมี regression evidence','Delivery: branch/commit/MR ใช้เลข Issue ใหม่ตรงกัน พร้อม handoff ที่คนอื่นเปิดตามได้'],en:['PM: the new issue frames the problem, scope, mock boundary, and original US-001 link','BA: state/behavior ACs make ENT, GEN, and Error/Retry checkable','Product Design: messages, recovery, keyboard/focus, and narrow width pass human review','Dev literacy: plan/diff stay scoped, checks pass, and regression is evidenced','Delivery: branch/commit/MR share the new issue number and another person can follow the handoff']}},
        {type:'capstone',steps:[
          {title:{th:'1 · เขียน Issue ใหม่จาก brief',en:'1 · Write a new issue from the brief'},hint:{th:'แยก state กับ behavior และระบุว่าอะไรต้องห้ามไป Preview',en:'Separate states from behavior and define what must block Preview'},guide:{th:['อ่าน brief ใน starter และ Issue/MR US-001 เดิม','เขียน context, in/out of scope, AC สำหรับ GEN/ENT/Error/Retry และ regression','เปิด Issue ใหม่ใน repo ของตนเอง; ลิงก์ Issue/MR US-001 เป็นฐานงาน','จดเลข Issue ใหม่เพื่อใช้ใน branch, commit footer และ MR'],en:['Read the starter brief and original US-001 issue/MR','Write context, scope, ACs for GEN/ENT/Error/Retry, and regression','File a new issue in your repository; link the original US-001 issue/MR as the base','Record the new issue number for branch, commit footer, and MR']}},
          {title:{th:'2 · แยกงานและตรวจแผน Agent',en:'2 · Isolate work and review the agent plan'},hint:{th:'เริ่มจาก main หลัง merge แล้วให้ Agent อธิบายไฟล์ที่ต้องแตะ',en:'Start from main after merge and ask the agent to name the files it needs'},guide:{th:['ยืนยัน main มี US-001 ที่ merge แล้วและ working tree สะอาด','สร้าง branch/worktree ใหม่ เช่น feature/34-clinic-availability; 34 เป็นเลขตัวอย่าง','ให้ Agent อ่าน AGENTS.md, brief, Issue ใหม่ และ implementation เดิมแบบ Explore ก่อน','ตรวจ plan ว่าเปลี่ยนเฉพาะ clinic choice/mock/stories/tests; อนุมัติก่อนลงมือ'],en:['Confirm main contains merged US-001 and the working tree is clean','Create a new branch/worktree, e.g. feature/34-clinic-availability; 34 is an example','Have the agent read AGENTS.md, the brief, new issue, and existing implementation in Explore mode','Review that the plan touches only clinic choice/mocks/stories/tests; approve before edits']}},
          {title:{th:'3 · สร้างและ review พฤติกรรมใหม่',en:'3 · Build and review the new behavior'},hint:{th:'เปิด Storybook ดูสามสถานะก่อนเดิน flow จริง',en:'Review three Storybook states before walking the actual flow'},guide:{th:['ให้ Agent ลงมือเป็นช่วงเล็ก: mock → state/story → interaction → integration','เปิด Storybook ตรวจ GEN, ENT และ Error รวมข้อความ, focus และ Retry','ใน app ลอง ENT ต้องไปต่อไม่ได้ แล้วเปลี่ยน GEN ต้องถึง Preview/Success โดยข้อมูลเดิมไม่หาย; Error/Retry ดูใน Storybook/test','review diff แล้ว commit เฉพาะ checkpoint ที่คนตรวจผ่าน พร้อม Refs #<เลขใหม่>'],en:['Have the agent work in small slices: mock → state/story → interaction → integration','Review GEN, ENT, Error, messages, focus, and Retry in Storybook','In the app, ENT must block progress and GEN must reach Preview/Success without losing values; review Error/Retry in Storybook/test','Review the diff and commit only verified checkpoints with Refs #<new number>']}},
          {title:{th:'4 · พิสูจน์งานใหม่และของเดิม',en:'4 · Prove new behavior and regression'},hint:{th:'AC ใหม่ผ่านอย่างเดียวไม่พอ; Search และ Confirm เดิมต้องยังผ่าน',en:'New ACs passing is insufficient; original Search and Confirm must still pass'},guide:{th:['รัน lint/test/build และ interaction test บน revision ที่จะส่ง','เดิน US-001 happy path, HN/ชื่อซ้ำ, Loading/Empty/Error, Preview/Back และ Success','ตรวจ keyboard และจอแคบทั้งสถานะใหม่กับ flow เดิม','อ่าน git diff ว่าไม่มี backend, real data, build output หรือไฟล์นอกงาน'],en:['Run lint/test/build and interaction tests on the revision to deliver','Walk US-001 happy path, HN/multiple matches, Loading/Empty/Error, Preview/Back, Success','Check keyboard and narrow width for both new states and original flow','Read git diff for backend, real data, build output, or unrelated files']}},
          {title:{th:'5 · เปิด MR และส่งต่อ',en:'5 · Open the MR and hand off'},hint:{th:'MR ต้อง Closes Issue ใหม่ และลิงก์ US-001 เป็นฐาน',en:'The MR closes the new issue and links US-001 as its base'},guide:{th:['sync origin/main แล้วรัน checks ซ้ำถ้ามีการเปลี่ยน','ให้ Agent ร่าง MR/handoff จากหลักฐาน; คนตรวจแล้วจึง push และเปิด Draft MR','ใส่ Closes #<เลข Issue ใหม่>, ลิงก์ MR เดิม, AC → evidence, mock boundary และคำถามค้าง','รับ review, แก้, commit, push; reviewer ตัดสินใจ merge แล้ว cleanup หลัง merge'],en:['Sync origin/main and rerun checks if the revision changes','Let the agent draft MR/handoff from evidence; a human reviews before push and Draft MR','Include Closes #<new issue>, the original MR link, AC → evidence, mock boundary, and open questions','Address review, commit, push; the reviewer decides on merge, then clean up']}}
        ]},
        {type:'prompt',title:{th:'Prompt สำรอง: ขอแผนเมื่อเริ่มไม่ถูก',en:'Fallback prompt: ask for a plan'},when:{th:'ใช้หลังร่าง Issue เองแล้ว แต่ยังไม่แน่ใจว่า extension นี้แตะส่วนใดของโค้ด',en:'Use after drafting your own issue if you are unsure which existing parts the extension touches.'},prompt:{th:`อ่าน AGENTS.md, docs/requirements/US-001-opd-checkin-th.md,
docs/capstone/clinic-availability.md, Issue ใหม่ และ implementation ปัจจุบัน
ห้ามแก้ไฟล์, commit, push หรือเปิด MR
บอก 1) behavior เดิมที่ต้องรักษา 2) ไฟล์/Storybook state ที่ควรเปลี่ยน
3) mock/interaction/manual checks สำหรับ ENT, GEN, Error/Retry
4) จุดที่ Issue ยังต้องให้ PM/BA/Design ตัดสินใจ
สรุปเป็นแผนสั้น ๆ พร้อมความเสี่ยง regression แล้วรอฉัน review`,en:`Read AGENTS.md, docs/requirements/US-001-opd-checkin.md,
docs/capstone/clinic-availability.md, the new issue, and the current implementation.
Do not edit files, commit, push, or open an MR.
List 1) original behavior to preserve, 2) files/Storybook states to change,
3) mocks, interaction, and manual checks for ENT, GEN, Error/Retry,
4) decisions the issue still needs from PM/BA/Design.
Return a short plan with regression risks and wait for my review.`},after:{th:['เทียบแผนกับ Issue ใหม่และ brief จริง','ถ้า Agent จะสร้าง Search/Confirm ใหม่ทั้งชุด ให้ลด scope กลับมาที่ clinic choice'],en:['Compare the plan with the real issue and brief','If the agent proposes rebuilding Search/Confirm, narrow it back to clinic choice']}},
        {type:'practice',title:{th:'ส่งงาน Capstone',en:'Deliver the capstone'},steps:{th:['เปิด Issue ใหม่ที่ลิงก์กลับ US-001 และมี AC ที่ตรวจได้','ทำงานบน branch/worktree ใหม่โดย Agent เสนอแผนก่อน','แสดง GEN/ENT/Error ใน Storybook และเดิน app ทั้ง blocked/recovery path','บันทึกหลักฐาน regression ของ US-001, keyboard/mobile และ lint/test/build','เปิด Draft MR ที่ Closes Issue ใหม่และส่ง handoff ให้ Developer'],en:['File a new issue linked to US-001 with checkable ACs','Work on a new branch/worktree after agent planning','Show GEN/ENT/Error in Storybook and walk blocked/recovery paths in the app','Record US-001 regression, keyboard/mobile, and lint/test/build evidence','Open a Draft MR that closes the new issue and hand it to a developer']},expected:{th:'มี follow-up MR ของปัญหาคลินิกไม่พร้อมรับที่ Developer ตรวจต่อได้ โดย US-001 เดิมยังทำงานครบและไม่มีการเชื่อมระบบจริง',en:'A reviewable follow-up MR for clinic unavailability, with original US-001 behavior intact and no real-system integration.'}}
      ],
      quiz:{q:{th:'อะไรทำให้ Capstone นี้ต่างจากการตรวจรับ US-001 ซ้ำ?',en:'What makes this capstone different from rechecking US-001?'},options:{th:['รัน lint/test/build เดิมอีกครั้งอย่างเดียว','เปิด Issue ใหม่เพื่อเพิ่ม unavailable/Error/Retry แล้วพิสูจน์ว่า flow เดิมยังผ่าน','สร้างหน้า Search และ Confirm ใหม่ทั้งหมด'],en:['Only rerun the old lint/test/build checks','File a new issue for unavailable/Error/Retry and prove the original flow still works','Rebuild the whole Search and Confirm flow']},answer:1,why:{th:'ผู้เรียนประยุกต์วงจรเดิมกับ behavior ใหม่บนฐานที่ผ่านแล้ว พร้อม regression evidence',en:'Learners apply the full loop to new behavior on a verified base, with regression evidence.'}},
      wrap:{th:['Capstone เพิ่ม behavior ใหม่ใน scenario เดิม','Issue และ MR ใหม่มี trace ของตัวเองและลิงก์งานเดิม','Developer ทำต่อจาก prototype ที่ตรวจแล้ว ไม่ต้องรื้อสร้างใหม่'],en:['The capstone adds new behavior within the same scenario','The new issue and MR have their own trace and link the original work','Developers continue from a verified prototype rather than rebuilding it']}
    },
    {
      id:'agent-skills', group:'extra', no:'18', duration:'60 min',
      title:{th:'ทำให้ทำซ้ำได้: Skill สำหรับสร้าง Issue และ MR',en:'Make It Repeatable: Skills for Issues and MRs'},
      intro:{th:'บทเสริมหลังจบ Capstone: คุณเขียน Issue เองมาแล้วสองรอบ (บทที่ 02 และ Capstone) และร่าง MR เองมาแล้วสองรอบ (บทที่ 15 และ Capstone) รอบนี้เก็บแบบฟอร์มที่ใช้ประจำเป็นไฟล์ template ของทีม แล้วห่อเป็น skill ให้ Agent ทำซ้ำได้ในมาตรฐานเดียวกันทุกครั้ง — พร้อม Diff Review Prompt จากบทที่ 14 ที่จะกลายเป็น skill ตัวที่สามไว้ตรวจทุก branch',en:'A bonus lesson for after the capstone: you have written issues by hand twice (lesson 02 and the capstone) and drafted MRs by hand twice (lesson 15 and the capstone). Now save the forms you keep reusing as team template files and wrap them in skills, so the agent repeats them to the same standard every time — joined by the Diff Review Prompt from lesson 14, which becomes a third skill for reviewing any branch.'},
      outcomes:{th:['บอกได้ว่างานแบบไหนควรทำเป็น skill และแบบไหนไม่ควร','เก็บแบบฟอร์มเปล่า Issue และ MR เป็นไฟล์ template และ commit เข้า Git เป็นมาตรฐานของทีม','สร้าง skill สำหรับสร้าง Issue ร่าง MR และตรวจ diff แล้วเชื่อมเข้ากับเครื่องมือที่ใช้','ตั้งเส้นความปลอดภัย: Agent ร่างและเสนอคำสั่ง คนเป็นผู้อนุมัติ'],en:['Tell which work belongs in a skill and which does not','Save the blank issue and MR forms as template files committed to Git as a team standard','Build skills for filing issues, drafting MRs, and reviewing diffs, and wire them into the tool you use','Set the safety line: the agent drafts and proposes, a human approves']},
      blocks:[
        {type:'prose',title:{th:'ทำเองก่อน แล้วค่อยทำให้เป็นระบบ',en:'By hand first, automated second'},body:{th:[
          'ลำดับของบทเรียนที่ผ่านมาไม่ใช่เรื่องบังเอิญ เราให้คุณ **เขียน Issue เองในบทที่ 02** **ตรวจ diff ด้วยตัวเองในบทที่ 14** และ **ตรวจกับแก้ร่าง MR เองในบทที่ 15** ก่อน เพราะถ้าคุณไม่เคยเขียนเอง คุณจะไม่มีเกณฑ์ในหัวไว้ตรวจของที่ Agent ร่างมา และจะลงเอยด้วยการกด approve ทุกอย่างที่มันเสนอ',
          ...
          'ยกตัวอย่างให้ชัด การจัดรูปแบบ Issue ให้ครบทุกหัวข้อคือรูปแบบตายตัว ทำเป็น skill ได้ การรายงานตรวจ diff ตามหัวข้อเดิมทุกครั้งก็เช่นกัน แต่การตัดสินว่า “Chief Complaint ควรบังคับกรอกหรือไม่” คือการตัดสินใจทางธุรกิจ ซึ่งไม่ควรอยู่ใน skill และไม่ควรอยู่กับ Agent',
          'สิ่งที่เราจะสร้างในบทนี้จึงเป็น skill ที่ทำงานสามอย่างเท่านั้น คือ **กรอกแบบฟอร์มให้ครบ** **เสนอคำสั่งที่จะยิงออกไป** และ **รายงานผลตรวจ diff เป็นภาษาธุรกิจ** ตัวที่สามคือ Diff Review Prompt จากบทที่ 14 ที่ห่อให้ใช้ซ้ำได้ทุก branch ส่วนการกดยิงและการตัดสินว่างานผ่านหรือไม่ ยังเป็นของคุณ'
        ],en:[
          'The order of the lessons so far was deliberate. You **wrote the issue by hand in lesson 02**, **reviewed the diff yourself in lesson 14**, and **reviewed and edited the MR draft yourself in lesson 15** — because without having done each by hand you would have no standard in your head to judge the agent’s draft against, and would end up approving whatever it proposed.',
          ...
          'Concretely: formatting an issue so every section is present is a fixed shape and makes a fine skill, and so is reporting a diff under the same headings every time. Deciding whether chief complaint should be mandatory is a business decision — it does not belong in a skill and it does not belong to the agent.',
          'So the skills we build here do exactly three things: **fill in the form completely**, **propose the command that would send it**, and **report the diff review in business language** — the third being the lesson 14 Diff Review Prompt wrapped for reuse. Pressing send and calling pass or fail stay with you.'
        ]}},
...
1. Run these read-only commands and treat their output as the source of truth:
   git status
   git log --oneline main..HEAD
   git diff main...HEAD --stat
...
3. Report on these headings, in business language rather than jargon:
   ...
   - Any file unrelated to the task, or changed by accident
   - New dependencies or shared config files modified
   - Changes risky for other parts of the system
   - What you actually tested, and the results
4. Scan git status for files that must never enter a commit: .env or anything
   holding keys or tokens, real patient data, build output such as .next/ or
   storybook-static/, and scratch files left behind. Name any that you find.
5. If the user asks for quality gates, run npm run lint, npm run test, and
   npm run build one at a time and report the raw results.
6. End with your verdict per heading and stop. The human decides pass or fail.
...
- Never commit, push, or open an MR out of this review.`,
        ...
        {type:'callout',tone:'danger',title:{th:'เส้นที่ skill ห้ามข้าม',en:'Lines a skill must not cross'},text:{th:'skill ทำให้ Agent ทำงานที่ “ออกไปนอกเครื่องคุณ” ได้ จึงต้องเขียนข้อห้ามไว้ในไฟล์ให้ชัดทุกครั้ง ได้แก่ ห้าม merge, ห้าม push ขึ้น main, ห้ามเปลี่ยน MR จาก Draft เป็นพร้อม review, ห้ามแตะ Issue หรือ MR ของคนอื่น และห้ามใส่ข้อมูลผู้ป่วยจริงหรือค่าใน `.env` ลงในเนื้อหา — skill ตรวจ diff ไม่มีคำสั่งออกนอกเครื่อง แต่มีกฎเท่ากัน คือห้ามแก้ไฟล์ใด ๆ ระหว่างตรวจ และห้ามรายงานว่าตรวจแล้วทั้งที่ไม่ได้รัน — เก็บโหมด Manual ไว้เหมือนที่ตั้งในบทที่ 00 เพื่อให้ทุกคำสั่งต้องผ่านการกดยืนยันของคุณก่อนเสมอ',en:'A skill lets the agent do things that leave your machine, so spell out the prohibitions in the file every time: never merge, never push to main, never flip an MR from draft to ready, never touch anyone else’s issue or MR, and never place real patient data or `.env` values in the body. The diff-review skill sends nothing out, yet carries the same rule: never touch a file while reviewing, and never report a check that did not run. Keep the Manual mode you set in lesson 00 so every command still stops for your confirmation.'}},
...
Create only this one file, run no commands, and show me the content first.`},
  ...
  final: {
    ...
    intro:{th:'คุณไม่จำเป็นต้องเขียนโค้ดเอง แต่ควรสร้างและส่งต่อ prototype จาก requirement ได้ แล้วประยุกต์วงจรเดิมกับ follow-up Issue ใหม่โดยไม่ทำ flow ที่ผ่านแล้วเสีย',en:'You do not need to write code yourself. You should guide AI from requirement to a handoff-ready prototype, then apply the same loop to a new follow-up issue without breaking the verified flow.'},
    skills:{th:['Requirement → Issue + AC','Design → DESIGN.md → Style Guide','Requirement → Component/State','Git branch + worktree','Agent Explore/Plan','Storybook review','Mock states','Browser/Terminal debugging','git diff + quality gates','Commit checkpoints → MR → Merge','AC → Evidence → Developer handoff','Follow-up Issue → regression evidence'],en:['Requirement → Issue + AC','Design → DESIGN.md → Style Guide','Requirement → Component/State','Git branch + worktree','Agent Explore/Plan','Storybook review','Mock states','Browser/Terminal debugging','git diff + quality gates','Commit checkpoints → MR → Merge','AC → Evidence → Developer handoff','Follow-up issue → regression evidence']}
  },
  glossary: {
    categories: [
      {
      ...
      },
      {
        id:'agent', name:{th:'AI Agent & Prompt',en:'AI Agent & Prompting'}, terms:[
          ...
          {term:'Agent Skill',th:'ไฟล์ SKILL.md สำหรับงานที่ทำซ้ำ เช่น ร่าง handoff จากหลักฐาน ตำแหน่งที่เครื่องมืออ่าน skill ต่างกัน ต้องตรวจจริง Skill ไม่ตัดสิน AC แทนคนหรือส่งงานออกนอกเครื่องเอง บทเสริมท้ายคอร์สสอนสร้างจริง',en:'A SKILL.md for repeatable work such as drafting an evidence-backed handoff. Skill discovery paths vary by tool and must be verified. A skill does not decide ACs for humans or send work off the machine itself. The bonus lesson at the end of the course builds one for real.'},
      ...
      },
      {
        id:'tools', name:{th:'เครื่องมือพื้นฐาน',en:'Essential Tools'}, terms:[
          ...
          {term:'npm',th:'เครื่องมือที่มาพร้อม Node.js ใช้ติดตั้ง library และรันคำสั่งของโปรเจกต์ เช่น `npm ci` และ `npm run dev`',en:'The tool bundled with Node.js. It installs libraries and runs project commands such as `npm ci` and `npm run dev`.'},
          {term:'Dependency',th:'library ของคนอื่นที่โปรเจกต์นำมาใช้ ติดตั้งครั้งแรกด้วย `npm ci` ตามรายการใน `package.json`',en:'Third-party libraries the project builds on, installed with `npm ci` according to the list in `package.json`.'},
      ...
      {
        id:'ui', name:{th:'Next.js, Design System & Storybook',en:'Next.js, Design System & Storybook'}, terms:[
          ...
          {term:'DESIGN.md',th:'ไฟล์เดียวที่สรุประบบภาพลักษณ์ของแบรนด์แบบ semantic คือผูก token (สี/font/spacing) เข้ากับกติกาและเหตุผลไว้ด้วยกัน ให้ AI Agent อ้างอิงซ้ำได้แทนการอธิบายใหม่ทุก prompt',en:'A single file that summarizes a brand’s visual system semantically — tokens (color/type/spacing) tied to rules and the reasoning behind them — so an AI agent can reuse it instead of being told the same thing in every prompt.'},
          {term:'Design Token',th:'ค่าที่ตั้งชื่อตามหน้าที่แทนการเขียนค่าดิบซ้ำ ๆ เช่น `colors.primary` แทนที่จะเขียน `#0F766E` กระจายอยู่หลายไฟล์ ทำให้เปลี่ยนค่าที่เดียวแล้วมีผลทั้งระบบ',en:'A value named by its job instead of repeating a raw value everywhere — `colors.primary` instead of `#0F766E` scattered across files — so changing it in one place updates the whole system.'},
          {term:'Style Guide',alias:'/design',th:'หน้าเว็บที่แสดง design token ทั้งหมด (สี, ตัวอักษร, spacing, ตัวอย่าง component) ให้ human ตรวจได้ด้วยตา สร้างโดยยึด DESIGN.md เป็นแหล่งค่า และต้องตรวจว่าไม่ drift',en:'A page that displays every design token — colors, type, spacing, sample components — for a human to review by eye. It uses DESIGN.md as its value source and must be checked for drift.'},
          ...
          {term:'Prototype',th:'หน้าจอที่กดเดิน flow ได้ด้วย mock data ในคอร์สนี้ พร้อม state และหลักฐานตรวจซ้ำได้ ยังไม่ใช่ระบบ HIS ที่ใช้ข้อมูลจริง',en:'A clickable flow using mock data in this course, with reviewable states and repeatable evidence. It is not a real-data HIS system.'},
          {term:'Visual Reference',th:'ภาพอ้างอิงรูปลักษณ์และลำดับสายตา ไม่ใช่ข้อกำหนดพฤติกรรม ถ้าภาพไม่มี Preview ให้ยึด US-001 สำหรับ flow',en:'An image guiding appearance and visual hierarchy, not behavior. If Preview is absent from the image, US-001 still defines the flow.'},
      ...
      },
      {
        id:'quality', name:{th:'คุณภาพ & การส่งมอบ',en:'Quality & Delivery'}, terms:[
          ...
          {term:'Developer Handoff',th:'ชุดลิงก์ Issue/AC, MR, Storybook, วิธีรัน, ผลตรวจ, mock contract และคำถามค้าง ที่ทำให้คนอื่นเปิดงานและพัฒนาต่อจากฐานเดิมได้',en:'Links to issue/ACs, MR, Storybook, run steps, checks, mock contract, and open decisions so another developer can run and continue the existing work.'},
          {term:'Follow-up Issue',th:'Issue ใหม่สำหรับพฤติกรรมที่เพิ่มบนงานเดิม มีเลข branch, commit และ MR ของตัวเอง และลิงก์กลับ Issue/MR เดิม',en:'A new issue for behavior added to existing work. It has its own branch, commit, and MR trace and links the original issue/MR.'},
          {term:'Regression Check',th:'การตรวจว่าความสามารถเดิมยังทำงานหลังเพิ่มสิ่งใหม่ เช่น เพิ่มสถานะคลินิกแล้ว Search, Preview และ Confirm ของ US-001 ยังผ่าน',en:'Checking that existing behavior still works after a change, such as US-001 Search, Preview, and Confirm after adding clinic availability.'},
          ...
          {term:'Epic',th:'ก้อนงานใหญ่กว่า story หนึ่งอัน ครอบคลุมหลายหน้าจอหลายรอบส่งงาน เช่น “OPD Check-in Lite” ทั้งโครง — Jira มี epic เป็น artifact จริง แต่ GitHub ไม่มีแบบ native และ GitLab จัด Epic ไว้ใน tier Premium ทีมจริงจึงมักใช้ label หรือ milestone จัดกลุ่มแทน คอร์สนี้จึงทำงานที่ระดับ Issue ซึ่งใช้ได้ทุกแพลตฟอร์ม (ส่วน sub-task ที่หั่นงานเล็กกว่า story อยู่นอกขอบเขตของคอร์ส)',en:'A chunk of work larger than one story, spanning several screens and deliveries — the whole “OPD Check-in Lite” effort, for example. Jira has real epic artifacts, but GitHub has no native epic and GitLab puts Epics in its Premium tier, so real teams often group work with labels or milestones instead. This course therefore works at the issue level, which every platform gives you (sub-tasks below a story are out of scope here).'},
          {term:'User Story',th:'วิธีเขียน requirement จากมุมผู้ใช้ในรูป “ในฐานะ… ฉันต้องการ… เพื่อ…” ใช้เล่าเจตนา ในคอร์สนี้เอกสาร requirement เล่นบทบาทนี้ แล้วตัดออกมาเป็น Issue ที่ลงมือทำได้ทันที — ถ้าคุ้น Jira ให้ถือว่า Issue หนึ่งใบของคอร์สนี้คือ story หนึ่งอันที่ตัดมาแล้วพอดี 1 branch',en:'A requirement written from the user’s point of view — “as a…, I want…, so that…”. It carries intent. In this course the requirement document plays that role and is cut into immediately actionable issues — if you come from Jira, one course issue is one story, sized to fit a single branch.'},
