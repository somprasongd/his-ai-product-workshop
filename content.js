    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'prose',title:{th:'commit เมื่อไร และข้อความควรเขียนอย่างไร',en:'When to commit, and what the message should say'},body:{th:[
          ...
          'ตลอดคอร์สนี้จุดตรวจมีรูปแบบซ้ำ ๆ อยู่สี่แบบ คือ component หนึ่งตัวกับ story ของมันผ่านสายตาคุณใน Storybook, interaction test ผ่าน, หน้าเว็บเดิน flow ได้ครบ และบั๊กหนึ่งตัวถูกแก้จบพร้อมหลักฐาน — สี่จังหวะนี้คือสี่ commit และคุณจะได้ทำจริงในบทที่ 08, 09, 11 และ 12',
          ...
          'This course repeats four such checkpoints: one component and its story pass your review in Storybook, an interaction test passes, the page walks the full flow, and one bug is fixed with evidence. Those four moments are four commits, and you will make them in lessons 08, 09, 11, and 12.',
...
Stories reviewed in Storybook for all four states using fixed mock data.
...
Summarize how you addressed each point.`},
        ...
        {type:'commands',title:{th:'Commit checkpoint 1 · component แรกที่ผ่าน review',en:'Commit checkpoint 1 · the first reviewed component'},
          lead:{th:'สามคำสั่งนี้ **คุณเป็นคนรันเอง** ไม่ใช่ Agent (prompt ทุกอันในคอร์สห้าม Agent commit) และรันจากในโฟลเดอร์ worktree `wt-us001-patient-checkin` ทำก็ต่อเมื่อคุณเปิดดู story ครบทุก state แล้วพอใจ ถ้ายังมี state ที่ดูแล้วไม่ผ่าน อย่าเพิ่ง commit',en:'You run these three yourself — not the agent (every prompt in this course forbids the agent from committing) — from inside the `wt-us001-patient-checkin` worktree. Only after you have opened every story state and are satisfied — if one state still looks wrong, do not commit yet.'},
          steps:[
...
Also tell me how to view this test running inside Storybook.`},
        ...
        {type:'commands',title:{th:'Commit checkpoint 2 · interaction test ที่ผ่านแล้ว',en:'Commit checkpoint 2 · the passing interaction test'},
          ...
          outro:{th:'ตอนนี้ประวัติของ branch คุณเริ่มเล่าเรื่องได้แล้ว ลอง `git log --oneline` ดู จะเห็นสองบรรทัดที่บอกได้ว่าทำอะไรไปบ้างโดยไม่ต้องเปิดโค้ด',en:'Your branch history now tells a story. Run `git log --oneline` and you get two lines that explain the work without opening any code.'}},
...
Do not modify any further files in this round.`},
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'callout',tone:'danger',title:{th:'ไฟล์ที่ห้ามหลุดเข้า commit',en:'Files that must never enter a commit'},text:{th:'ก่อน stage ให้กวาดตา `git status` หาสี่อย่างนี้: ไฟล์ `.env` หรืออะไรก็ตามที่มี key/token/รหัสผ่าน, ไฟล์ข้อมูลผู้ป่วยจริงที่เผลอวางไว้ในโปรเจกต์, ผลลัพธ์ของการ build เช่น `.next/` หรือ `storybook-static/` และไฟล์ทดลองที่ Agent สร้างทิ้งไว้ เช่น `test.js` หรือ `temp-notes.md` — สามอย่างแรกลบออกจาก commit ให้หมด ส่วนอย่างสุดท้ายให้ถาม Agent ก่อนว่ายังจำเป็นหรือไม่ ถ้าไฟล์พวกนี้ถูก push ขึ้นไปแล้ว การลบทีหลังไม่ได้ลบออกจากประวัติ',en:'Before staging, scan `git status` for four things: a `.env` or anything holding a key, token, or password; real patient data accidentally dropped into the project; build output such as `.next/` or `storybook-static/`; and scratch files the agent left behind like `test.js` or `temp-notes.md`. Remove the first three from the commit entirely and ask the agent whether the last is still needed. Once such a file is pushed, deleting it later does not remove it from history.'}},
...
Keep it short and never include anything that was not actually done.`},
    ...
    },
    {
      ...
      outcomes:{th:['บอกได้ว่างานแบบไหนควรทำเป็น skill และแบบไหนไม่ควร','เก็บ template ของทีมไว้ในโปรเจกต์ให้ Agent อ่านได้','สร้าง skill สำหรับสร้าง Issue และร่าง MR ตาม template','ตั้งเส้นความปลอดภัย: Agent ร่างและเสนอคำสั่ง คนเป็นผู้อนุมัติ'],en:['Tell which work belongs in a skill and which does not','Keep the team templates inside the project where the agent can read them','Build a skill for filing issues and one for drafting MRs from those templates','Set the safety line: the agent drafts and proposes, a human approves']},
      blocks:[
        ...
        {type:'code',title:{th:'ไฟล์ที่จะได้เมื่อจบบทนี้',en:'The files you will have by the end'},
          lead:{th:'สี่ไฟล์เล็ก ๆ ที่เปลี่ยนจาก “ความรู้ในหัวคุณ” เป็น “มาตรฐานที่ทีมใช้ร่วมกัน” สังเกตว่า template อยู่ใน `docs/` เพราะเป็นของทีม ส่วน skill อยู่ในโฟลเดอร์ของเครื่องมือ',en:'Four small files that move a standard out of your head and into the team. Note that templates live under `docs/` because they belong to the team, while skills live in the tool’s own folder.'},
...
.claude/
  skills/
    create-issue/SKILL.md  <- Claude Desktop
    create-mr/SKILL.md
AGENTS.md                  <- กติกาที่ใช้ร่วมกันทุกเครื่องมือ / shared rules
.gitignore                 <- เพิ่ม .tmp/ เข้าไป / add .tmp/`,
          note:{th:'`.tmp/` คือที่พักของเนื้อหาที่ Agent ร่างก่อนส่ง ต้องใส่ไว้ใน `.gitignore` เพราะเป็นไฟล์ชั่วคราว ไม่ใช่ผลงานที่ต้องเก็บในประวัติ',en:'`.tmp/` is scratch space for bodies the agent drafts before sending. Add it to `.gitignore`: it is temporary, not work that belongs in history.'}},
        {type:'code',title:{th:'ตัวอย่าง SKILL.md สำหรับสร้าง Issue',en:'Example SKILL.md for filing an issue'},
          ...
          label:'.claude/skills/create-issue/SKILL.md',code:`---
name: create-issue
...
- Never put real patient data, tokens, or .env values into the body.`,
        ...
        {type:'code',title:{th:'ตัวอย่าง SKILL.md สำหรับร่าง MR',en:'Example SKILL.md for drafting an MR'},
          ...
          label:'.claude/skills/create-mr/SKILL.md',code:`---
name: create-mr
...
- Never edit source files while drafting.`,
        ...
        {type:'agent-setup',title:{th:'ติดตั้ง skill ในเครื่องมือที่คุณใช้',en:'Install the skills in the tool you use'},
          lead:{th:'สองเครื่องมือเก็บ “คำสั่งที่ใช้ซ้ำ” คนละที่ แต่แนวคิดเดียวกัน คือเขียนขั้นตอนไว้เป็นไฟล์ในโปรเจกต์ แล้วให้เครื่องมืออ่าน เลือกแท็บของเครื่องมือที่คุณเลือกไว้ตั้งแต่บทที่ 00',en:'The two tools keep “reusable instructions” in different places but with the same idea: write the steps into a file in the project and let the tool read it. Pick the tab for the tool you chose back in lesson 00.'},
          tools:[
            {id:'chatgpt',name:{th:'ChatGPT Desktop App',en:'ChatGPT Desktop App'},steps:[
              ...
              {title:{th:'ใส่กติกาลงใน AGENTS.md',en:'Add the rules to AGENTS.md'},what:{th:'Codex อ่าน `AGENTS.md` ของโปรเจกต์อยู่แล้วตั้งแต่บทที่ 05 เพิ่มหัวข้อใหม่ว่า เมื่อผู้ใช้ขอให้สร้าง Issue หรือ MR ให้ทำตามไฟล์ template ที่ระบุ เขียนผลลง `.tmp/` และห้ามยิงคำสั่งเองโดยไม่ได้รับอนุมัติ — คัดลอกหัวข้อ Never จากตัวอย่างสองบล็อกด้านบนมาใส่ได้เลย',en:'Codex already reads the project `AGENTS.md`, as of lesson 05. Add a section saying that when asked to create an issue or an MR it must follow the named template file, write the result into `.tmp/`, and never fire a command without approval — copy the Never lists from the two examples above.'},expect:{th:'`AGENTS.md` มีหัวข้อใหม่ที่อ้างถึงไฟล์ template ทั้งสอง',en:'`AGENTS.md` has a new section referencing both template files.'}},
              {title:{th:'เก็บ prompt ที่ใช้ซ้ำไว้เป็นไฟล์',en:'Save the reusable prompt as a file'},what:{th:'สร้าง `docs/prompts/create-issue.md` แล้ววางขั้นตอนเดียวกับตัวอย่าง SKILL.md ด้านบน เวลาจะใช้ก็เปิดไฟล์แล้วคัดลอกไปวางในแชท วิธีนี้ไม่หรูเท่า skill แต่ได้ผลลัพธ์เหมือนกันคือทุกคนในทีมใช้ขั้นตอนเดียวกัน',en:'Create `docs/prompts/create-issue.md` holding the same steps as the SKILL.md example above. To use it, open the file and paste it into the chat. Less elegant than a skill, but it achieves the same thing: everyone follows the same sequence.'},expect:{th:'เปิดไฟล์ prompt แล้วใช้งานได้ทันทีโดยไม่ต้องนึกขั้นตอนเอง',en:'You can open the prompt file and use it without recalling the steps from memory.'}},
            ...
            {id:'claude',name:{th:'Claude Desktop App',en:'Claude Desktop App'},steps:[
              {title:{th:'เก็บ template ไว้ใน docs/templates',en:'Put the templates in docs/templates'},what:{th:'สร้างสองไฟล์ตามโครงในบทที่ 02 และ 14 ให้เป็นไฟล์เปล่าที่มีแต่หัวข้อ ไม่ต้องใส่เนื้อหาของ US-001 ลงไป เพราะ template ต้องใช้กับงานถัดไปได้ด้วย',en:'Create the two files using the shapes from lessons 02 and 14, as blank forms with headings only. Do not bake US-001 content into them — a template has to fit the next task too.'},expect:{th:'`docs/templates/issue-template.md` และ `docs/templates/mr-template.md` มีอยู่จริงในโปรเจกต์',en:'`docs/templates/issue-template.md` and `docs/templates/mr-template.md` exist in the project.'}},
              {title:{th:'สร้างโฟลเดอร์ skill ในโปรเจกต์',en:'Create the skill folders in the project'},what:{th:'Claude Code อ่าน skill จากโฟลเดอร์ `.claude/skills/` ของโปรเจกต์ หนึ่ง skill คือหนึ่งโฟลเดอร์ที่มีไฟล์ชื่อ `SKILL.md` อยู่ข้างใน เมื่อวางไว้ในโปรเจกต์ ทุกคนในทีมที่ clone ไปก็ได้ skill เดียวกัน',en:'Claude Code reads skills from the project’s `.claude/skills/` folder. One skill is one folder containing a `SKILL.md`. Because it lives in the project, everyone who clones the repo gets the same skill.'},cmd:`mkdir -p .claude/skills/create-issue .claude/skills/create-mr`,expect:{th:'มีสองโฟลเดอร์ว่างรอไฟล์ `SKILL.md`',en:'Two empty folders waiting for their `SKILL.md`.'}},
              {title:{th:'เขียนไฟล์ SKILL.md ทั้งสองตัว',en:'Write both SKILL.md files'},what:{th:'คัดลอกเนื้อหาจากสองบล็อกตัวอย่างด้านบนมาวาง แล้วแก้ชื่อไฟล์ template ให้ตรงกับของจริงในโปรเจกต์ ส่วนหัวที่คั่นด้วย `---` สำคัญมาก เพราะเป็นที่ที่เครื่องมืออ่านชื่อและคำอธิบายว่าจะหยิบ skill นี้มาใช้ตอนไหน',en:'Paste the content from the two example blocks above and correct the template filenames to match the project. The `---` header matters: it is where the tool reads the name and the description that decide when this skill gets picked up.'},expect:{th:'`.claude/skills/create-issue/SKILL.md` และ `.claude/skills/create-mr/SKILL.md` มีเนื้อหาครบ',en:'Both `.claude/skills/create-issue/SKILL.md` and `.claude/skills/create-mr/SKILL.md` are complete.'}},
              {title:{th:'ทดสอบเรียกใช้',en:'Try calling it'},what:{th:'พิมพ์ `/create-issue` ในช่องแชท หรือบอกเป็นภาษาคนว่า “ช่วยร่าง Issue จาก requirement ใบถัดไปให้หน่อย” ถ้า skill ถูกอ่านแล้ว Agent จะเดินตามลำดับในไฟล์ คือถามกลับก่อน แล้วค่อยเขียนลง `.tmp/` และหยุดรอการอนุมัติ',en:'Type `/create-issue` in the chat, or just say “draft an issue for the next requirement”. If the skill was picked up, the agent follows the file’s sequence: ask back first, write into `.tmp/`, then stop for approval.'},expect:{th:'Agent ถามกลับในจุดที่ requirement ไม่ได้ตอบ และยังไม่มีคำสั่งไหนถูกรันโดยไม่ผ่านการกดยืนยัน',en:'The agent asks back where the requirement is silent, and no command runs without your confirmation.'}}
            ]}
          ],
...
แล้วสร้างไฟล์ .claude/skills/create-issue/SKILL.md ตามเงื่อนไขนี้
...
Then create .claude/skills/create-issue/SKILL.md under these conditions:
...
Create only this one file, run no commands, and show me the content first.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: ใช้ skill กับงานใบที่สอง',en:'Practice: use the skill on a second piece of work'},steps:{th:['สร้าง `docs/templates/issue-template.md` และ `docs/templates/mr-template.md` จากโครงในบทที่ 02 และ 14','เพิ่ม `.tmp/` ลงใน `.gitignore`','สร้าง skill หรือไฟล์ prompt ตามแท็บเครื่องมือของคุณ','ใช้ skill ร่าง Issue ใบที่สอง เช่น US-002 Queue Board แล้วนับว่ามันถามกลับกี่ข้อ','เทียบร่างที่ได้กับเช็กลิสต์ในบทที่ 02 ว่าข้อไหนยังขาด แล้วแก้ที่ **ไฟล์ skill** ไม่ใช่แก้ที่ร่าง','commit งานนี้: `git add docs .claude .gitignore` แล้ว `git commit -m "docs: add issue and MR templates with agent skills"`','push แล้วเปิด MR เล็ก ๆ ใบนี้ให้เพื่อน review ว่าข้อห้ามใน skill รัดกุมพอหรือยัง'],en:['Create `docs/templates/issue-template.md` and `docs/templates/mr-template.md` from the shapes in lessons 02 and 14','Add `.tmp/` to `.gitignore`','Build the skill or the prompt file, following your tool’s tab','Use it to draft a second issue, e.g. US-002 Queue Board, and count how many questions it asks back','Compare the draft against the lesson 02 checklist, then fix what is missing in the **skill file**, not in the draft','Commit the work: `git add docs .claude .gitignore` then `git commit -m "docs: add issue and MR templates with agent skills"`','Push and open this small MR for a peer to review whether the prohibitions are tight enough']},expected:{th:'Issue ใบที่สองใช้เวลาน้อยกว่าใบแรกมาก แต่ยังมีหัวข้อครบเท่ากัน และคุณยังเป็นคนตอบคำถามเรื่องขอบเขตกับ AC เองทุกข้อ',en:'The second issue takes far less time than the first while covering the same sections — and you still answer every scope and AC question yourself.'}}
      ],
      ...
      wrap:{th:['ทำเองก่อนจนมีเกณฑ์ แล้วค่อยทำให้ทำซ้ำได้','template อยู่ใน `docs/` เพราะเป็นของทีม ไม่ใช่ของคนใดคนหนึ่ง','skill กรอกฟอร์มและเสนอคำสั่ง ส่วนการกดส่งยังเป็นของคน','แก้พฤติกรรมที่ไฟล์ ไม่ใช่ที่แชท'],en:['Do it by hand until you have a standard, then make it repeatable','Templates live in `docs/` because they belong to the team, not to one person','The skill fills the form and proposes the command; pressing send stays human','Fix behaviour in the file, not in the chat']}
    },
    {
