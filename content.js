...
Answer as questions back to me. Do not answer them on my behalf.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: เขียน Issue ของ US-001 ด้วยตัวเอง',en:'Practice: write the US-001 issue yourself'},steps:{th:['เปิด `docs/requirements/US-001-opd-checkin.md` ใน starter repo แล้วอ่านให้จบก่อน','ร่าง Issue ตาม template ด้านบน โดยยังไม่เปิดกลับมาดู template ระหว่างเขียน','แยก AC ออกเป็นสองกอง คือสถานะ และพฤติกรรม แล้วนับว่าแต่ละกองมีกี่ข้อ','ส่ง Prompt ด้านบนให้ Agent แล้วจดรายการ “สิ่งที่มันจะเดา” ไว้ทุกข้อ','แก้ Issue จนรายการนั้นเหลือเฉพาะเรื่องที่คุณตั้งใจให้เป็นอิสระของ Agent เช่น ชื่อไฟล์หรือโครงสร้างโฟลเดอร์','เปิด Issue จริงตามบล็อกด้านบน แล้วจดเลข Issue ไว้','ทดลองพิมพ์ gh issue view <เลข> หรือ glab issue view <เลข> ใน terminal จนอ่าน Issue นี้ออก เพราะบทที่ 05 จะให้ Agent รันคำสั่งนี้เอง','ยังไม่ต้องเปิด branch และยังไม่ต้องเรียก Agent ให้ลงมือ — บทที่ 03 และ 04 จะพาไปเปิดพื้นที่ทำงานเอง'],en:['Open `docs/requirements/US-001-opd-checkin.md` in the starter repo and read it through first','Draft the issue using the template above, without looking back at it while you write','Sort your ACs into two piles — states and behaviours — and count each pile','Send the prompt above and write down every item the agent says it would guess','Revise the issue until that list contains only what you deliberately leave to the agent, such as file names or folder structure','File the real issue using the block above and note its number','Try gh issue view <number> or glab issue view <number> in a terminal until it prints the issue — lesson 05 has the agent run this exact command','Do not open a branch or call the agent yet — lessons 03 and 04 set up the workspace']},expected:{th:'ได้ Issue หนึ่งใบที่มี Context, In/Out of scope, AC แยกสถานะกับพฤติกรรม และ Definition of Done โดยไม่มีข้อใดที่ต้องตีความ และคุณอธิบายได้ว่าทำไมแต่ละข้อใน Out of scope ถึงถูกตัดออก รวมถึงมีเลข Issue จริงที่อ่านกลับได้ด้วย gh issue view หรือ glab issue view',en:'One issue with context, in/out of scope, ACs split into states and behaviours, and a definition of done — with nothing left to interpretation, and you can explain why each out-of-scope item was cut, plus a real issue number you can read back with gh issue view or glab issue view.'}}
      ],
...
Keep it short and never include anything that was not actually done.`},
  ...
  templates/
    ...
    mr-template.md         <- โครงจากบทที่ 15 / the shape from lesson 15
.agents/
...
- Never edit source files while drafting.`,
        ...
        {type:'agent-setup',title:{th:'ติดตั้ง skill ในเครื่องมือที่คุณใช้',en:'Install the skills in the tool you use'},
          ...
          tools:[
            {id:'chatgpt',name:{th:'ChatGPT Desktop App',en:'ChatGPT Desktop App'},steps:[
              {title:{th:'เก็บ template ไว้ใน docs/templates',en:'Put the templates in docs/templates'},what:{th:'สร้างสองไฟล์ตามโครงในบทที่ 02 และ 15 ให้เป็นไฟล์เปล่าที่มีแต่หัวข้อ ไม่ต้องใส่เนื้อหาของ US-001 ลงไป เพราะ template ต้องใช้กับงานถัดไปได้ด้วย',en:'Create the two files using the shapes from lessons 02 and 15, as blank forms with headings only. Do not bake US-001 content into them — a template has to fit the next task too.'},expect:{th:'`docs/templates/issue-template.md` และ `docs/templates/mr-template.md` มีอยู่จริงในโปรเจกต์',en:'`docs/templates/issue-template.md` and `docs/templates/mr-template.md` exist in the project.'}},
