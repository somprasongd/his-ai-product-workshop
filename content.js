...
- Print layout for the visit slip is not covered.`,
        ...
        {type:'prompt',title:{th:'Prompt: ให้ Agent สรุป Design เป็น DESIGN.md',en:'Prompt: have the agent summarize the design into DESIGN.md'},
          when:{th:'ใช้เมื่อมีภาพอ้างอิงพร้อมแล้ว เช่น screenshot ของหน้า /opd/check-in ปัจจุบัน หรือไฟล์ Figma/ภาพ mockup ที่ทีม Design ส่งมา แนบภาพเข้าไปในแชทของ Agent ก่อนส่ง prompt นี้',en:'Use it once you have a reference image ready — a screenshot of the current /opd/check-in page, or a Figma/mockup image the design team handed off. Attach the image to the agent’s chat before sending this prompt.'},
...
Tell me which URL to open to see the result.`},
        ...
        {type:'practice',title:{th:'ลงมือทำ: จากภาพสู่ Style Guide',en:'Practice: from image to style guide'},steps:{th:['เตรียมภาพอ้างอิง — screenshot หน้า /opd/check-in ปัจจุบัน หรือภาพ mockup ที่ได้รับมา','แนบภาพแล้วส่ง Prompt สรุป DESIGN.md ให้ Agent','ตรวจ DESIGN.md ด้วย checklist ด้านบนจนพอใจ','ส่ง Prompt สร้างหน้า /design แล้วเปิดเทียบกับ DESIGN.md','ลองแก้ token หนึ่งค่าดูว่าหน้า Style Guide เปลี่ยนตามหรือไม่'],en:['Prepare a reference image — a screenshot of the current /opd/check-in page, or a received mockup','Attach it and send the DESIGN.md summarization prompt to the agent','Review DESIGN.md against the checklist above until satisfied','Send the /design page prompt and compare the result against DESIGN.md','Change one token value and check whether the style guide page follows it']},expected:{th:'มีไฟล์ DESIGN.md ที่ตรวจแล้วผ่าน และหน้า /design ที่แสดง token ทุกกลุ่มตรงกับไฟล์ ยังไม่ commit',en:'A reviewed, passing DESIGN.md file and a /design page that displays every token group matching it. Not committed yet.'}}
      ],
    ...
    },
    {
      ...
      blocks:[
        ...
        {type:'capstone',steps:[
          ...
          {title:{th:'4 · Design System',en:'4 · Design System'},hint:{th:'แนบภาพ reference ก่อนสั่ง Agent สรุป DESIGN.md อย่าปล่อยให้มันเดาแบรนด์เอง',en:'Attach a reference image before asking the agent to summarize DESIGN.md — do not let it guess the brand'},guide:{th:['เตรียมภาพ reference (screenshot หน้า /opd/check-in ปัจจุบัน หรือ mockup ที่ได้รับมา)','ส่ง Prompt สรุป DESIGN.md จากบทที่ 07 พร้อมภาพแนบ','ตรวจ DESIGN.md ด้วย checklist ในบทที่ 07 ก่อนไปต่อ','ส่ง Prompt สร้างหน้า /design แล้วเทียบกับ DESIGN.md','ยังไม่ commit — จะรวมกับ Commit checkpoint 1 ในขั้นถัดไป'],en:['Prepare a reference image (a screenshot of the current /opd/check-in page, or a received mockup)','Send the DESIGN.md summarization prompt from lesson 07 with the image attached','Check DESIGN.md against the lesson 07 checklist before moving on','Send the /design page prompt and compare it against DESIGN.md','Do not commit yet — it bundles into Commit checkpoint 1 in the next step']}},
